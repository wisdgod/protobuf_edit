//! The groupless editing draft: handle-based edits with precise
...
//! — at the root that refusal stops the open (returning the
//! buffer), inside a payload it is a resident verdict and the
//! payload stays readable as bytes.
//!
...
//! [`Draft::descend`], whose verdict is resident.
//!
...
//! save the source again, padding included.
//!
...
//!
//! Coordinates: write · buffered · offline · groupless · tolerant (type-level) · owned · revisable.
//!
//! # Examples
//!
//! ```
...
//! let msg = vec![0x08, 0x96, 0x81, 0x00, 0x12, 0x02, 0x68, 0x69];
//! let mut draft = Draft::open(msg).unwrap();
//! let tops: Vec<_> = draft.top().collect();
//! assert_eq!(draft.payload_bytes(tops[1]).unwrap(), [0x68, 0x69]);
//!
...
crate::revise::groupless::revising_machine! {
    ...
    doc_mod: "protobuf_edit::draft::groupless",
}
...
crate::revise::groupless::revising_machine! {
    ...
    #[doc = " `TransferBorrowDraft` (feature `transfer-draft-groupless`) add"]
...
crate::revise::groupless::revising_machine! {
    ...
    #[doc = ""]
    #[doc = " ```"]
    #[doc = " use protobuf_edit::draft::groupless::BorrowDraft;"]
    #[doc = ""]
...
crate::revise::groupless::revising_machine! {
    ...
    #[doc = ""]
    #[doc = " ```"]
    #[doc = " use protobuf_edit::draft::groupless::MixDraft;"]
    #[doc = ""]
