//! The groupless retained inspector: group codes are a capability
//! refusal.
//!
...
//! the format's unassigned codes. One parse over the moved-in
//! buffer builds a preorder row table, the product is total (legal
//! prefix + at most one fault), and detection stays blind to
//! commitment. The product owns source and rows together —
...
//!
//! Coordinates: read · buffered · offline · groupless · Standard (value-level) · owned.
//!
//! # Examples
//!
//! ```
...
#[cfg(all(test, feature = "collect-groupless"))]
...
/// fault. The type is its own provenance proof: `source` and
/// `rows` are private, minted together by the one parse, and
/// immutable for the product's life — the unsafe discharges in the
/// value queries cite exactly this invariant. Rows hold `u32`
/// coordinates, never pointers, so moving the product moves
/// nothing but the two owners.
///
...
impl Retained {
    ...
    /// use protobuf_edit::retain::NoAdvice;
    /// use protobuf_edit::retain::groupless::{FaultKind, Retained};
    ///
    /// // An empty group of field 1 — well-formed wire outside
    /// // this language.
    /// let msg = vec![0x0B, 0x0C];
    /// let tree = Retained::parse(msg, DepthLimit::REFERENCE, &mut NoAdvice).unwrap();
    ...
    /// word against its minimal encoding — the `NonMinimal*`
    /// faults, at the construct's first byte, exactly where the
    /// stream scanner's canonical validator judges them.
    ///
    ...
    /// use protobuf_edit::DepthLimit;
    /// use protobuf_edit::retain::NoAdvice;
    /// use protobuf_edit::retain::groupless::{RecordSpans, Retained};
    ///
    /// // LEN f2 "hi": tag, length prefix, payload — in wire order.
    /// let msg = vec![0x12, 0x02, 0x68, 0x69];
    /// let tree = Retained::parse(msg, DepthLimit::REFERENCE, &mut NoAdvice).unwrap();
...
/// disposition alone reads it. The machine borrows the buffer the
/// door admitted; the product assembles from its parts once the
/// borrow ends.
...
impl<A: Advisor> Machine<'_, '_, A> {
    ...
    fn record<const MINIMAL: bool>(&mut self) -> Result<(), Stop> {
        ...
        // at open, the root is the admitted length), so
        // `self.zone <= bytes.len()`.
        let (word, tag_width) =
