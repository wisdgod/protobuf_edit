//! The groupless retained inspector's module suite: the borrowed
    ...
    TA: crate::inspect::Advisor,
{
    ...
    for (kid, tid) in kept.nodes().zip(tree.nodes()) {
        ...
        assert_eq!(
            kept.parent(kid).map(super::NodeId::as_inner),
    ...
    for pos in 0..=crate::admission::admitted_u32(bytes.len()) + 1 {
        assert_eq!(
            kept.narrowest(pos).map(super::NodeId::as_inner),
...
fn the_canonical_engine_names_each_padded_construct() {
    ...
    let minimal = [0x08, 0x96, 0x01, 0x12, 0x02, 0x68, 0x69];
...
fn every_query_face_agrees_with_the_borrowed_tree() {
    let fixtures: &[&[u8]] = &[
        // Empty message.
        &[],
        // varint f1=150 · LEN f2 "hi" (payload parses; f2 commits
        // under Pin, speculates under NoAdvice).
        &[0x08, 0x96, 0x01, 0x12, 0x02, 0x68, 0x69],
        // Scalars of every kind: varint, i32, i64.
        &[0x08, 0x2A, 0x15, 0x01, 0x02, 0x03, 0x04, 0x19, 1, 2, 3, 4, 5, 6, 7, 8],
        // Nested LENs three deep: f2 { f2 { f1 varint } }.
        &[0x12, 0x06, 0x12, 0x04, 0x12, 0x02, 0x08, 0x01],
        // LEN f3 whose payload parses (Opaque under Pin pins it to
        // bytes; NoAdvice speculates it open).
        &[0x1A, 0x02, 0x08, 0x01],
        // LEN f1 whose payload is not a message: speculation
        // absorbs, and no advisor commits f1.
        &[0x0A, 0x02, 0xFF, 0x01],
        // Padded framing: padded tag, padded value — tolerant
        // rides them, canonical faults at the first.
        &[0x88, 0x00, 0x96, 0x01],
        &[0x08, 0x96, 0x81, 0x00],
        // Truncated at the root: legal prefix plus a fault.
        &[0x08, 0x96, 0x01, 0x08],
        // A group code: this dialect's capability refusal.
        &[0x0B, 0x0C],
        // A declared length puncturing its seal.
        &[0x12, 0x05, 0x01],
        // Committed payload carrying a real fault (f2 commits under
        // Pin: the fault is real; NoAdvice absorbs it).
        &[0x12, 0x01, 0x08],
    ];
    for bytes in fixtures {
        agree_all_standards(bytes, DepthLimit::REFERENCE);
        // A tight bound exercises demote-to-opaque and the
        // committed depth fault.
