//! The grouped one-shot intake: owned input under transactional
...
//! [`Intake::descend`], whose verdict is resident — a wire fault
//! or a refusal inside a payload parks on the record and the
//! intake lives on. Descent is an explicit commitment: nothing
//! here speculates a payload into a message.
//!
...
//! canonical admission.
//!
//! Coordinates: write · buffered · offline · grouped · canonical (type-level) · owned · commit-only.
//!
//! # Examples
//!
//! ```
...
crate::editor::grouped::one_shot_machine! {
    ...
    /// `transfer-intake-grouped`) adds whole-record relocation,
    ...
    a_noun: "an intake",
    A_noun: "An intake",
    doc_mod: "protobuf_edit::intake::grouped",
