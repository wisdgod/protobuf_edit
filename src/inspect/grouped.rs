//! The grouped inspector: groups walk structurally.
//!
...
//! supply sound).
//!
//! Coordinates: read · buffered · offline · grouped · Standard (value-level) · borrowed.
//!
//! # Examples
//!
//! ```
...
impl<'a> Tree<'a> {
    ...
    /// exactly where the stream scanner's canonical validator
    /// judges them.
    ///
    ...
    /// ```
    /// use protobuf_edit::DepthLimit;
    /// use protobuf_edit::inspect::grouped::{RecordSpans, Tree};
    /// use protobuf_edit::inspect::{Admitted, NoAdvice};
    ///
    /// // group f2 { varint f3=1 }
    /// let msg = [0x13, 0x18, 0x01, 0x14];
    /// let input = Admitted::new(&msg).unwrap();
    /// let tree = Tree::parse(input, DepthLimit::REFERENCE, &mut NoAdvice);
