//! The groupless inspector: group codes are a capability refusal.
//!
//! Groupless toolchains inspect their own traffic; a group code
//! (3 or 4) is well-formed wire *outside this language* — refusing
//! it is this dialect's correctness feature, typed distinctly from
//! the format's unassigned codes. One parse over [`Admitted`] bytes
//! builds a preorder row table, the product is total (legal prefix
//! + at most one fault), and detection stays blind to commitment.
//!
...
//!
//! Coordinates: read · buffered · offline · groupless · Standard (value-level) · borrowed.
//!
//! # Examples
//!
//! ```
...
use super::{
    Admitted, Advice, Advisor, Ancestry, NodeId, RowCount, Stage, frames_reserve, mint,
    rows_reserve,
};
...
/// tree's life — the unsafe discharges in the value queries cite
...
impl<'a> Tree<'a> {
    /// Parses the admitted bytes. Total: admission is the argument's
    /// type, and faults are product, not error.
    ///
    ...
    /// ```
    #[must_use]
    pub fn parse<A: Advisor>(input: Admitted<'a>, depth: DepthLimit, advice: &mut A) -> Self {
        Self::parse_standard(input, Standard::Tolerant, depth, advice)
    }
    ...
    /// construct's first byte, exactly where the stream scanner's
    /// canonical validator judges them.
    ///
    ...
    ///
    /// let tolerant =
    ///     Tree::parse_standard(input, Standard::Tolerant, DepthLimit::REFERENCE, &mut NoAdvice);
    ...
    pub fn parse_standard<A: Advisor>(
        ...
        let machine = Machine {
            ...
            path: Vec::new(),
            rows: Vec::with_capacity(rows_reserve(input.len())),
            limit: depth,
            advice,
        };
        match standard {
    ...
    pub const fn node_count(&self) -> u32 {
        RowCount::of(self.rows.len()).as_inner()
    }
    ...
    /// use protobuf_edit::inspect::groupless::Tree;
    ///
    /// // varint f1=150 (value padded): the designation carries the
    /// // exact spelling.
    /// let msg = [0x08, 0x96, 0x81, 0x00];
    /// let input = Admitted::new(&msg).unwrap();
    /// let tree = Tree::parse(input, DepthLimit::REFERENCE, &mut NoAdvice);
...
impl<'a, A: Advisor> Machine<'a, '_, A> {
    ...
    fn seal_descendants(&mut self, row: NodeId) {
        let idx = row.index();
        let descendants = RowCount::of(self.rows.len() - 1 - idx);
    ...
    fn run<const MINIMAL: bool>(mut self) -> Tree<'a> {
        loop {
    ...
    fn len_record<const MINIMAL: bool>(
        ...
        match advice {
            ...
            Advice::Speculate | Advice::Commit => {
                ...
                let row = mint(RowCount::of(self.rows.len()).as_inner());
                self.push_leaf(
                    at,
                    field,
                    RecordKind::Len,
    ...
    fn clip(mut self, failure: Failure) -> Tree<'a> {
    ...
    fn finish(self, fault: Option<Fault>) -> Tree<'a> {
        let end = self.input.end();
        self.finish_at(end, fault)
    }

    fn finish_at(self, indexed_end: Coord, fault: Option<Fault>) -> Tree<'a> {
        Tree { input: self.input, rows: self.rows.into_boxed_slice(), indexed_end, fault }
    }
}
