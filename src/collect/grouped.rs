//! The grouped stream collector: groups walk structurally.
//!
...
//! owned inspector publishes them: full depth, advisor-driven for
//! LEN payloads (opaque leaves, speculative descent with absorber
//! demotion, committed descent), structural for groups — a group
//! row publishes provisionally at its opening tag because
//! descendants name it as parent, and its close writes the
//! interior length, end-tag width, and descendant count. The
//! consuming [`Collector::finish`] seals the accumulated source
//! and the finished preorder rows into the same product one
//! buffered parse would have built over the concatenated bytes,
//! reading no byte again.
//!
//! A stream does not know its end until `finish`, and that changes
//! exactly one judgment: a LEN record met while no finite extent
//! encloses it — at the root layer, however many groups are open,
//! since groups do not seal a new extent — cannot be checked
//! against the document end the way the buffered parse checks it
//! first of all. The collector holds one open root-layer LEN
//! transaction: an O(1) checkpoint of the row, stack, and path
//! marks taken before advice or row publication. While the
//! declared endpoint is unproven, parsing continues normally
//! inside the declared extent; an unabsorbed fault met there —
//! wire, group, or depth — is *deferred* (its clip is delayed and
//! later bytes copy without parsing) because the buffered parse
//! would never reach it if the stream ends short. Proof commits
//! the deferred fault with its ordinary clip; an end-of-stream
//! underfill restores the checkpoint — enclosing group frames
//! survive it and clip at the overrun's cut, exactly the buffered
//! geometry. A declared endpoint past the coordinate class enters
//! copy-only collection at once, consulting nothing. At most one
//! transaction can be open: entering the LEN seals a finite extent
...
//! supplied stream beside its fault. The only feed error is the
//! pre-read coordinate refusal ([`FeedOversize`]), which returns
//! every previously absorbed byte and spends the collector.
//! Speculation unwinds across chunk edges without re-reading a
//! byte: truncation back to the absorber row, register
//! restoration, and an in-feed skip to the absorber's endpoint.
//!
//! Coordinates: read · stream · offline · grouped · Standard (value-level) · owned.
//!
//! # Examples
//!
//! ```
...
//!
//! // varint f1=150 · group f2 { varint f3=1 }, fed byte by byte:
//! // chunk edges never show in the product.
...
impl Fault {
    ...
    /// stream length, one past the last byte).
    #[inline]
    #[must_use]
...
/// variant elsewhere. End-of-stream truncations are judged at
/// `finish` and quote the same vocabulary a buffered parse quotes
/// at its extent end.
#[non_exhaustive]
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
pub enum FaultKind {
    ...
    /// appeared (groups may not cross LEN boundaries, and the
    /// stream's end closes nothing).
    GroupUnclosed {
...
impl FaultKind {
    ...
    /// is the collection's declared [`Standard`]); this dialect has
...
struct Row {
    ...
    /// ≤ 10, group interiors ≤ stream length).
    payload_len: Extent,
...
impl Retained {
    ...
    /// ```
    /// use protobuf_edit::collect::NoAdvice;
    /// use protobuf_edit::collect::grouped::{Collector, RecordSpans};
    /// use protobuf_edit::{DepthLimit, Standard};
    ///
    ...
    /// let mut collector =
    ///     Collector::new(Standard::Tolerant, DepthLimit::REFERENCE, &mut advice);
    /// collector.feed(&[0x13, 0x18, 0x01, 0x14]).unwrap();
    /// let tree = collector.finish();
    ...
    fn structural_nesting(&self, id: NodeId) -> u32 {
        ...
        for index in first..first + root.descendants {
            ...
            // whose rows the collection minted in-table.
...
/// `finish`, so the root extent has no endpoint — and groups
/// inherit it, since a group seals no new extent. Feed admission
...
/// publish whole when their extents complete (group rows publish
/// provisionally at their opening tag and finish at close).
#[derive(Clone, Copy)]
enum Resume {
    /// Between records; the carry may hold a partial tag (a head
    /// tag or a group end tag — classification tells them apart).
    Head,
...
/// for everything inside it, and groups seal nothing, so a second
/// unsealed-layer LEN only exists after this one resolves — and no
/// absorber can enclose it (an enclosing speculative LEN would
/// have installed a finite zone; the frames above the checkpoint
/// are all groups), so restoration re-arms the root registers
/// directly. Enclosing group frames sit below `stack_base` and
/// survive restoration; the overrun's clip closes them.
struct RootLenTxn {
    /// Row count at the checkpoint (restoration truncates here).
    rows_base: u32,
    /// Stack depth at the checkpoint: the enclosing open groups.
    stack_base: u16,
...
struct Core<'a, A: Advisor> {
    ...
    /// root layer; groups inherit) — maintained by open and close,
    /// never recomputed.
    zone: u32,
    ...
    /// conditional and evaporates with the unwind — and the root
...
impl<'a, A: Advisor> Core<'a, A> {
    ...
    /// as "bytes" (truncation — `descendants` and group end facts
    /// are written only at close, so no written state needs repair
    /// — then an in-feed skip to the absorber's endpoint); an
    /// unproven root transaction defers the fault the buffered
    /// parse may never reach; a committed fault clips now and
    /// latches the fault tail.
    #[cold]
    ...
    /// Commits a fault: clips every open container to the
    /// failure's `cut` with the same close-time writes as a normal
    /// pop — groups clip to the boundary (their end tag never
    /// appeared), LEN rows keep their declared spans — discards
    /// any resolved root transaction, and latches the fault tail.
    #[cold]
    ...
    /// reached: a LEN closes clean and may resolve the root
    /// transaction; a group standing at a LEN's endpoint is
    /// unterminated — its end tag can no longer appear inside the
    /// seal — and faults. A fault disposition changes the state,
    ...
    fn cascade(&mut self) {
        while self.pos() == self.zone {
            ...
            match frame.kind {
                ...
                FrameKind::Group => {
                    // The popped row is clipped here; the
                    // disposition and any final clip only see the
                    // remaining stack.
                    let at = self.zone;
                    self.clip_group_row(frame.row, at);
                    self.restore(&frame);
                    self.fault(at, at, FaultKind::GroupUnclosed { open: frame.field });
                    return;
                }
            }
        }
    ...
    /// its value stage, its container frame, or its group close.
    #[allow(
        clippy::as_conversions,
    ...
    fn head<const MINIMAL: bool>(&mut self, rest: &mut &[u8]) {
        ...
        match classify(low3) {
            ...
            TagClass::Record(RecordKind::Group) => self.group_open(head),
            TagClass::GroupEnd => self.group_close(head),
            TagClass::Unassigned => {
    ...
    /// the backing).
    const fn word_start(&self) -> u32 {
        self.pos() - self.word.width as u32
    }
    ...
    /// Admits a fixed head against its extent — a sealed extent is
    /// known, so a short one refuses before any payload byte
    /// arrives, exactly where the buffered parse refuses it (the
    /// unsealed root defers to the stream's end) — then collects
    ...
    /// Opens a group: a provisional row publishes at the opening
    /// tag (descendants will name it as parent), the frame inherits
    /// its opener's zone, and close writes the remaining facts.
    fn group_open(&mut self, head: PendingHead) {
        if self.at_depth_limit() {
            let kind = FaultKind::DepthExceeded { field: head.field, limit: self.limit };
            return self.fault(head.start, head.start, kind);
        }
    ...
    fn group_close(&mut self, head: PendingHead) {
        let frame = match self.stack.last() {
            ...
            _ => {
                return self.fault(
                    head.start,
                    head.start,
                    FaultKind::GroupEndOrphan { found: head.field },
                );
            }
        };
        if frame.field != head.field {
            let kind = FaultKind::GroupEndMismatch { open: frame.field, found: head.field };
            return self.fault(head.start, head.start, kind);
        }
        self.stack.pop();
        self.restore(&frame);
        ...
        // class.
        r.payload_len = unsafe { Extent::new_unchecked(head.start - body_start) };
        r.delim_width = Some(head.tag_width);
        r.descendants = descendants;
        self.state = ParseState::Live(Resume::Head);
    }
    ...
    /// an unsealed layer — the root, however many groups are open —
    /// has the stream's unknown end for its seal, so the head opens
    /// the root transaction instead: checkpoint first, then the
    /// ordinary advice flow (or copy-only collection for an
    /// endpoint no admissible stream can fill).
    #[allow(
        clippy::as_conversions,
    ...
    fn len_head(&mut self, head: PendingHead, declared: PayloadLen, prefix_width: WordWidth) {
        ...
        if self.zone == ROOT_ZONE {
            // The one-transaction law: entering an unsealed-layer
            // LEN seals a finite extent for everything inside it
            // (groups seal nothing), so no second transaction can
            // open before this one resolves.
    ...
    fn advised_len(
        ...
        match advice {
            ...
            Advice::Speculate | Advice::Commit => {
                ...
                self.open(row, head.field, FrameKind::Len, payload_end, absorbing);
                self.state = ParseState::Live(Resume::Head);
            }
        }
    }
    ...
    fn drive<const MINIMAL: bool>(&mut self, mut rest: &[u8]) {
        ...
        loop {
            ...
            // sealed frames (a group unterminated at its seal
            // faults, and the disposition owns the state, so the
            // loop re-judges), park on a drained chunk only while
            ...
            if self.word.width == 0 {
                self.cascade();
                if !matches!(self.state, ParseState::Live(Resume::Head)) {
                    continue;
                }
            }
            if rest.is_empty() && self.pos() < self.zone {
    ...
    /// interior fault — the buffered precedence; enclosing group
    /// frames survive and clip at the overrun's cut); a committed
    /// fault keeps its clipped rows and the full accumulated
    /// source; a word or payload the end cut becomes the buffered
    /// extent-end fault; a group still open at the end never
    /// closes; then the already-final rows box and the source
    /// moves — no byte is read again.
    fn seal(mut self) -> Retained {
        ...
        if let Some(txn) = self.root_len.take() {
            ...
            // actually left. The enclosing groups clip at its cut.
            ...
            let eof = match resume {
                Resume::Head if self.word.width == 0 => None,
                Resume::Head => {
                    let at = self.word_start();
                    Some(Failure {
                        fault: Fault {
            ...
            if let Some(failure) = eof {
                ...
                // A clean boundary under open containers: every
                // remaining frame is a group (a LEN frame's finite
                // seal lies at or inside the transaction's
                // endpoint, and the transaction arm consumed that
                // case), and the stream's end is the extent end
                // that arrived first — the innermost group is
                // unterminated, and the clip closes the rest.
                debug_assert!(matches!(frame.kind, FrameKind::Group));
                self.clip_group_row(frame.row, end);
                self.restore(&frame);
                let fault = Fault { at: end, kind: FaultKind::GroupUnclosed { open: frame.field } };
                self.commit(Failure { fault, cut: end });
            }
        }
...
impl<'a, A: Advisor> Collector<'a, A> {
    ...
    /// comparison), `depth` bounds container nesting — groups and
    /// LEN descents alike — with no default, and `advice` is
    /// consulted at every LEN record head (empty payloads
    /// included; groups are structural and never consult).
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// let mut collector =
    ///     Collector::new(Standard::Tolerant, DepthLimit::REFERENCE, &mut advice);
    /// collector.feed(&[0x0B, 0x0C]).unwrap(); // an empty group
    ...
    /// use protobuf_edit::{DepthLimit, Standard};
    ///
    /// // A group frame split across feeds.
    /// let mut advice = NoAdvice;
    /// let mut collector =
    ///     Collector::new(Standard::Tolerant, DepthLimit::REFERENCE, &mut advice);
    /// collector.feed(&[0x13, 0x18]).unwrap();
    /// collector.feed(&[0x01, 0x14]).unwrap();
    /// let tree = collector.finish();
    ...
    /// an unclosed group, an underfilled root-layer LEN) becomes
    /// the product's fault, with the complete accumulated source
    /// beside it. No source byte is read again: the seal judges
    /// carried state, boxes the finished rows, and moves the
    /// source.
    ///
    /// # Panics
    ///
    ...
    /// ```
    /// use protobuf_edit::collect::NoAdvice;
    /// use protobuf_edit::collect::grouped::{Collector, FaultKind};
    /// use protobuf_edit::{DepthLimit, Standard};
    ///
    /// // A group left open by the stream's end: the product is the
    /// // clipped prefix plus the fault.
    /// let mut advice = NoAdvice;
    /// let mut collector =
    ///     Collector::new(Standard::Tolerant, DepthLimit::REFERENCE, &mut advice);
    /// collector.feed(&[0x0B, 0x08, 0x01]).unwrap();
    /// let tree = collector.finish();
