//! The grouped streaming rewirer: the full six-code wire language
//! including legacy start/end-group framing.
//!
...
//! `FnMut(&[u8])`. Groups cross by syntax and never move the
//! length seal, so in this dialect the free layer is the root
//! *and every pure group chain*: admission refuses no path shape
//! for variable-length actions (any crossing may be a group), and
//! a match the document nevertheless puts under an entered LEN
//! faults at the record as [`RuleFaultKind::Cascade`] — the
//! authoring declared group ancestry, the document showed a LEN.
//!
//! A group is a record: a path targeting a group field applies its
//! action to the whole tree — [`Action::Delete`] suppresses the
//! subtree to its matching end (wire law still runs inside),
//! [`Action::Insert`] emits before the open tag and the tree rides
//! as if unmatched, and [`Action::Rewrite`] is a kind mismatch (no
//! value kind answers a group).
//!
//! Coordinates: write · stream · static · grouped · Standard (value-level) · commit-only.
//!
//! # Examples
//!
//! Deleting a root group deletes its whole tree:
//!
//! ```
...
//!
//! // group f1 { varint f2=7 } · varint f3=1
//! let msg = [0x0B, 0x10, 0x07, 0x0C, 0x18, 0x01];
//! let mut out = Vec::new();
...
impl<'r> Actions<'r> {
    ...
    /// [`ActionError::OversizeReplacement`] for a [`Value::Len`]
    /// replacement no announced length could equal. This dialect
    /// never mints [`ActionError::CascadeUnsound`]: any crossing
    /// may be a group, so every path shape has a possible free
    /// match — the judgment moves to the match, as
...
pub enum RuleFaultKind {
    ...
    /// the mismatch is judged at the match (a group record
    /// mismatches every value kind).
    KindMismatch {
        /// The offending path.
        path: u16,
    },
    /// A variable-length action matched under an entered LEN: the
    /// authoring declared free (group) ancestry and the document
    /// put the match under a sealed length — the declaration is
    /// proven wrong at this record.
    Cascade {
        /// The offending path.
        path: u16,
    },
...
/// endpoint (the live one rides the pump); group frames keep
/// their field for end-tag matching.
enum Frame {
    Len { prev_zone: u64 },
    Group { field: FieldNumber },
}

/// The one-pass grouped streaming rewirer.
///
...
pub struct Rewirer<'r> {
    ...
    actions: &'r [Action<'r>],
    stage: StagedHead,
    /// Suppression floor: the deleted group's frame index.
    floor: Option<usize>,
}
...
impl<'r> Rewirer<'r> {
    ...
    /// # Examples
    ///
    /// A variable-length action whose match sits under an entered
    /// LEN is the caller's schema declaration proven wrong —
    /// faulted at the record:
    ///
    /// ```
    /// use protobuf_edit::path::{Program, Segment};
    /// use protobuf_edit::rewire::Action;
    /// use protobuf_edit::rewire::grouped::{Actions, Fault, Rewirer, RuleFaultKind};
    /// use protobuf_edit::rewire::Standard;
    /// use protobuf_edit::{DepthLimit, FieldNumber};
    ///
    /// let f = |n| FieldNumber::new(n).unwrap();
    /// // "f1 is a group": delete f2 inside it.
    ...
    /// let actions = Actions::over(&program, &table).unwrap();
    ///
    /// // But the document spells f1 as a LEN: the free-ancestry
    /// // declaration breaks at the matched record.
    /// let msg = [0x0A, 0x02, 0x10, 0x07];
    /// let mut rw = Rewirer::new(&actions, Standard::Tolerant, DepthLimit::REFERENCE);
    /// let fault = rw.feed(&msg, &mut |_: &[u8]| {}).unwrap_err();
    /// let Fault::Rule(rule_fault) = fault else { unreachable!() };
    /// assert!(matches!(rule_fault.kind(), RuleFaultKind::Cascade { path: 0 }));
    /// ```
    #[track_caller]
    ...
    fn drive<O: FnMut(&[u8]), const MINIMAL: bool>(
        ...
        loop {
            ...
            // a suppressed subtree never commits a LEN — so no
            // endpoint ever arrives suppressed.
    ...
    /// use protobuf_edit::path::{Program, Segment};
    /// use protobuf_edit::rewire::Action;
    /// use protobuf_edit::rewire::grouped::{Actions, Fault, Rewirer, WireBreach};
    /// use protobuf_edit::rewire::Standard;
    /// use protobuf_edit::{DepthLimit, FieldNumber};
    ///
    /// let f1 = FieldNumber::new(1).unwrap();
    ...
    /// let actions = Actions::over(&program, &table).unwrap();
    ///
    /// // A group of field 2 opened and never closed.
    /// let mut rw = Rewirer::new(&actions, Standard::Tolerant, DepthLimit::REFERENCE);
    /// rw.feed(&[0x13], &mut |_: &[u8]| {}).unwrap();
    /// let fault = rw.finish().unwrap_err();
    /// assert!(matches!(fault, Fault::Wire { at: 1, breach: WireBreach::Grouping }));
    /// ```
    #[track_caller]
    pub fn finish(mut self) -> Result<(), Fault> {
        ...
        match self.stack.last() {
            Some(&Frame::Len { .. }) => return Err(self.wire(at, WireBreach::Truncated)),
            Some(&Frame::Group { .. }) => return Err(self.wire(at, WireBreach::Grouping)),
            None => {}
        }
        Ok(())
    }
    ...
    fn head<O: FnMut(&[u8]), const MINIMAL: bool>(
        ...
        match classify(low3) {
            ...
            TagClass::Record(RecordKind::Group) => self.group_open(field, start, out),
            TagClass::GroupEnd => self.group_end(field, out),
            TagClass::Unassigned => self.halt_wire(start, WireBreach::Tag),
        }
    }

    /// Stages the completed head for the record's action and frees
    /// the carry for the value construct. Suppressed records skip
    /// the copy: nothing inside a deleted tree is probed or
    ...
    fn group_open<O: FnMut(&[u8])>(
        ...
        match hits {
            ...
            Hits::One(id) => match self.action(id) {
                Action::Rewrite(_) => {
                    return self.halt_breach(at, RuleFaultKind::KindMismatch { path: id });
                }
                Action::Delete => {
                    if self.pump.locked() {
                        return self.halt_breach(at, RuleFaultKind::Cascade { path: id });
                    }
                    // The staged child states stay uncommitted; the
                    // next probe's staging clears them.
                ...
                Action::Insert(bytes) => {
                    ...
                    // Terminal insertion: the bytes emit, then the
                    // tree rides as if unmatched (walked below).
                    out(bytes);
                }
            },
            Hits::Conflict(first, second) => {
                return self.halt_breach(at, RuleFaultKind::Conflict { first, second });
            }
        }
    ...
    fn group_end<O: FnMut(&[u8])>(
        &mut self,
        end: FieldNumber,
        out: &mut O,
    ) -> ControlFlow<Result<(), Fault>> {
        let at = self.pump.construct_start();
        match self.stack.last() {
            None | Some(&Frame::Len { .. }) => self.halt_wire(at, WireBreach::Grouping),
            ...
            Some(&Frame::Group { .. }) => {
                ...
                if self.floor == Some(self.stack.len()) {
                    // The deleted tree's own end tag: suppression
                    // ends silently (the matcher never descended).
                    self.floor = None;
    ...
    fn varint_value<O: FnMut(&[u8]), const MINIMAL: bool>(
        ...
        if self.suppressed() {
            ...
            // consumer inside a deleted tree.
        } else {
    ...
    fn fixed_tail<O: FnMut(&[u8])>(
        ...
        if self.suppressed() {
            // Collected under wire law; no consumer inside a
            // deleted tree.
        } else {
            match self.matcher.probe_target(field) {
                Hits::None => {
    ...
    fn len_word<O: FnMut(&[u8]), const MINIMAL: bool>(
        ...
        // unsealed cursor (the root, or inside an open group), an
        // end needing the sentinel coordinate (or beyond) is beyond
        // this machine's space — capability, summarized with the
        // feed gate's exhaustion.
    ...
    fn ride_len<O: FnMut(&[u8])>(
        ...
        if routed {
            ...
            out(self.stage.bytes());
            out(self.pump.carry.bytes());
            self.stack.push(Frame::Len { prev_zone: self.pump.zone });
            self.pump.zone = end;
            self.matcher.commit_descent();
            self.mode = Mode::Head;
        } else {
