//! The groupless maintain editor: handle-based edits with precise
...
//! is stored on the row as an input fact, every scanned scalar's
//! decoded word is banked at the scan, and every untouched span
//! reproduces its padding byte-exactly at save. The root layer is
//! flat and eager; LEN payloads stay opaque until
//! [`Maintain::descend`], whose verdict is resident.
//!
...
//!
//! Coordinates: write · sequential-repeatable · offline · groupless · tolerant (type-level) · revisable.
//!
//! # Examples
//!
//! ```
...
//!
//! // The padded varint rides verbatim; the same-length payload
//! // keeps its prefix.
...
/// bound, so no policy section exists either — descent is
/// caller-stepped and every scan is iterative. Wire-declared
/// quantities are quoted as their wire types; a bad record never
/// reaches the row table, so its field number travels with the
/// fault — inside the [`Stage`] coordinate for varint reads (the
/// tag stage carries none: no field exists yet), on the variant
...
const fn banked_word(kind: RecordKind, word: u64) -> Word {
    match kind {
        ...
        // The callers' kind dispatch keeps LEN rows out.
    ...
    ($self:ident, $script:ident, $lens:ident, $book:ident) => {{
        ...
        loop {
            let Some(id) = cur else {
                let Some(container) = open else { break };
                let row = $self.row(container);
                // An opened LEN's close: its minted prefix slot
...
macro_rules! maintain_machine {
    (
        ...
        impl<$($plt,)? S: StableReplaySource> $Machine<$($plt,)? S> {
            ...
            /// bytes pass unchanged. One walk (the sizing pass
            /// spends none — the rows carry the banked words and
            /// met widths).
            ///
            ...
            /// untouched.
            ///
            /// The face caches nothing: body totals and the sizing
            /// spine are call-local, so
            ...
            /// the sizing scratch, the script, or the output
            /// reservation, [`SaveFault::BodyOverCap`] when an
            /// opened LEN's canonical body outgrows the length
            /// class, [`SaveFault::Source`] and [`SaveFault::Torn`]
            /// off the emission walk. On `Err` no buffer exists and
            ...
            fn size_pass<E>(&self) -> Result<(u64, Vec<u64>), SaveFault<E>> {
                ...
                loop {
                    let Some(id) = cur else {
                        ...
                        // The fidelity criterion: an unchanged body
                        // length keeps the source prefix, padding
                        // included.
                        let prefix = if body == frame.src_len {
                            u64::from(frame.prefix_w.w())
                        } else {
