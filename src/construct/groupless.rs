//! The groupless dialect constructor: four wire codes.
//!
//! The group verbs are absent at the vocabulary level — the
//! construct side has no input, so no refusal word is needed:
//! absence is completeness. Every nested value is a LEN frame, so
//! every message node pays the prefix arithmetic the grouped
//! dialect's groups avoid.
//!
//! Coordinates: author (outside the input axes) · groupless.
//!
//! # Examples
//!
//! ```
...
//! builder.push_packed_uint32(f4, &[1, 150, 2]);
//! builder.push_sint32(f1, -1); // zigzag: one wire byte
//! assert_eq!(
//!     builder.finish().unwrap(),
//!     [0x22, 0x04, 0x01, 0x96, 0x01, 0x02, 0x08, 0x01]
//! );
//! ```
...
/// close by closure scope ([`message`](Self::message)), so framing
/// cannot be mismatched. `'p` backs the borrowed payloads
/// (`push_len`, `push_string`, `raw_bytes`): each is held until
/// the finish copies it into the output.
///
/// # Examples
///
/// ```
...
/// Scalar and packed pushes, framing, and the finish family are
/// the mixed machine's, face for face.
///
/// # Examples
///
/// ```
/// use protobuf_edit::FieldNumber;
/// use protobuf_edit::construct::groupless::CopyBuilder;
///
/// let f1 = FieldNumber::new(1).unwrap();
...
///     builder.push_len(f2, &payload);
/// }
/// builder.push_varint(f1, 150);
/// assert_eq!(
///     builder.finish().unwrap(),
///     [0x12, 0x04, 0xAB, 0xAB, 0xAB, 0xAB, 0x08, 0x96, 0x01]
/// );
/// ```
#[must_use]
...
impl<'p> Builder<'p> {
    typed_faces!(mixed: groupless, 'p, BodyBuilder);

    /// Pushes one designated external record as a canonical root
    /// record: the designation's exact bytes ride the borrow table
    /// and copy once, at the finish. This builder's output is
    /// canonical, so the argument is the proof-carrying form — a
    /// tolerant designation upgrades through `try_canonical`, which
    /// refuses padded framing. A padded record can still embed
    /// byte-exactly as an opaque LEN payload through
    ...
    ///
    /// let source = [0x12, 0x02, 0x68, 0x69];
    ...
    /// ```
    #[inline]
    pub fn push_record(&mut self, source: crate::source::groupless::CanonicalRecordRef<'p>) {
    ...
    #[inline]
    pub fn push_record_copy(&mut self, source: crate::source::groupless::CanonicalRecordRef<'_>) {
...
impl CopyBuilder {
    typed_faces!(copy: groupless, CopyBodyBuilder);

    /// Pushes one designated external record as a canonical root
    /// record: the designation's exact bytes land in the owned
    /// staging store at the push, so no designation lifetime binds
    /// the builder. This builder's output is canonical, so the
    /// argument is the proof-carrying form — a tolerant designation
    /// upgrades through `try_canonical`, which refuses padded
    /// framing.
    #[inline]
    pub fn push_record(&mut self, source: crate::source::groupless::CanonicalRecordRef<'_>) {
        self.core.put_raw_bytes_copy(source.as_bytes());
    }
}
