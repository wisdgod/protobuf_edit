//! The grouped stream-ingest intake: chunked input judged
...
//! interior extent, so group state survives chunk edges as
//! row-arena coordinates, never a second stack. Nesting is judged
//! against the declared [`DepthLimit`] at every open tag
//! ([`Refusal::DepthExceeded`]); an orphan or mismatched end tag is
//! a grammar fault at its deciding byte.
//!
...
//! word's first byte). Group frame tags are tag words and take the
//! same judgment, open and end alike. Every admitted framing word
//! is therefore minimal: the finished rows store no width column,
//! spans derive from the record's own facts, and saved documents
//! re-ingest under the same admission. Every feed is admitted
//! against the finished editor's coordinate class before a byte is
//! read ([`IngestFaultKind::CoordinateLimit`]), and a completed LEN
//! prefix is judged the moment it completes — an endpoint past the
//! class faults even though its bytes have not arrived.
//!
...
//! cell is feature `intake-grouped`).
//!
//! Coordinates: write · stream · offline · grouped · canonical (type-level) · owned · commit-only.
//!
//! # Examples
//!
//! ```
//! use protobuf_edit::DepthLimit;
//! use protobuf_edit::stream_intake::grouped::Ingest;
//!
//! // varint f1=150 · group f2 { varint f3=9 }, fed byte by byte:
//! // chunk edges never show in the product, and the group's
//! // interior is materialized eagerly.
...
//! time — a group end tag is a tag word and takes the same
//! judgment:
//!
//! ```
//! use protobuf_edit::DepthLimit;
//! use protobuf_edit::stream_intake::grouped::{Ingest, IngestFaultKind, Refusal};
//!
//! // group f1 closed by an end tag padded to two bytes.
//! let mut ingest = Ingest::new(DepthLimit::REFERENCE);
//! ingest.feed(&[0x0B, 0x8C]).unwrap();
//! let failure = ingest.feed(&[0x00]).unwrap_err();
//! assert_eq!(failure.fault().at(), 1);
//! assert!(matches!(
//!     failure.fault().kind(),
//!     IngestFaultKind::Refused(Refusal::NonMinimalTag { at: 1, width: 2 })
//! ));
//! assert_eq!(failure.source(), [0x0B, 0x8C, 0x00]);
//! ```
...
crate::editor::grouped::one_shot_machine! {
    ...
    /// `transfer-stream-intake-grouped`) seals through
    ...
    a_noun: "an intake",
    A_noun: "An intake",
    doc_mod: "protobuf_edit::stream_intake::grouped",
    doc_open: "{ let mut ingest = protobuf_edit::stream_intake::grouped::Ingest::new(DepthLimit::REFERENCE); ingest.feed(&msg).unwrap(); ingest.finish() }",
    doc_open_empty: "protobuf_edit::stream_intake::grouped::Ingest::new(DepthLimit::REFERENCE).finish()",
...
crate::editor::grouped::one_shot_machine! {
    ...
    A_noun: "An intake",
    doc_mod: "protobuf_edit::stream_intake::grouped",
    doc_open: "{ let mut ingest = protobuf_edit::stream_intake::grouped::Ingest::new(DepthLimit::REFERENCE); ingest.feed(&msg).unwrap(); ingest.finish_borrow() }",
    doc_open_empty: "protobuf_edit::stream_intake::grouped::Ingest::new(DepthLimit::REFERENCE).finish_borrow()",
...
crate::editor::grouped::one_shot_machine! {
    ...
    A_noun: "An intake",
    doc_mod: "protobuf_edit::stream_intake::grouped",
    doc_open: "{ let mut ingest = protobuf_edit::stream_intake::grouped::Ingest::new(DepthLimit::REFERENCE); ingest.feed(&msg).unwrap(); ingest.finish_copy() }",
    doc_open_empty: "protobuf_edit::stream_intake::grouped::Ingest::new(DepthLimit::REFERENCE).finish_copy()",
...
pub enum IngestFaultKind {
    ...
    /// word's last byte arrives) or a group nesting past the depth
    /// limit.
    Refused(Refusal),
...
/// A mid-stream policy refusal: both conventions name the same
/// byte (the refused construct's first).
#[cold]
...
impl IngestCore {
    ...
    /// Steps the head tag; a completed tag is judged minimal —
    /// open and end frame tags included — then classifies and
    /// opens its value stage.
    #[allow(
        clippy::as_conversions,
    ...
    fn head(&mut self, rest: &mut &[u8]) -> Result<(), IngestFault> {
        ...
        let kind = match classify(low3) {
            ...
            TagClass::GroupEnd => return self.close_group(start, field),
            TagClass::Unassigned => {
        ...
        match kind {
            ...
            RecordKind::Group => {
                if self.open_depth == u32::from(self.limit.as_inner()) {
                    ...
                    return Err(refused_now(start.as_inner(), refusal));
                }
                // The provisional publish: following records name
                // this row as parent; its interior extent is
                // patched at the verified close.
    ...
    /// A group end tag completed (already proven minimal by the
    /// head gate): verify the frame, patch the open row's interior
    /// extent, and pop the open-group chain by one parent link.
    fn close_group(&mut self, start: u32, field: FieldNumber) -> Result<(), IngestFault> {
    ...
    fn len_word(&mut self, rest: &mut &[u8], head: PendingHead) -> Result<(), IngestFault> {
        ...
        let row =
            Row::scanned(head.field, RecordKind::Len, head.start, Extent::from_len(len), self.open);
...
impl Ingest {
    /// Starts an ingest job. The depth bound governs the eagerly
    /// parsed group nesting at every open tag and is retained for
    /// the finished intake's future LEN descents — LEN interiors
    /// stay opaque before the seal.
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// faults, canonical-minimality refusals, and depth refusals
    /// ([`IngestFaultKind::Wire`], [`IngestFaultKind::Refused`])
    /// are judged as their deciding byte arrives — a padded word
    /// the moment its last byte lands, at the construct's first
    /// byte — with the chunk absorbed whole. Every failure returns
    /// the accumulated source and spends the shell.
    ///
    /// # Panics
    ///
    ...
    /// mid-word ([`ReadFault::Truncated`] at its stage), a fixed
    /// or counted payload still owed bytes
    /// ([`FaultKind::PayloadCut`]), or a frame still open
    /// ([`FaultKind::GroupUnclosed`] at its open tag). The
    /// accumulated source rides back inside the [`Failure`],
    /// absorbed.
    ///
    /// # Panics
    ///
