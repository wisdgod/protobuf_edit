//! The grouped stream-ingest intake's transfer sibling.
//!
...
//! as its own sealed machine so the base intake pays none of the
//! capability's carried state or dispatch. The ingest phase is the
//! base cell's [`Ingest`] whole — feed grammar, canonical
//! judgments, custody, and truncation judgments untouched — and
...
//! the command — canonical admission makes both take
//! `CanonicalRecordRef`, so every import zone is proven minimal.
//! The local transfer faces — `copy_record`, `move_record`,
//! `copy_payload`, `move_payload` — stage zero bytes: the rows
//! carry coordinates into the machine's own ingested source.

use alloc::vec::Vec;

use crate::admission::{Coord, Extent, admitted_u32, usize_of};
use crate::stream_intake::{Handle, PayloadAt, PayloadStore, RowId, WordAt, WordStore, parts_len_usize};
...
pub use crate::stream_intake::transfer::PayloadTarget;
...
crate::editor::grouped::one_shot_machine! {
    ...
    /// machine. Handles stay valid for the intake's life; rows and
    ...
    a_noun: "an intake",
    A_noun: "An intake",
    doc_mod: "protobuf_edit::stream_intake::grouped::transfer",
    doc_open: "{ let mut ingest = protobuf_edit::stream_intake::grouped::Ingest::new(DepthLimit::REFERENCE); ingest.feed(&msg).unwrap(); ingest.finish_transfer() }",
    doc_open_empty: "protobuf_edit::stream_intake::grouped::Ingest::new(DepthLimit::REFERENCE).finish_transfer()",
    doc_recipes: " Price-reserve-save composes growth-free: reserve exactly [`TransferIntake::save_len`]'s answer and [`TransferIntake::save_into`] never grows the buffer.",
}
...
impl Ingest {
    /// Declares EOF and seals into the transfer editing intake —
    ...
    /// use protobuf_edit::stream_intake::grouped::transfer::InsertAt;
    ///
    ...
    /// );
    /// ```
    pub fn finish_transfer<'p>(self) -> Result<TransferIntake<'p>, super::Failure> {
