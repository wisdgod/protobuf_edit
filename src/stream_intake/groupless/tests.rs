...
/// emission-time geometry (`None`: every framing word and value is
/// minimal and the item seals). Words are judged in the fused
/// scan's own order — per record, tag first, then the value side —
/// so the first padded word in document order decides.
...
fn derived_refusal(expected: &[Expected]) -> Option<DerivedRefusal> {
    for exp in expected {
        let word = (exp.field << 3) | exp.code;
        if exp.tag_width > encoded_len32(word) {
            return Some(DerivedRefusal {
                refusal: Refusal::NonMinimalTag {
                    at: exp.start,
                    width: u8::try_from(exp.tag_width).unwrap(),
                },
                at: exp.start,
                word_end: exp.start + exp.tag_width,
            });
        }
        let field = FieldNumber::new(exp.field).unwrap();
        let value_at = exp.start + exp.tag_width;
        match exp.code {
            0 if exp.payload_len > encoded_len64(exp.value.unwrap()) => {
                return Some(DerivedRefusal {
                    refusal: Refusal::NonMinimalValue {
                        at: value_at,
                        field,
                        width: u8::try_from(exp.payload_len).unwrap(),
                    },
                    at: value_at,
                    word_end: value_at + exp.payload_len,
                });
            }
            2 if exp.delim_width > encoded_len32(exp.payload_len) => {
                return Some(DerivedRefusal {
                    refusal: Refusal::NonMinimalLen {
                        at: value_at,
                        field,
                        width: u8::try_from(exp.delim_width).unwrap(),
                    },
                    at: value_at,
                    word_end: value_at + exp.delim_width,
                });
            }
            _ => {}
        }
...
fn assert_sealed(intake: &Intake<'_>, bytes: &[u8], expected: &[Expected]) {
    ...
    for (&handle, exp) in tops.iter().zip(expected) {
        assert_eq!(intake.field(handle).as_inner(), exp.field);
        assert_eq!(code_of(intake.kind(handle)), exp.code);
        ...
        match intake.kind(handle) {
            RecordKind::Varint => {
                assert_eq!(intake.varint_word(handle), exp.value);
            }
            RecordKind::Len => {
                let body = &bytes
...
fn fault_docs() -> Vec<FaultDoc> {
    ...
    wrap.push(0x02);
    // A ten-byte spelling of value zero: in class at full width,
    // refused as padding — the canonical judgment carried through
    // nine continuation bytes.
    let mut padded_wide = vec![0x08];
    padded_wide.extend_from_slice(&[0x80; 9]);
    padded_wide.push(0x00);
    vec![
        ...
        // A padded tag: the canonical refusal at the tag's first
        // byte, judged the moment the padding byte arrives.
        FaultDoc {
            bytes: vec![0x88, 0x00, 0x01],
            deciding: 1,
            at: 0,
            kind: IngestFaultKind::Refused(Refusal::NonMinimalTag { at: 0, width: 2 }),
        },
        // A padded tag that also spells field zero: the canonical
        // judgment precedes the field judgment, as in the buffered
        // root scan.
        FaultDoc {
            bytes: vec![0x80, 0x00],
            deciding: 1,
        ...
        // A padded value: the canonical refusal at the value's
        // first byte.
        FaultDoc {
            bytes: vec![0x08, 0x81, 0x00],
            deciding: 2,
            at: 1,
        ...
        // The padded value carried through nine continuation bytes:
        // in class at full width, refused as padding on the tenth.
        FaultDoc {
            bytes: padded_wide,
            deciding: 10,
            at: 1,
        ...
        FaultDoc {
            ...
            kind: IngestFaultKind::Wire(Fault {
                at: 1,
                kind: FaultKind::Len { field: field2, fault: ReadFault::OutOfClass },
            }),
        },
        // A padded length prefix: the canonical refusal at the
        // prefix's first byte, ahead of any endpoint judgment.
        FaultDoc {
            bytes: vec![0x12, 0x82, 0x00],
            deciding: 2,
            at: 1,
...
/// construction snapshot on the sealed side, refusal kind and
/// coordinates on the refused side.
#[cfg(feature = "intake-groupless")]
mod twin {
    use super::*;
    use crate::intake::groupless as buffered;
    ...
    fn as_local(refusal: buffered::Refusal) -> Refusal {
        match refusal {
            ...
            buffered::Refusal::GroupCode { at, field, low3 } => {
                Refusal::GroupCode { at, field, low3 }
            }
    ...
    fn the_seal_builds_exactly_what_the_buffered_open_builds() {
        for (bytes, expected) in stream_corpus::groupless_items() {
            ...
            for plan in stream_corpus::chunkings(&bytes) {
                ...
                // The private snapshot: arena order, links, derived
                // widths, value words, edit state, and the top
                // anchor — the handle-numeric comparison public
                // iteration cannot give.
    ...
    /// construct coordinates — and the stream cell answers it at
    /// every split inside the offending word.
    #[test]
    fn every_padded_item_refuses_as_the_buffered_intake_at_the_padded_word() {
        ...
        // the corpus stopped carrying them.
        let mut met = [false; 3];
        for (bytes, expected) in stream_corpus::groupless_items() {
            ...
            met[match derived.refusal {
                ...
                _ => unreachable!("the derivation names padding only"),
            }] = true;
...
/// The `NonMinimal*` refusals are parity rows: both machines judge
/// the padded word the moment its last byte arrives, at the
/// construct's first byte, as policy.
...
mod scan_parity {
    ...
    fn scan_outcome(plan: &[Vec<u8>]) -> Result<(), (u64, FaultClass)> {
        let mut parser = Parser::new(Standard::CanonicalMinimal, DepthLimit::REFERENCE);
        for chunk in plan {
    ...
    const fn class_of(kind: IngestFaultKind) -> FaultClass {
        match kind {
            ...
            // The refusal alphabet splits: padding and depth are
            // policy (another configuration accepts them), the
            // group code capability (the twin dialect's language).
            IngestFaultKind::Refused(refusal) => match refusal {
                Refusal::NonMinimalTag { .. }
                | Refusal::NonMinimalLen { .. }
                | Refusal::NonMinimalValue { .. }
                | Refusal::DepthExceeded { .. } => FaultClass::Policy,
                Refusal::GroupCode { .. } => FaultClass::Capability,
            },
...
fn the_sealed_machine_descends_and_edits_as_the_buffered_shape() {
    ...
    let container = intake.top().by_field(f2).next().unwrap();
    let Descent::Opened { first: Some(inner) } = intake.descend(container).unwrap() else {
        panic!("a well-formed payload opens");
    };
    intake.set_varint(inner, 7).unwrap();
    assert_eq!(intake.save().unwrap(), [0x08, 0x01, 0x12, 0x02, 0x08, 0x07]);
}
