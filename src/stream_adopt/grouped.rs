//! The grouped stream-ingest adopt: chunked input parsed as it
//! arrives, sealed at `finish` into a one-shot editing adopt.
//!
//! The finished machine carries owned tenure, width-carrying rows,
//! commit-only edits, and a byte-fidelity save — the buffered
//! adopt's faces over a source that arrived in chunks.
//!
//! This dialect speaks the full six-code wire language: group
//! frames are parsed eagerly as their tags arrive — a group row
//! publishes provisionally at its open tag (following records name
//! it as parent) and is patched at the verified close with its
//! interior extent and actual end-tag width, so group state
//! survives chunk edges as row-arena coordinates, never a second
//! stack. Nesting is judged against the declared [`DepthLimit`] at
//! every open tag ([`Refusal::DepthExceeded`]); an orphan or
//! mismatched end tag is a grammar fault at its deciding byte.
//!
...
//! wire-judged) until an explicit [`Adopt::descend`] after the
...
//! partition rule ([`crate::stream_adopt`]): a one-shot machine
//! holds nothing a re-run cannot rebuild.
//!
//! The finished machine is the buffered adopt's shape over the
//! ingested source: the same commands, saves, canonical faces, and
//! transactional release. Its only door is the seal — the cell's
//! input presence is the stream, so no buffered `open` exists here
//! (the buffered cell is feature `adopt-groupless`).
//!
//! Coordinates: write · stream · offline · grouped · tolerant (type-level) · owned · commit-only.
//!
//! # Examples
//!
//! ```
...
//!
//! // varint f1=150 (value padded) · group f2 { varint f3=9 },
//! // fed byte by byte: chunk edges never show in the product, and
//! // the group's interior is materialized eagerly.
...
//!
//! // The padded varint and both frame tags rode verbatim.
//! let mut out = Vec::new();
//! adopt.save_into(&mut out).unwrap();
//! assert_eq!(out, [0x08, 0x96, 0x81, 0x00, 0x13, 0x18, 0x07, 0x14]);
//! ```
...
crate::editor::grouped::one_shot_machine! {
    /// A one-shot editing adopt sealed from a chunked stream.
    ///
    /// The buffered adopt's editing machine over the ingested
    ...
    /// mid-edit adopt moves, returns, and caches (rows address the
    /// source by `u32` offsets, never pointers). Handles stay valid
    /// for the adopt's life; rows and stored values are never
    ...
    /// held until the save copies it into the output, and an adopt
    /// with no borrowed payloads inhabits `Adopt<'static>`. The
    /// only door is [`Ingest::finish`] — the input arrived as a
    /// stream, so no buffered open exists on this type. The thin
    /// payload-backing siblings [`BorrowAdopt`] and [`CopyAdopt`]
    /// seal through [`Ingest::finish_borrow`]/[`Ingest::finish_copy`];
    /// the transfer sibling `TransferAdopt` (feature
    /// `transfer-stream-adopt-grouped`) seals through
    ...
    a_noun: "an adopt",
    A_noun: "An adopt",
    doc_mod: "protobuf_edit::stream_adopt::grouped",
    doc_open: "{ let mut ingest = protobuf_edit::stream_adopt::grouped::Ingest::new(DepthLimit::REFERENCE); ingest.feed(&msg).unwrap(); ingest.finish() }",
    doc_open_empty: "protobuf_edit::stream_adopt::grouped::Ingest::new(DepthLimit::REFERENCE).finish()",
    doc_recipes: " Price-reserve-save composes growth-free: reserve exactly [`Adopt::save_len`]'s answer and [`Adopt::save_into`] never grows the buffer.",
}

crate::editor::grouped::one_shot_machine! {
    /// The borrowed-only adopt sealed from a chunked stream: every
    ...
    /// payload owner must outlive the adopt.
    ...
    A_noun: "An adopt",
    doc_mod: "protobuf_edit::stream_adopt::grouped",
    doc_open: "{ let mut ingest = protobuf_edit::stream_adopt::grouped::Ingest::new(DepthLimit::REFERENCE); ingest.feed(&msg).unwrap(); ingest.finish_borrow() }",
    doc_open_empty: "protobuf_edit::stream_adopt::grouped::Ingest::new(DepthLimit::REFERENCE).finish_borrow()",
    doc_recipes: " Price-reserve-save composes growth-free: reserve exactly [`BorrowAdopt::save_len`]'s answer and [`BorrowAdopt::save_into`] never grows the buffer.",
}

crate::editor::grouped::one_shot_machine! {
    /// The copy-only adopt sealed from a chunked stream: every
    ...
    A_noun: "An adopt",
    doc_mod: "protobuf_edit::stream_adopt::grouped",
    doc_open: "{ let mut ingest = protobuf_edit::stream_adopt::grouped::Ingest::new(DepthLimit::REFERENCE); ingest.feed(&msg).unwrap(); ingest.finish_copy() }",
    doc_open_empty: "protobuf_edit::stream_adopt::grouped::Ingest::new(DepthLimit::REFERENCE).finish_copy()",
    doc_recipes: " Price-reserve-save composes growth-free: reserve exactly [`CopyAdopt::save_len`]'s answer and [`CopyAdopt::save_into`] never grows the buffer.",
}
...
/// construct, with the same buffered coordinates and depth policy,
/// that opening the concatenated bytes would name. The
/// coordinate-class refusal is this cell's own: the stream is
/// admitted against the finished editor's source cap before bytes
/// are read.
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
pub enum IngestFaultKind {
    ...
    Wire(Fault),
    /// Lawful wire outside the declared bounds: a group nesting
...
struct IngestCore {
    ...
    first: Option<RowId>,
    /// The innermost chain's tail — the sibling-link anchor.
    last: Option<RowId>,
    /// The innermost group still open (`None`: records land at the
    /// root). The open-group chain is the arena's own parent links.
    open: Option<RowId>,
    /// Containers open around the cursor, judged against `limit`
    /// at every group open.
    open_depth: u32,
    carry: VarintCarry,
    phase: Phase,
...
impl IngestCore {
    ...
    /// Publishes one completed row at the innermost chain's tail
    /// and answers its coordinate.
    fn publish(&mut self, row: Row) -> RowId {
        ...
        match self.last {
            ...
            None => match self.open {
                Some(group) => {
                    // SAFETY: `group` was minted by this ingest's
                    // publish.
                    unsafe { self.rows.get_unchecked_mut(group.index()) }.kid = Some(id);
                }
    ...
    fn head(&mut self, rest: &mut &[u8]) -> Result<(), IngestFault> {
        ...
        let kind = match classify(low3) {
            ...
            TagClass::GroupEnd => return self.close_group(start, width, field),
            TagClass::Unassigned => {
        ...
        match kind {
            ...
            RecordKind::Group => {
                if self.open_depth == u32::from(self.limit.as_inner()) {
                    let refusal = Refusal::DepthExceeded { at: start.as_inner(), field };
                    return Err(IngestFault {
                        at: u64::from(start.as_inner()),
                        kind: IngestFaultKind::Refused(refusal),
                    });
                }
                // The provisional publish: following records name
                // this row as parent; its interior extent and
                // end-tag width are patched at the verified close.
                let id = self.publish(Row::scanned(
                    field,
                    RecordKind::Group,
                    start,
                    Extent::from_width(0),
    ...
    /// A group end tag completed: verify the frame, patch the open
    /// row's interior extent and actual end-tag width, and pop the
    /// open-group chain by one parent link.
    fn close_group(
        &mut self,
        start: u32,
        width: WordWidth,
        ...
        let Some(group) = self.open else {
            return Err(wire_now(start, FaultKind::GroupEndOrphan { found: field }));
        };
        // SAFETY: `group` was minted by this ingest's publish.
        let row = unsafe { self.rows.get_unchecked_mut(group.index()) };
        if row.field != field {
            return Err(wire_now(
                start,
        ...
        // SAFETY: the ingest walked forward from the open tag, so the
        // end tag sits at or past the head's payload start within the
        // capped source, and the cap is the class bound.
        row.payload_len =
            unsafe { Extent::new_unchecked(start - (row.start.as_inner() + row.tag_w())) };
        row.delim_width = Some(width);
        self.open = row.parent;
        self.open_depth -= 1;
    ...
    fn varint_value(&mut self, rest: &mut &[u8], head: PendingHead) -> Result<(), IngestFault> {
        ...
        let _ = self.publish(Row::scanned(
            head.field,
            RecordKind::Varint,
            head.start,
        ...
        match self.phase {
            Phase::Head => {
                if self.carry.width == 0 {
                    // Between constructs: only an open frame can
                    // refuse the seal.
                    let group = self.open?;
                    let row = &self.rows[group.index()];
                    return Some(wire_eof(
                        eof,
                        row.start.as_inner(),
                        FaultKind::GroupUnclosed { open: row.field },
                    ));
                }
                let at = admitted_u32(self.source.len() - usize::from(self.carry.width));
...
/// arrive, and seals into the finished editing adopt.
///
/// One fused pass: `feed` reserves room for the whole chunk, then
/// examines each byte once — one append into the reserved final
/// backing and one fold into the varint carry. The phase owns no
/// query, command, or save face; only a successful [`Ingest::finish`]
/// (or a payload-backing sibling door) publishes a machine those
/// faces exist on.
///
...
impl Ingest {
    ...
    /// finished adopt's future LEN descents — ingest itself never
    ...
    /// ingest.feed(&[0x08, 0x2A]).unwrap();
    /// let adopt = ingest.finish().unwrap();
    /// assert_eq!(adopt.source(), [0x08, 0x2A]);
    /// ```
    ...
    /// the fused copy/parse loop. A chunk edge is never a fault —
    /// a construct cut here resumes on the next feed, and only
    /// [`Ingest::finish`] declares EOF.
    ///
    /// # Errors
    ///
    ...
    /// with the chunk [`ChunkDisposition::Unabsorbed`]. Wire faults and
    /// depth refusals ([`IngestFaultKind::Wire`],
    /// [`IngestFaultKind::Refused`]) are judged as their deciding
    /// byte arrives, with the chunk absorbed whole. Every failure
    /// returns the accumulated source and spends the shell.
    ///
    /// # Panics
    ///
    ...
    /// finalize, and no byte of the source is traversed again.
    ///
    /// # Errors
    ///
    ...
    /// ```
    pub fn finish<'p>(self) -> Result<Adopt<'p>, Failure> {
    ...
    /// sealed parts under [`BorrowAdopt`]'s payload supply.
    ///
    /// # Errors
    ///
    /// As [`Ingest::finish`].
    ...
    ///
    /// After a returned [`Failure`] — the shell is spent.
    pub fn finish_borrow<'p>(self) -> Result<BorrowAdopt<'p>, Failure> {
        let sealed = self.seal()?;
        Ok(BorrowAdopt {
            source: sealed.source,
            rows: sealed.rows,
    ...
    /// sealed parts under [`CopyAdopt`]'s payload supply.
    ///
    /// # Errors
    ///
    /// As [`Ingest::finish`].
    ...
    ///
    /// After a returned [`Failure`] — the shell is spent.
    pub fn finish_copy(self) -> Result<CopyAdopt, Failure> {
        let sealed = self.seal()?;
        Ok(CopyAdopt {
            source: sealed.source,
            rows: sealed.rows,
