...
use super::{Descent, InsertAt, TransferAdopt};
...
fn the_seal_door_re_tags_rows_and_moves_the_source() {
    ...
    let mut adopt = ingest_transfer(&[0x08, 0x05, 0x12, 0x02, 0x68, 0x69]);
    ...
    assert_eq!(adopt.save().unwrap(), [0x12, 0x02, 0x68, 0x69, 0x12, 0x02, 0x68, 0x69, 0x08, 0x05]);
    assert_eq!(adopt.into_source(), [0x08, 0x05, 0x12, 0x02, 0x68, 0x69]);
}

#[test]
fn an_imported_record_is_first_class_over_the_sealed_source() {
    // An import into the sealed editor descends and edits like the
    // buffered sibling's.
    ...
    let mut adopt = ingest_transfer(&[0x08, 0x2A]);
