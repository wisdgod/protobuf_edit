//! The grouped fixed-scratch one-shot patch: borrowed input,
...
//! opaque until [`Patch::descend`], whose verdict is resident — a
//! wire fault or a refusal inside a payload parks on the record
//! and the machine lives on. Descent is an explicit commitment:
...
//! prefix rides verbatim while its body length is unchanged; group
//! framing rides verbatim around edited interiors), the canonical
//! contract (`save_canonical_into`/`save_canonical_sink` minimally
//! emit every varint construct in the materialized commitment
//! closure — group interiors are in-band syntax and always sit in
//! it), and every verdict and fault value match the heap twin's —
//! only the output faces change deployment: `save_into` fills a
//! caller slice, refusing [`SaveFault::OutputShort`] with zero
//! bytes written, and the sink faces hand borrowed slices out.
//!
//! Coordinates: write · buffered · offline · grouped · tolerant (type-level) · borrowed · commit-only · fixed scratch.
//!
//! # Examples
//!
//! ```
...
//! use protobuf_edit::fixed_patch::grouped::{Patch, Plan};
//!
...
/// Lawful wire this machine refuses: within the language, past its
/// declared bounds.
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
pub enum Refusal {
...
pub enum OpenFault {
    ...
    Wire(Fault),
    /// The root layer is lawful wire past this machine's declared
    /// bounds.
    Refused(Refusal),
}
...
pub enum Descent<'p> {
    ...
    Faulted(&'p Fault),
    /// The payload is lawful wire past this machine's declared
    /// bounds (resident).
    Refused(&'p Refusal),
}
...
/// under `parent`, group frames materializing in-band: an open tag
/// opens a chain, its end tag settles the group's interior length
/// and end-tag width onto the row. Depth is judged here — groups
/// nest within one scan, so the scan carries the bound the LEN
/// descend passes in. Widths ride onto the rows as scanned; nothing
/// is re-derived from values. On any halt the caller discards the
...
fn scan_layer(
    ...
    while pos < end {
        ...
        let kind = match classify(Low3::from_word(word)) {
            ...
            TagClass::GroupEnd => {
                ...
                let row = rows.get_mut(group.as_inner());
                if row.field != field {
                ...
                // SAFETY: the interior width is a difference of
                // in-class scan coordinates, ordered by the walk.
                row.payload_len =
        ...
        match last {
            ...
            None => match open {
                Some(group) => {
                    rows.get_mut(group.as_inner()).kid = Some(id);
                }
                None => first = Some(id),
            },
        }
    ...
    if let Some(group) = open {
        let row = rows.get(group.as_inner());
...
enum Arm {
    ...
    /// A source group: tag and end tag verbatim, interior recurses.
    ReGroup { tag_at: u32, tag_end: u32, first: Option<RowId> },
    /// An authored group: minimal open and end words.
    NewGroup { head: u32, first: Option<RowId> },
}
...
enum CanonicalArm {
    ...
    /// A group: minimal open tag, the live in-band interior, then
    /// the matching minimal end tag — group interiors are syntax
    /// and always sit in the closure.
    Group { head: u32, first: Option<RowId> },
}
    ...
    match row.base() {
        ...
        Base::Intact => match row.kind {
            ...
            RecordKind::Group => Arm::ReGroup {
                tag_at: row.start.as_inner(),
                tag_end: row.start.as_inner() + row.tag_w(),
                first: row.kid,
            },
        ...
        Base::Replaced => {
            ...
            match row.kind {
                ...
                RecordKind::Group => {
                    ...
                    // SAFETY: no command writes a Replaced base onto
                    // a group row — groups carry no value side.
        ...
        Base::Inserted => {
            ...
            match row.kind {
                ...
                RecordKind::Group => Arm::NewGroup { head, first: row.kid },
            }
        }
    }
}
    ...
    loop {
        let Some(id) = cur else {
            ...
            match row.kind {
                ...
                RecordKind::Group => match row.base() {
                    Base::Intact => {
                        acc += u64::from(row.tag_w()) + u64::from(row.delim_w());
                    }
                ...
                RecordKind::Varint | RecordKind::I32 | RecordKind::I64 => {
                    debug_assert!(false, "scalars never open a frame");
                    // SAFETY: the walk enters frames for containers
                    // alone.
                    unsafe { core::hint::unreachable_unchecked() }
                }
            }
        ...
        match settle(rows, words, row) {
            ...
            Arm::ReGroup { first, .. } | Arm::NewGroup { first, .. } => {
                open = Some(id);
                cur = first;
                continue;
            }
        ...
        let Some(id) = cur else {
            ...
            if matches!(row.kind, RecordKind::Group) {
                match row.base() {
                    Base::Intact => {
                        let end_at =
                            row.start.as_inner() + row.tag_w() + row.payload_len.as_inner();
...
fn settle_canonical(source: &[u8], words: &WordStore<'_>, row: &Row) -> CanonicalArm {
    ...
    match row.base() {
        Base::Intact => match row.kind {
            ...
            RecordKind::Group => CanonicalArm::Group { head, first: row.kid },
        },
...
impl<'a, 'p, 's> Patch<'a, 'p, 's> {
    ...
    pub fn open(
        ...
        let top =
            scan_layer(&mut rows, source, 0, len, None, 0, limit).map_err(|halt| match halt {
    ...
    /// A record's direct interior in wire order: group members, or
    /// a descended LEN's records. Empty for scalars and for
    ...
    /// group closure. The fact is source-derived: the walk descends
    /// source-backed group kids only — authored subtrees are
    /// skipped whole, so edits under the closure never move an
    /// original designation's depth — and descended LEN interiors
    /// sit under an opacity boundary, contributing nothing.
    ...
    /// source record bytes — a group's whole closure with its end
    /// tag — bound to their proved field, kind, framing geometry,
    /// and structural group depth. The designation names the
    ...
    /// # Errors
    ///
    /// As [`Patch::insert_varint`], less the store edge (a group
    /// stores no value).
    ///
    /// # Panics
    ///
    ...
    pub fn insert_group(&mut self, at: InsertAt, field: FieldNumber) -> Result<Handle, EditFault> {
        let point = self.resolve_anchor(at)?;
        let id = self.mint_insert()?;
        self.apply_insert(&point, id, field, RecordKind::Group, 0);
        Ok(Handle(id))
    }
