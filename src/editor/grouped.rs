//! The grouped one-shot editor family core, emitted per machine by
//! [`one_shot_machine!`] for the six one-shot editors: the
...
/// Emits the grouped one-shot editor family into the invoking
...
macro_rules! one_shot_machine {
    (
        ...
        $crate::editor::grouped::one_shot_machine!(@vocabulary $cap buffered, acceptance: $acc, noun: $noun, a_noun: $a_noun, A_noun: $A_noun);
    };
    (
        ...
        $crate::editor::grouped::one_shot_machine!(@vocabulary $cap stream, acceptance: $acc, noun: $noun, a_noun: $a_noun, A_noun: $A_noun);
    };
    (
        ...
        /// word, length word, varint value, or payload start; for
        /// [`FaultKind::GroupUnclosed`] it is the unmatched open tag.
        ...
        pub enum FaultKind {
            ...
            /// An end-group tag names a field other than the innermost
            /// open group's.
            GroupEndMismatch {
            ...
            /// A group still open when its extent ends.
            GroupUnclosed {
                /// The open group's field.
                open: FieldNumber,
            },
        ...
        impl core::fmt::Display for Fault {
            fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
                ...
                match self.kind {
                    ...
                    FaultKind::GroupEndMismatch { open, found } => write!(
                        f,
                        "end tag at {at} names field {} while group field {} is open",
                        found.as_inner(),
                        open.as_inner()
                    ),
                    ...
                    FaultKind::GroupUnclosed { open } => {
                        write!(f, "group of field {} opened at {at} never closes", open.as_inner())
                    }
                }
            }
        }
        ...
        $crate::editor::grouped::one_shot_machine!(@refusal $acc, [$noun]);
        $crate::editor::grouped::one_shot_machine!(@open_fault $doors, [$noun] [$a_noun]);

        $crate::editor::grouped::one_shot_machine!(@1s_edit_fault $cap, noun: $noun, a_noun: $a_noun, A_noun: $A_noun);
        ...
        pub enum Descent<'p> {
            ...
            Faulted(&'p Fault),
            #[doc = concat!(" The payload is lawful wire outside this ", $noun, "'s declared")]
            /// bounds (resident).
            Refused(&'p Refusal),
        }
        ...
        $crate::editor::grouped::one_shot_machine!(@1s_src_marks $cap);

        $crate::editor::grouped::one_shot_machine!(@1s_base_enum $cap);

        $crate::editor::grouped::one_shot_machine!(@row_struct $acc);

        impl Row {
            $crate::editor::grouped::one_shot_machine!(@row_ctor $cap $acc);

            $crate::editor::grouped::one_shot_machine!(@1s_row_base $cap);

            const fn set_replaced(&mut self) {
                self.state = (self.state & !BASE_MASK) | BASE_REPLACED;
            }

            $crate::editor::grouped::one_shot_machine!(@1s_row_src_marks $cap);
            ...
            $crate::editor::grouped::one_shot_machine!(@1s_rides_verbatim $cap, );

            $crate::editor::grouped::one_shot_machine!(@row_widths $cap $acc);
            ...
            /// The payload's offset in the row's zone. The one kind
            /// dispatch: a LEN's delimiter precedes its payload, a
            /// group's follows it. In the coordinate class: the
            /// payload's bytes follow the offset inside the admitted
            /// zone, so the sum cannot leave it.
        ...
        $crate::editor::grouped::one_shot_machine!(@scan $acc);
        ...
        $crate::editor::grouped::one_shot_machine!(@1s_arm_enum $cap);
        ...
        enum Close {
            /// A spine LEN: the framing decision waits on the body size,
            /// recorded for the emit pass.
            Len {
                /// The body's slot in the size table.
                slot: usize,
                /// The source prefix's met width (the verbatim
                /// candidate).
                prefix_w: WordWidth,
            ...
            /// A source group: verbatim framing on both sides.
            ReGroup {
                /// The source tag's met width.
                tag_w: WordWidth,
                /// The source end tag's met width.
                end_w: WordWidth,
            },
            /// An authored group: minimal framing on both sides.
            NewGroup { head: u32, end_word: u32 },
        }

        $crate::editor::grouped::one_shot_machine!(@1s_out_family $cap);
        ...
        $crate::editor::grouped::one_shot_machine!(@1s_import_value_at $cap);
        $crate::editor::grouped::one_shot_machine!(@1s_import_group_end $cap);
        ...
        $crate::editor::grouped::one_shot_machine!(@canonical_vocab $cap $acc);
    };
    (@1s_out_family transfer) => {
        ...
        trait Out<'a> {
            ...
            /// Emits one minimal head or group-end word.
    ...
    (@1s_edit_fault $cap:ident, noun: $noun:literal, a_noun: $a_noun:literal, A_noun: $A_noun:literal) => {
        ...
        pub enum EditFault {
            ...
            /// emitted owner.
            MoveIntoSource,
            /// The transfer would nest groups past the declared depth
            /// bound: the enclosing destination containers plus the
            /// source's structural group depth exceed the limit, so the
            /// saved document could not re-open under it.
            DepthExceeded {
                /// The declared depth bound.
                limit: u16,
                /// The nesting the refused transfer would reach.
                need: u32,
            },
            #[doc = concat!(" The ", $noun, "'s edit storage is full; the refusal is permanent")]
            #[doc = concat!(" for this ", $noun, ".")]
        ...
        impl core::fmt::Display for EditFault {
            fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
                match *self {
                    ...
                    Self::DepthExceeded { limit, need } => write!(
                        f,
                        "the transfer nests groups to depth {need}, past the declared bound {limit}"
                    ),
                    Self::IndexSpaceExhausted => f.write_str(concat!("the ", $noun, "'s edit storage is full")),
                }
            }
    ...
    (@1s_src_marks transfer) => {
        ...
        /// a row of a local whole-record copy's cloned structural
        /// closure, or one scanned out of such a copy's retained
        ...
        /// transfer-designated — an imported external record (a group's
        /// closure with its end tag included) or a local source-payload
        /// designation, discriminated by `Row.value` ([`SrcValue`]). The
        /// designation rides rows, never the payload stores: ordinary
        /// slots stay untouched by the transfer faces.
        const BASE_SRC: u8 = 3;
    ...
    (@1s_arm_enum plain) => {
        ...
        enum Arm {
            ...
            /// A source group: tag and end tag verbatim, interior
            /// recurses.
            ReGroup { tag_at: u32, tag_end: u32, end_at: u32, end_end: u32, first: Option<RowId> },
            /// An authored group: minimal open and end words.
            NewGroup { head: u32, end_word: u32, first: Option<RowId> },
        }
    };
    (@1s_arm_enum transfer) => {
        ...
        enum Arm {
            ...
            /// An imported LEN with interior edits: the slot tag rides
            /// verbatim, the prefix rides verbatim iff the walked body
            /// keeps the slot's length, and the walk recurses into the
            /// first-class interior rows. Offsets are slot-local.
            ImportSpine {
            ...
            /// An imported group with interior edits: both met tags ride
            /// verbatim from the slot, and the walk recurses into the
            /// first-class interior rows. Offsets are slot-local.
            ImportGroup {
                /// The open tag's window in the slot.
                tag_at: u32,
                /// One past the open tag.
                tag_end: u32,
                /// The end tag's window in the slot.
                end_at: u32,
                /// One past the end tag (the slot's length).
                end_end: u32,
                /// The import slot: the interior windows' zone.
                value: PayloadAt,
                /// The interior's first row.
    ...
    (@1s_import_group_end plain) => {};
    (@1s_import_group_end transfer) => {
        /// The end tag's offset inside an imported group's slot bytes.
        /// The suffix is the group-end word, possibly zero-extended;
        /// suffix decodes are width-unique for nonzero words, so the
        /// shortest terminating suffix decoding the end word is the met
        /// end tag. Imported groups are structurally complete, so the
        /// probe cannot refuse.
        fn import_group_end(bytes: &[u8], field: FieldNumber) -> u32 {
            let end_word = u64::from(group_end_word(field));
            for width in 1..=bytes.len().min(10) {
                let at = bytes.len() - width;
                if let Ok((word, met)) = slice::value64(bytes, at, bytes.len()) {
                    if usize::from(met) == width && word == end_word {
                        return admitted_u32(at);
                    }
                }
            }
    ...
    (@1s_nesting plain, $Machine:ident, [$noun:literal] [$a_noun:literal] [$A_noun:literal] [$doc_mod:literal] [$doc_open:literal] [$doc_open_empty:literal]) => {
            /// The record's structural group nesting for a designation:
            /// zero for non-groups, one plus the deepest nested group
            /// for a group closure. The fact is source-derived: the walk
            /// descends source-backed group kids only — authored
            /// subtrees are skipped whole, so edits under the closure
            /// never move an original designation's depth — and
            /// descended LEN interiors sit under an opacity boundary,
            /// contributing nothing.
            fn structural_nesting(&self, id: RowId) -> u32 {
                if !matches!(self.row(id).kind, RecordKind::Group) {
                    return 0;
                }
                let mut deepest: u32 = 1;
                let mut depth: u32 = 1;
                let mut open = id;
                let mut cur = self.row(id).kid;
                loop {
                    match cur {
                        Some(next) => {
                            let row = self.row(next);
                            if matches!(row.kind, RecordKind::Group) && row.has_source() {
                                depth += 1;
                                if depth > deepest {
                                    deepest = depth;
                                }
                        ...
                        None => {
                            ...
                            let done = self.row(open);
                            depth -= 1;
                            cur = done.next;
                            let Some(parent) = done.parent else {
                                unreachable!("group descent stays under the designated record")
                            };
                            open = parent;
                        }
                    }
    ...
    (@1s_nesting $cap:ident, $Machine:ident, [$noun:literal] [$a_noun:literal] [$A_noun:literal] [$doc_mod:literal] [$doc_open:literal] [$doc_open_empty:literal]) => {
            /// The record's structural group nesting for a designation:
            /// zero for non-groups, one plus the deepest nested group
            /// for a group closure. The fact is source-derived: the walk
            /// descends source-backed group kids only — authored,
            /// imported, and copied subtrees are skipped whole, so edits
            /// under the closure never move an original designation's
            /// depth — and descended LEN interiors sit under an opacity
            /// boundary, contributing nothing.
            fn structural_nesting(&self, id: RowId) -> u32 {
                if !matches!(self.row(id).kind, RecordKind::Group) {
                    return 0;
                }
    ...
    (@1s_descend plain, $Machine:ident, [$noun:literal] [$a_noun:literal] [$A_noun:literal] [$doc_mod:literal] [$doc_open:literal] [$doc_open_empty:literal]) => {
            /// Opens a container's interior for editing. Groups are open
            /// from the scan; a LEN payload parses on the first call — an
            /// explicit commitment that these bytes are a message, never a
            /// speculation — and the verdict is resident: a wire fault or
            /// a refusal parks on the record and projects unchanged on
            /// every later call.
            ///
            /// # Errors
            ///
            ...
            ///
            /// // LEN f2 wrapping { varint f1=1 } · LEN f3 { group cut short }
            ...
            ///
            #[doc = concat!(" // The broken payload's verdict is resident, and the ", $noun)]
            /// // lives on.
            #[doc = concat!(" assert!(matches!(", $noun, ".descend(tops[1]).unwrap(), Descent::Faulted(_)));")]
            #[doc = concat!(" assert!(matches!(", $noun, ".descend(tops[1]).unwrap(), Descent::Faulted(_)));")]
            /// ```
            #[track_caller]
            pub fn descend(&mut self, handle: Handle) -> Result<Descent<'_>, EditFault> {
                ...
                match row.kind {
                    RecordKind::Group => return Ok(Descent::Opened { first: row.kid.map(Handle) }),
                    RecordKind::Len => {}
    ...
    (@1s_descend transfer, $Machine:ident, [$noun:literal] [$a_noun:literal] [$A_noun:literal] [$doc_mod:literal] [$doc_open:literal] [$doc_open_empty:literal]) => {
            ...
            /// every later call.
            ///
            /// An imported record's interior descends too: the first call
            /// parses it — a LEN's payload or a group's closure between
            /// the met tags — into first-class rows, readable and
            /// editable like scanned rows, over the import's own bytes. Verdict
            /// and fault offsets inside them index the import's own byte
            ...
            pub fn descend(&mut self, handle: Handle) -> Result<Descent<'_>, EditFault> {
                ...
                match row.kind {
                    RecordKind::Group => {
                        // An imported group's interior parses out of its
                        // slot between the met tags into first-class
                        // rows; every other group's interior is live
                        // rows already.
                        if !matches!(row.base(), Base::Src) {
                            return Ok(Descent::Opened { first: row.kid.map(Handle) });
                        }
                    }
                    RecordKind::Len => {}
                ...
                match row.base() {
                    Base::Intact => {}
                    // An imported record's interior parses out of its
                    // slot at slot-local offsets into first-class rows.
                ...
                // rows, the import slot (slot-local, groups between
                // their met tags) for imports.
                let (zone_mark, body_at, body_end): (u8, u32, u32) = if matches!(row.base(), Base::Src) {
                    ...
                    if matches!(row.kind, RecordKind::Group) {
                        (ZONE_IMPORT, admitted_u32(at), import_group_end(bytes, row.field))
                    } else {
                        match slice::len_word(bytes, at, bytes.len()) {
                            Ok((len, width)) => {
                ...
                let scan = if matches!(row.base(), Base::Src) {
                    ...
                    scan_layer(&mut self.rows, bytes, body_at, body_end, Some(id), depth + 1, self.limit)
                } else {
                    scan_layer(
                        &mut self.rows,
                        &self.source,
    ...
    (@1s_container_gate plain, $Machine:ident, [$noun:literal] [$a_noun:literal] [$A_noun:literal] [$doc_mod:literal] [$doc_open:literal] [$doc_open_empty:literal]) => {
    ...
    (@1s_container_gate transfer, $Machine:ident, [$noun:literal] [$a_noun:literal] [$A_noun:literal] [$doc_mod:literal] [$doc_open:literal] [$doc_open_empty:literal]) => {
            /// Gates an insertion container; `Ok` carries its row.
            #[track_caller]
            fn container_gate(&self, container: Option<Handle>) -> Result<Option<RowId>, EditFault> {
                let Some(handle) = container else {
                ...
                match row.kind {
                    RecordKind::Group => {
                        // A live import's descended group interior is
                        // first-class and admits splices like any opened
                        // scanned layer; an undescended import stays
                        // sealed. Scanned groups, authored groups, and
                        // local copies' cloned closures all carry real
                        // interiors and admit splices.
                        if matches!(row.base(), Base::Src) {
                            if row.opened() {
                                return Ok(Some(handle.0));
                            }
                            return Err(EditFault::TargetUnopened);
                        }
                        Ok(Some(handle.0))
                    }
                    RecordKind::Len => {
    ...
    (@1s_transfer_faces $cap:ident, $Machine:ident, [$noun:literal] [$a_noun:literal] [$A_noun:literal] [$doc_mod:literal] [$doc_open:literal] [$doc_open_empty:literal]) => {
            ...
            /// Refuses a transfer whose group structure would nest past
            /// the declared depth bound: the enclosing destination
            /// containers plus the source's structural group depth must
            /// stay within it, or the saved document could not re-open
            /// under this limit. Judged before any reservation; the
            /// account is exact for the depths the transfer itself
            /// creates (scalar and LEN sources carry no group depth and
            /// pass whole).
            fn transfer_depth_gate(
                &self,
                parent: Option<RowId>,
                source_depth: u32,
            ) -> Result<(), EditFault> {
                let enclosing = parent.map_or(0, |at| self.depth_of(at) + 1);
                let need = enclosing + source_depth;
                if need > u32::from(self.limit.as_inner()) {
                    return Err(EditFault::DepthExceeded { limit: self.limit.as_inner(), need });
                }
                Ok(())
            }

            /// Clones the designated record's structural closure under
            /// an output-authored identity: the root row and — for a
            /// group — every row reachable through group kids, in order
            /// (a LEN interior stays opaque in the copy even if the
            /// source handle had been descended). Rows append at the
            /// arena tail; on a spent row domain the appended tail is
            /// discarded whole and nothing was linked.
            fn clone_alias_subtree(
                &mut self,
                src_root: RowId,
                parent: Option<RowId>,
                next: Option<RowId>,
            ) -> Result<RowId, EditFault> {
                let mark = self.rows.len();
                match self.try_clone_rows(src_root, parent, next) {
                    Ok(root) => Ok(root),
                    Err(fault) => {
                        self.rows.truncate(mark);
                        Err(fault)
            ...
            /// [`Self::clone_alias_subtree`]'s fallible walk: group kids
            /// descend (their structure is mandatory), everything else
            /// clones flat.
            fn try_clone_rows(
                &mut self,
                src_root: RowId,
                ...
                /// One suspended sibling chain of the clone walk.
                struct CloneFrame {
                    src_next: Option<RowId>,
                    clone_parent: RowId,
                    prev_clone: Option<RowId>,
                }

                let root_id = self.mint_insert()?;
                let src = *self.row(src_root);
                self.rows.push(src.cloned_alias(parent, next));
                if !matches!(src.kind, RecordKind::Group) {
                    return Ok(root_id);
                }
                let mut stack: Vec<CloneFrame> = Vec::new();
                let mut src_cur = src.kid;
                let mut clone_parent = root_id;
                let mut prev_clone: Option<RowId> = None;
                loop {
                    match src_cur {
                        Some(sid) => {
                            let member = *self.row(sid);
                            // The designation names the source reading:
                            // members without source geometry — authored,
                            // imported, or copied — are pending output,
                            // not source bytes, and do not clone (their
                            // subtrees included).
                            if !member.has_source() {
                                src_cur = member.next;
                                continue;
                            }
                            let cid = self.mint_insert()?;
                            let mut clone = member.cloned_alias(Some(clone_parent), None);
                            // Interior rows carry the identity alone; the
                            // root marks the splice.
                            clone.state = (clone.state & !ZONE_MASK) | ZONE_ALIAS;
                            self.rows.push(clone);
                            match prev_clone {
                                Some(prev) => self.row_mut(prev).next = Some(cid),
                                None => self.row_mut(clone_parent).kid = Some(cid),
                            }
                            if matches!(member.kind, RecordKind::Group) {
                                stack.push(CloneFrame {
                                    src_next: member.next,
                                    clone_parent,
                                    prev_clone: Some(cid),
                                });
                                clone_parent = cid;
                        ...
                        None => match stack.pop() {
                            Some(frame) => {
                                src_cur = frame.src_next;
                                clone_parent = frame.clone_parent;
                                prev_clone = frame.prev_clone;
                            }
                            None => break,
                        },
                    }
            ...
            /// widths, the whole payload, a group's complete closure and
            /// matching end tag — while the original keeps its own
            /// pre-existing edit state. The copy is output-authored: its
            /// status reads `Inserted`, it answers no source span, and
            /// it does not designate; a group clones its mandatory
            /// structure, while a LEN interior starts opaque and a later
            #[doc = concat!(" [`", stringify!($Machine), "::descend`] parses the retained source-backed")]
            /// bytes (unlike caller-authored opaque payloads). Zero
            /// payload bytes stage: the rows store coordinates into the
            /// machine's own source.
            ///
            ...
            /// rows), [`EditFault::DepthExceeded`] when the enclosing
            /// destination containers plus the closure's group depth
            /// pass the declared bound,
            /// [`EditFault::IndexSpaceExhausted`] when the cloned
            /// closure would leave the row domain, plus the anchor gates
            ...
            ///
            /// // Group f1 { varint f2=5 }: the closure copies whole.
            ...
            #[doc = concat!(" assert_eq!(", $noun, ".save().unwrap(), [0x0B, 0x10, 0x05, 0x0C, 0x0B, 0x10, 0x05, 0x0C]);")]
            /// ```
            #[track_caller]
            pub fn copy_record(&mut self, source: Handle, at: InsertAt) -> Result<Handle, EditFault> {
                ...
                self.transfer_depth_gate(plan.parent, self.structural_nesting(source.0))?;
                let next = self.plan_next(&plan);
                let id = self.clone_alias_subtree(source.0, plan.parent, next)?;
                self.link_row(&plan, id);
            ...
            /// subtree, [`EditFault::DepthExceeded`] when the enclosing
            /// destination containers plus the closure's group depth
            /// pass the declared bound,
            /// [`EditFault::IndexSpaceExhausted`] when the
            /// cloned closure would leave the row domain, plus the
            #[doc = concat!(" anchor gates of [`", stringify!($Machine), "::insert_varint`]. On any `Err` the")]
            ...
            #[doc = concat!(" use ", $doc_mod, "::{InsertAt, ", stringify!($Machine), "};")]
            ///
            /// // Group f1 { varint f2=5 } · varint f3=6: relocate the
            /// // group after the scalar.
            /// let msg = [0x0B, 0x10, 0x05, 0x0C, 0x18, 0x06];
            #[doc = concat!(" let mut ", $noun, " = ", $doc_open, ".unwrap();")]
            #[doc = concat!(" let tops: Vec<_> = ", $noun, ".top().collect();")]
            #[doc = concat!(" ", $noun, ".move_record(tops[0], InsertAt::After(tops[1])).unwrap();")]
            #[doc = concat!(" assert_eq!(", $noun, ".save().unwrap(), [0x18, 0x06, 0x0B, 0x10, 0x05, 0x0C]);")]
            /// ```
            #[track_caller]
            pub fn move_record(&mut self, source: Handle, at: InsertAt) -> Result<Handle, EditFault> {
                self.move_source(source)?;
                let plan = self.resolve_anchor(at)?;
            ...
            /// interior lands as the source's declaration — opaque
            /// bytes, judged only if an explicit descend later commits
            /// them. A group has no payload: its interior is structural
            #[doc = concat!(" wire, transferred whole by [`", stringify!($Machine), "::copy_record`].")]
            ///
            /// # Errors
            ///
            ...
            fn settle(&self, row: &Row) -> Arm {
                ...
                match row.base() {
                    ...
                    Base::Intact => match row.kind {
                        ...
                        RecordKind::Group => {
                            let tag_end = row.start.as_inner() + row.tag_w();
                            let end_at = tag_end + row.payload_len.as_inner();
                            Arm::ReGroup {
                                tag_at: row.start.as_inner(),
                                tag_end,
                                end_at,
                                end_end: row.span_end(),
                                first: row.kid,
                            }
                        }
                    ...
                    Base::Replaced => {
                        ...
                        match row.kind {
                            ...
                            // Groups have no payload replacement: the
                            // setter's kind gate closes this arm.
                            RecordKind::Group => unsafe {
                                debug_assert!(false, "a group cannot be Replaced");
                    ...
                    Base::Inserted => {
                        ...
                        match row.kind {
                            ...
                            RecordKind::Group => {
                                Arm::NewGroup { head, end_word: group_end_word(row.field), first: row.kid }
                            }
                        }
                    }
                }
            ...
            fn settle(&self, row: &Row) -> Arm {
                ...
                match row.base() {
                    ...
                    Base::Src => match row.src_value() {
                        ...
                        // instead — LENs re-derive the prefix from the
                        // walked body, groups keep both met tags.
                        SrcValue::Imported(value) => {
                            if row.opened() && row.dirty() {
                                let bytes = self.import_slot(value);
                                let at = import_value_at(bytes);
                                if matches!(row.kind, RecordKind::Group) {
                                    let end_at = import_group_end(bytes, row.field);
                                    Arm::ImportGroup {
                                        tag_at: 0,
                                        tag_end: admitted_u32(at),
                                        end_at,
                                        end_end: admitted_u32(bytes.len()),
                                        value,
                                        first: row.kid,
                                    }
                                } else {
            ...
            fn size_subtree(&self, root: RowId, bodies: &mut Vec<u32>) -> Result<u64, SaveFault> {
                ...
                loop {
                    let Some(id) = cur else {
                        ...
                        acc = frame.outer
                            + match frame.close {
                                Close::Len { slot, prefix_w, src_len, at, tag_w } => {
                                    ...
                                    bodies[slot] = body;
                                    let prefix =
                                        if body == src_len { prefix_w.w() } else { encoded_len32(body) };
                                    u64::from(tag_w.w()) + u64::from(prefix) + acc
                                }
                                Close::ReGroup { tag_w, end_w } => {
                                    u64::from(tag_w.w()) + acc + u64::from(end_w.w())
                                }
                                Close::NewGroup { head, end_word } => {
                                    u64::from(encoded_len32(head))
                                        + acc
                    ...
                    match self.settle(row) {
                        ...
                        Arm::Spine { tag_at, tag_end, prefix_end, src_len, first } => {
                            let slot = bodies.len();
                            bodies.push(0);
                            let (tag_w, prefix_w) = $crate::editor::grouped::one_shot_machine!(
                            ...
                            spine.push(SizeFrame {
                                ...
                                outer: acc,
                                close: Close::Len { slot, prefix_w, src_len, at: tag_at, tag_w },
                            });
                            acc = 0;
                            cur = first;
                            continue;
                        }
                        Arm::ReGroup { tag_at, tag_end, end_at, end_end, first } => {
                            let (tag_w, end_w) = $crate::editor::grouped::one_shot_machine!(
                                @frame_group_widths $acc, row, tag_at, tag_end, end_at, end_end
                            );
                            spine.push(SizeFrame {
                                next: if spine.is_empty() { None } else { row.next },
                                outer: acc,
                                close: Close::ReGroup { tag_w, end_w },
                            });
                            acc = 0;
                            cur = first;
                            continue;
                        ...
                        Arm::NewGroup { head, end_word, first } => {
                            spine.push(SizeFrame {
                                next: if spine.is_empty() { None } else { row.next },
                                outer: acc,
                                close: Close::NewGroup { head, end_word },
                            });
                            acc = 0;
                            cur = first;
                            continue;
            ...
            fn emit_subtree<O: Out>(
                ...
                loop {
                    let Some(id) = cur else {
                        ...
                        if matches!(row.kind, RecordKind::Group) {
                            match row.base() {
                                // A source group's end tag rides verbatim.
                                Base::Intact => {
                                    let end_at = row.start.as_inner() + row.tag_w() + row.payload_len.as_inner();
                                    emit.verbatim(end_at, row.span_end());
                                }
                                // An authored group's end tag is minimal;
                                // groups are never `Replaced`.
                                Base::Replaced | Base::Inserted => {
                                    emit.word(group_end_word(row.field));
                                }
                            }
                        }
                    ...
                    match self.settle(row) {
                        ...
                        Arm::ReGroup { tag_at, tag_end, first, .. } => {
                            emit.verbatim(tag_at, tag_end);
                            open = Some(id);
                            cur = first;
                            continue;
            ...
            fn size_subtree(&self, root: RowId, bodies: &mut Vec<u32>) -> Result<u64, SaveFault> {
                ...
                loop {
                    ...
                    match self.settle(row) {
                        ...
                        Arm::ImportGroup { tag_at, tag_end, end_at, end_end, first, .. } => {
                            #[allow(
                                clippy::cast_possible_truncation,
                                clippy::as_conversions,
                                reason = "framing windows span at most five bytes"
                            ...
                            // SAFETY: the settle measured the slot's met
                            // framing — the tag window from the import's
                            // own admission and the end window from the
                            // slot's terminating end-tag suffix — both
                            // inside the five-byte window.
                            let (tag_w, end_w) = unsafe {
                                (
                                    WordWidth::met_unchecked((tag_end - tag_at) as u8),
                                    WordWidth::met_unchecked((end_end - end_at) as u8),
                                )
                            };
                            spine.push(SizeFrame {
                                next: if spine.is_empty() { None } else { row.next },
            ...
            fn emit_subtree<'s, O: Out<'s>>(
                ...
                loop {
                    let Some(id) = cur else {
                        ...
                        if matches!(row.kind, RecordKind::Group) {
                            match row.base() {
                                ...
                                Base::Intact => {
                                    let end_at = row.start.as_inner() + row.tag_w() + row.payload_len.as_inner();
                                    emit.verbatim_in(self.zone_of(row), end_at, row.span_end());
                                }
                                // An authored group's end tag is minimal;
                                // groups are never `Replaced`.
                                Base::Replaced | Base::Inserted => {
                                ...
                                // A dirty imported group's met end tag
                                // rides verbatim from its slot.
                                Base::Src => {
                                    let bytes = self.import_bytes(row);
                                    emit.verbatim_in(
                                        bytes,
                                        import_group_end(bytes, row.field),
                    ...
                    match self.settle(row) {
                        ...
                        Arm::ImportGroup { tag_at, tag_end, value, first, .. } => {
                            emit.verbatim_in(self.import_slot(value), tag_at, tag_end);
                            open = Some(id);
                            cur = first;
                            continue;
                        }
    ...
    (@1s_splice_spans plain, $Machine:ident, [$noun:literal] [$a_noun:literal] [$A_noun:literal] [$doc_mod:literal] [$doc_open:literal] [$doc_open_empty:literal]) => {
            ...
            /// climb-out — a group's end after its end tag prices in.
            fn splice_spans(
                &self,
                root: RowId,
                bodies: &[u32],
                ...
                loop {
                    let Some(id) = cur else {
                        ...
                        if matches!(row.kind, RecordKind::Group) {
                            out += match row.base() {
                                // A source group's end tag rides verbatim.
                                Base::Intact => row.delim_w(),
                                // An authored group's end tag is minimal;
                                // groups are never `Replaced`.
                                Base::Replaced | Base::Inserted => encoded_len32(group_end_word(row.field)),
                            };
                        }
            ...
            fn splice_spans(
                ...
                loop {
                    let Some(id) = cur else {
                        ...
                        if matches!(row.kind, RecordKind::Group) {
                            out += match row.base() {
                                ...
                                // groups are never `Replaced`.
                                Base::Replaced | Base::Inserted => encoded_len32(group_end_word(row.field)),
                                // Transfer-designated records price whole
                                // through their own arms; the walk never
                                // opens them.
                                // A dirty imported group's met end
                                // tag prices verbatim from its slot.
                                Base::Src => {
                                    let bytes = self.import_bytes(row);
                                    admitted_u32(bytes.len())
                                        - import_group_end(bytes, row.field)
                    ...
                    match self.settle(row) {
                        ...
                        Arm::ReGroup { tag_at, tag_end, first, .. }
                        | Arm::ImportGroup { tag_at, tag_end, first, .. } => {
                            frames.push(entries.len());
                            entries.push((Handle(id), Span::new(out, out)));
                            out += tag_end - tag_at;
                            open = Some(id);
    ...
    (@1s_sp_mixed plain, payload: $p:lifetime, $Machine:ident, [$noun:literal] [$a_noun:literal] [$A_noun:literal] [$doc_mod:literal] [$doc_open:literal] [$doc_open_empty:literal]) => {
            ...
            /// ```
            /// use protobuf_edit::DepthLimit;
            #[doc = concat!(" use ", $doc_mod, "::{Descent, EditFault, ", stringify!($Machine), "};")]
            ///
            /// let msg = [0x12, 0x02, 0x68, 0x69];
            #[doc = concat!(" let mut ", $noun, " = ", $doc_open, ".unwrap();")]
            #[doc = concat!(" let record = ", $noun, ".top().next().unwrap();")]
            ///
            /// // Same length: the prefix will ride verbatim at save.
            #[doc = concat!(" ", $noun, ".set_payload(record, b\"no\").unwrap();")]
            #[doc = concat!(" assert_eq!(", $noun, ".payload_bytes(record).unwrap(), b\"no\");")]
            ///
            /// // An opened interior refuses wholesale replacement.
            #[doc = concat!(" let mut opened = ", $doc_open, ".unwrap();")]
            /// let record = opened.top().next().unwrap();
            /// assert!(matches!(opened.descend(record).unwrap(), Descent::Opened { .. }));
            /// assert_eq!(opened.set_payload(record, b"no").err(), Some(EditFault::OpenedTarget));
            /// ```
            #[inline]
            #[track_caller]
    ...
    (@1s_cloned_alias $cap:ident tolerant) => {
            ...
            /// exact geometry under an output-authored identity. Group
            /// closures clone their structural rows separately; a LEN
    ...
    (@1s_link_row plain, $Machine:ident, [$noun:literal] [$a_noun:literal] [$A_noun:literal] [$doc_mod:literal] [$doc_open:literal] [$doc_open_empty:literal]) => {};
    (@1s_link_row $cap:ident, $Machine:ident, [$noun:literal] [$a_noun:literal] [$A_noun:literal] [$doc_mod:literal] [$doc_open:literal] [$doc_open_empty:literal]) => {
            /// Links an already-pushed row into its proven plan — the
            /// linking suffix every splice shares.
            fn link_row(&mut self, plan: &Plan, id: RowId) {
                match plan.prev {
                    Some(prev) => self.row_mut(prev).next = Some(id),
                    None => match plan.parent {
                        Some(parent) => self.row_mut(parent).kid = Some(id),
    ...
    (@canonical_vocab plain tolerant) => {
        ...
        enum CanonicalArm {
            ...
            /// A group: minimal open tag, the live in-band interior,
            /// then the matching minimal end tag — group interiors are
            /// syntax and always sit in the closure.
            Group { head: u32, end_word: u32, first: Option<RowId> },
        }
        ...
        /// One frame of the canonical sizing walk's container spine.
        ...
        /// What a canonical sizing frame contributes at close.
        enum CanonicalClose {
            /// An opened LEN: minimal head, the prefix re-derived from
            /// the canonical body total, recorded for the emit walk.
            Len {
                ...
                /// coordinate class (the settle minted it typed), raw
                /// on a layout ground — a coordinate-typed niche here
                /// repacks the close from 24 to 16 bytes, reshaping
                /// the sizing walk.
                at: u32,
            },
            /// A group: minimal framing words on both sides.
            Group { head: u32, end_word: u32 },
        }
    };
    ...
    (@canonical_vocab $cap:ident tolerant) => {
        ...
        enum CanonicalArm {
            ...
            /// syntax and always sit in the closure.
            Group { head: u32, end_word: u32, first: Option<RowId> },
            /// An imported external record: its slot's token stream
            /// re-encodes minimally (LEN interiors opaque, group
            /// closures walked whole).
            Import { value: PayloadAt },
        }
    ...
    (@met_geometry canonical, $row:ident) => {
        (
            ...
            match $row.kind {
                ...
                RecordKind::Group => {
                    Some(WordWidth::minimal_of(group_end_word($row.field)))
                }
    ...
    // A group frame's met framing widths at its push, same doors.
    (@frame_group_widths tolerant, $row:ident, $tag_at:ident, $tag_end:ident, $end_at:ident, $end_end:ident) => {
        match ($row.tag_width, $row.delim_width) {
            (Some(tag_w), Some(end_w)) => {
                debug_assert!(
                    $tag_end - $tag_at == tag_w.w() && $end_end - $end_at == end_w.w(),
                    "the settle's group windows are the stored met widths"
                );
                (tag_w, end_w)
            }
            // SAFETY: the settle frames only geometry-owning group
            // rows, whose scan (or clone) birth stored the met tag
            // width and whose group close patched the met end width.
            _ => unsafe { core::hint::unreachable_unchecked() },
        }
    };
    (@frame_group_widths canonical, $row:ident, $tag_at:ident, $tag_end:ident, $end_at:ident, $end_end:ident) => {{
        #[allow(
            clippy::cast_possible_truncation,
            clippy::as_conversions,
            reason = "framing windows span at most five bytes"
        ...
        // SAFETY: the settle built the windows from the row's own
        // framing (`tag_end = tag_at + tag_w`,
        // `end_at = end_end - delim_w`), and canonical admission pins
        // each width to its word's encoded length — both subtractions
        // land in the five-byte window.
        let widths = unsafe {
            (
                WordWidth::met_unchecked(($tag_end - $tag_at) as u8),
                WordWidth::met_unchecked(($end_end - $end_at) as u8),
    ...
    (@open_fault buffered, [$noun:literal] [$a_noun:literal]) => {
        ...
        pub enum OpenFault {
            ...
            Wire(Fault),
            #[doc = concat!(" The root layer is lawful wire outside this ", $noun, "'s declared")]
            /// bounds.
            Refused(Refusal),
        }
    ...
    (@refusal tolerant, [$noun:literal]) => {
        #[doc = concat!(" Lawful wire this ", $noun, "'s declared bounds refuse.")]
        #[non_exhaustive]
        #[derive(Clone, Copy, PartialEq, Eq, Debug)]
        pub enum Refusal {
    ...
    (@refusal canonical, [$noun:literal]) => {
        ...
        /// canonical-minimal policy and the declared depth bound.
        #[non_exhaustive]
        #[derive(Clone, Copy, PartialEq, Eq, Debug)]
        pub enum Refusal {
    ...
    (@row_struct tolerant) => {
        ...
        /// union the whole record, order kind-dependent — so the record
        /// span is one formula for all kinds (the group end tag commutes
        /// into the sum) while the payload's position dispatches on kind.
        /// Widths are stored input facts: tolerant admission accepts
        /// padding, and span arithmetic must reproduce it byte-exactly.
        ///
        ...
        struct Row {
            ...
            start: Coord,
            /// Source payload extent: a LEN's declared length, a group's
            /// interior length, a varint value's scanned width, or 4/8 for
            /// the fixed kinds. Meaningless for authored rows.
            payload_len: Extent,
            ...
            /// The second framing word's actual input width: a LEN's
            /// length prefix or a group's end tag. `None` for scalars and
            /// authored rows.
    ...
    (@row_struct canonical) => {
        ...
        /// span is one formula for all kinds (the group end tag commutes
        /// into the sum) while the payload's position dispatches on kind.
        /// No width columns exist: canonical admission proved every
        /// framing word minimal, so widths are pure functions of the
        /// stored facts.
        ///
        /// Declaration order fixes the memory order: the coordinate
        /// columns tie with the links on niche size, so the stable
    ...
    (@row_ctor $cap:ident tolerant) => {
            ...
            $crate::editor::grouped::one_shot_machine!(@1s_cloned_alias $cap tolerant);
    };
    (@row_ctor $cap:ident canonical) => {
            ...
            $crate::editor::grouped::one_shot_machine!(@1s_cloned_alias $cap canonical);
    };
    ...
    (@row_widths plain canonical) => {
            ...
            /// each width is its word's own encoded length (a group's end
            /// tag encodes at its head tag's width — same field, adjacent
            /// code, one varint length).
            const fn tag_w(&self) -> u32 {
            ...
            const fn delim_w(&self) -> u32 {
                ...
                match self.kind {
                    RecordKind::Len => encoded_len32(self.payload_len.as_inner()),
                    RecordKind::Group => head_width(self.field, self.kind),
                    _ => 0,
                }
            }
    ...
    (@scan tolerant) => {
        /// Scans one layer of `bytes[start..end]` into provisional rows
        /// under `parent`, materializing groups eagerly (the open-group
        /// chain is the arena's own parent links — no stack, no
        /// recursion). Widths ride onto the rows as scanned; nothing is
        /// re-derived from values. `depth` counts the containers already
        /// open around this layer against `limit`. On any halt the caller
        ...
        fn scan_layer(
            ...
            let bound = u32::from(limit.as_inner());
            let mut first: Option<RowId> = None;
            // The innermost group this scan holds open (`None`: records
            // land directly in the scanned layer).
            let mut open: Option<RowId> = None;
            let mut open_depth = depth;
            // The previous sibling in the innermost layer.
            ...
            while pos < end {
                ...
                let kind = match classify(Low3::from_word(word)) {
                    ...
                    TagClass::GroupEnd => {
                        let Some(group) = open else {
                            return Err(halt_wire(pos, FaultKind::GroupEndOrphan { found: field }));
                        };
                        // SAFETY: `group` was minted by this scan's push.
                        let row = unsafe { rows.get_unchecked_mut(group.index()) };
                        if row.field != field {
                            return Err(halt_wire(
                                pos,
                                FaultKind::GroupEndMismatch { open: row.field, found: field },
                            ));
                        }
                        // SAFETY: the scan walked forward from the open
                        // tag, so `pos` sits at or past the head's
                        // payload start within one zone extent, and the
                        // zone tops at the class bound.
                        row.payload_len =
                            unsafe { Extent::new_unchecked(pos - (row.start.as_inner() + row.tag_w())) };
                        row.delim_width = Some(tag_width);
                        open = if row.parent == parent { None } else { row.parent };
                        open_depth -= 1;
                        last = Some(group);
                        pos = value_at;
                        continue;
                ...
                let (payload_len, delim_width, record_end) = match kind {
                    ...
                    // A group's extent and end-tag width are patched at
                    // its close.
                    RecordKind::Group => {
                        if open_depth == bound {
                            return Err(Halt::Refused(Refusal::DepthExceeded { at: pos, field }));
                        }
                        (Extent::from_width(0), None, value_at)
                    }
                };
                ...
                match last {
                    ...
                    None => match open {
                        Some(group) => {
                            // SAFETY: `group` was minted by this scan's push.
                            unsafe { rows.get_unchecked_mut(group.index()) }.kid = Some(id);
                        }
                        None => first = Some(id),
                    },
                }
                ...
                if matches!(kind, RecordKind::Group) {
                    open = Some(id);
                    open_depth += 1;
                    last = None;
                } else {
            ...
            if let Some(group) = open {
                let row = &rows[group.index()];
                return Err(halt_wire(row.start.as_inner(), FaultKind::GroupUnclosed { open: row.field }));
            }
            Ok(first)
        }
    };
    (@scan canonical) => {
        ...
        /// under `parent`, materializing groups eagerly (the open-group
        /// chain is the arena's own parent links — no stack, no
        /// recursion), refusing any framing word or varint value wider
        /// than its own encoding — the canonical-minimal gate that makes
        /// every downstream width a pure derivation. `depth` counts the
        /// containers already open around this layer against `limit`. On
        /// any halt the caller discards the provisional tail; nothing
        ...
        fn scan_layer(
            ...
            while pos < end {
                ...
                let (payload_len, record_end) = match kind {
                    ...
                    // A group's extent is patched at its close.
                    RecordKind::Group => {
                        if open_depth == bound {
                            return Err(Halt::Refused(Refusal::DepthExceeded { at: pos, field }));
                        }
                        (Extent::from_width(0), value_at)
                    }
                };
                let id = mint_row(rows)?;
                match last {
                ...
                // the LEN class) tops at the class bound.
                let start = unsafe { Coord::new_unchecked(pos) };
                rows.push(Row::scanned(field, kind, start, payload_len, open.or(parent)));
                if matches!(kind, RecordKind::Group) {
                    open = Some(id);
                    open_depth += 1;
                    last = None;
    ...
    (@set_varint tolerant, [$noun:literal] [$doc_mod:literal] [$doc_open:literal], $Machine:ident) => {
            ...
            /// ```
            /// use protobuf_edit::DepthLimit;
            #[doc = concat!(" use ", $doc_mod, "::{EditFault, ", stringify!($Machine), "};")]
            /// use protobuf_edit::wire::grouped::RecordKind;
            ///
            /// let msg = [0x08, 0x96, 0x01, 0x11, 0, 0, 0, 0, 0, 0, 0, 0];
            #[doc = concat!(" let mut ", $noun, " = ", $doc_open, ".unwrap();")]
            #[doc = concat!(" let tops: Vec<_> = ", $noun, ".top().collect();")]
            ///
            #[doc = concat!(" ", $noun, ".set_varint(tops[0], 7).unwrap();")]
            #[doc = concat!(" assert_eq!(", $noun, ".varint_word(tops[0]).unwrap(), 7);")]
            ///
            /// // The kind gate is a structured refusal, not a panic.
            /// assert_eq!(
            #[doc = concat!("     ", $noun, ".set_varint(tops[1], 7).err(),")]
            ///     Some(EditFault::KindMismatch { have: RecordKind::I64 })
            /// );
            /// ```
            #[inline]
            #[track_caller]
    ...
    (@source_spans tolerant, [$noun:literal] [$doc_mod:literal] [$doc_open:literal], $Machine:ident) => {
            ...
            /// # Examples
            ///
            /// Padded framing is reported at its met width:
            ///
            /// ```
            /// use protobuf_edit::DepthLimit;
            #[doc = concat!(" use ", $doc_mod, "::{", stringify!($Machine), ", RecordSpans};")]
            ///
            /// // LEN f2, prefix 0x82 0x00 = 2 padded to two bytes.
            /// let msg = [0x12, 0x82, 0x00, 0x08, 0x01];
            #[doc = concat!(" let ", $noun, " = ", $doc_open, ".unwrap();")]
            #[doc = concat!(" let record = ", $noun, ".top().next().unwrap();")]
            #[doc = concat!(" let RecordSpans::Len { prefix, payload, .. } = ", $noun, ".source_spans(record).unwrap() else {")]
            ///     unreachable!()
            /// };
            ...
            pub fn source_spans(&self, handle: Handle) -> Option<RecordSpans> {
                ...
                Some(match row.kind {
                    ...
                    RecordKind::Group => {
                        let interior_end = tag.end() + row.payload_len.as_inner();
                        RecordSpans::Group {
                            tag,
                            interior: Span::new(tag.end(), interior_end),
                            end_tag: Span::new(interior_end, row.span_end()),
                        }
                    }
                })
            }
    };
    (@source_spans canonical, [$noun:literal] [$doc_mod:literal] [$doc_open:literal], $Machine:ident) => {
            /// The record's source geometry, split by role at the scanned
            /// widths; `None` for authored records.
            ///
            /// # Panics
            ...
            /// # Examples
            ///
            /// Every segment sits at the minimal width canonical
            /// admission proved:
            ///
            /// ```
            /// use protobuf_edit::DepthLimit;
            #[doc = concat!(" use ", $doc_mod, "::{", stringify!($Machine), ", RecordSpans};")]
            ///
            /// // LEN f2 holding two payload bytes.
            /// let msg = [0x12, 0x02, 0x08, 0x01];
            #[doc = concat!(" let ", $noun, " = ", $doc_open, ".unwrap();")]
            #[doc = concat!(" let record = ", $noun, ".top().next().unwrap();")]
            #[doc = concat!(" let RecordSpans::Len { prefix, payload, .. } = ", $noun, ".source_spans(record).unwrap() else {")]
            ///     unreachable!()
    ...
    (
        ...
        $crate::editor::grouped::one_shot_machine!(@struct $(#[$mdoc])* $Machine<$($lt),*> { source: $src } $payloads);
        $crate::editor::grouped::one_shot_machine!(@doors $tenure $acc $Machine<$($lt),*> { source: $src } $payloads, [$noun] [$a_noun] [$A_noun] [$doc_mod] [$doc_open] [$doc_open_empty]);
        $crate::editor::grouped::one_shot_machine!(@core $cap $acc $Machine<$($lt),*> [$noun] [$a_noun] [$A_noun] [$doc_mod] [$doc_open] [$doc_open_empty] [$doc_recipes]);
        $crate::editor::grouped::one_shot_machine!(@payload_mixed $cap $acc $Machine<$($lt),*> payload: $p, frames: ($Frame, $SizedFrame), [$noun] [$a_noun] [$A_noun] [$doc_mod] [$doc_open] [$doc_open_empty]);
        $crate::editor::grouped::one_shot_machine!(@frames $cap $acc $Machine<$($lt),*> frames: ($Frame, $SizedFrame), [$noun] [$a_noun] [$A_noun] [$doc_mod] [$doc_open] [$doc_open_empty]);
    };
    (
        $(#[$mdoc:meta])*
        machine $Machine:ident<$($lt:lifetime),*> { source: $src:ty }
        ...
        $crate::editor::grouped::one_shot_machine!(@doors $tenure $acc $Machine<$($lt),*> { source: $src } $payloads, [$noun] [$a_noun] [$A_noun] [$doc_mod] [$doc_open] [$doc_open_empty]);
        $crate::editor::grouped::one_shot_machine!(@core $cap $acc $Machine<$($lt),*> [$noun] [$a_noun] [$A_noun] [$doc_mod] [$doc_open] [$doc_open_empty] [$doc_recipes]);
        $crate::editor::grouped::one_shot_machine!(@payload_borrowed $cap $acc $Machine<$($lt),*> payload: $p, mixed: $Mixed, [$noun] [$a_noun] [$A_noun] [$doc_mod] [$doc_open] [$doc_open_empty]);
    };
    (
        $(#[$mdoc:meta])*
        machine $Machine:ident<$($lt:lifetime),*> { source: $src:ty }
        ...
        $crate::editor::grouped::one_shot_machine!(@doors $tenure $acc $Machine<$($lt),*> { source: $src } $payloads, [$noun] [$a_noun] [$A_noun] [$doc_mod] [$doc_open] [$doc_open_empty]);
        $crate::editor::grouped::one_shot_machine!(@core $cap $acc $Machine<$($lt),*> [$noun] [$a_noun] [$A_noun] [$doc_mod] [$doc_open] [$doc_open_empty] [$doc_recipes]);
        $crate::editor::grouped::one_shot_machine!(@payload_copied $cap $acc $Machine<$($lt),*> mixed: $Mixed, frames: ($Frame, $SizedFrame), [$noun] [$a_noun] [$A_noun] [$doc_mod] [$doc_open] [$doc_open_empty]);
        $crate::editor::grouped::one_shot_machine!(@frames $cap $acc $Machine<$($lt),*> frames: ($Frame, $SizedFrame), [$noun] [$a_noun] [$A_noun] [$doc_mod] [$doc_open] [$doc_open_empty]);
    };
    (
    ...
    (
        ...
        impl<$($lt),*> $Machine<$($lt),*> {
            /// Borrows and scans `source`: zero bytes are copied, the root
            /// layer materializes eagerly (groups included), and LEN
            #[doc = concat!(" payloads wait for [`", stringify!($Machine), "::descend`].")]
            ///
            /// # Errors
            ///
            /// [`OpenFault::TooLarge`] beyond the coordinate class
            /// (`i32::MAX` bytes), [`OpenFault::Wire`] when the root layer
            /// violates the wire grammar, and [`OpenFault::Refused`] when a
            /// root-layer container nests past `limit`.
            ///
            /// # Examples
            ///
            /// Tolerant admission: a continuation-padded tag is lawful
            /// input, stored at its met width; a truncated record is a
            /// grammar fault:
            ///
            /// ```
            /// use protobuf_edit::DepthLimit;
            #[doc = concat!(" use ", $doc_mod, "::{Fault, FaultKind, OpenFault, ", stringify!($Machine), "};")]
            ///
            /// // Field 1 varint 1, tag padded to two bytes.
            /// let padded = [0x88, 0x00, 0x01];
            #[doc = concat!(" assert!(", stringify!($Machine), "::open(&padded, DepthLimit::REFERENCE).is_ok());")]
            ///
            /// let cut = [0x08];
            /// assert!(matches!(
            #[doc = concat!("     ", stringify!($Machine), "::open(&cut, DepthLimit::REFERENCE).err(),")]
            ///     Some(OpenFault::Wire(Fault { at: 1, kind: FaultKind::Value { .. } }))
            /// ));
            /// ```
            pub fn open(source: $src, limit: DepthLimit) -> Result<Self, OpenFault> {
                ...
                let top = scan_layer(&mut rows, source, 0, len, None, 0, limit).map_err(|halt| {
                    match halt {
                        ...
                        Halt::Exhausted => {
                            ...
                            // source byte and admission bounds the source
                            // at `i32::MAX`, so a fresh arena cannot leave
                            // the row domain.
    ...
    (
        ...
        impl<$($lt),*> $Machine<$($lt),*> {
            ...
            /// copied (the buffer moves in), the root layer materializes
            /// eagerly (groups included), and LEN payloads wait for
            ...
            /// [`OpenFault::Refused`] when a root-layer container nests
            /// past `limit`.
            ///
            /// # Examples
            ///
            /// Tolerant admission: a continuation-padded tag is lawful
            /// input, stored at its met width; a truncated record is a
            /// grammar fault, and the buffer rides back untouched:
            ///
            /// ```
            /// use protobuf_edit::DepthLimit;
            #[doc = concat!(" use ", $doc_mod, "::{", stringify!($Machine), ", Fault, FaultKind, OpenFault};")]
            ///
            /// // Field 1 varint 1, tag padded to two bytes.
            ...
            ///
            /// let cut = vec![0x08];
            #[doc = concat!(" let Err((back, fault)) = ", stringify!($Machine), "::open(cut, DepthLimit::REFERENCE) else {")]
            ///     unreachable!()
            /// };
            /// assert!(matches!(fault, OpenFault::Wire(Fault { at: 1, kind: FaultKind::Value { .. } })));
            /// assert_eq!(back, [0x08]);
            /// ```
            pub fn open(source: $src, limit: DepthLimit) -> Result<Self, ($src, OpenFault)> {
                ...
                let mut rows = Vec::new();
                let top = match scan_layer(&mut rows, &source, 0, len, None, 0, limit) {
                    Ok(top) => top,
    ...
    (
        ...
        impl<$($lt),*> $Machine<$($lt),*> {
            ...
            /// admission: zero bytes are copied, the root layer
            /// materializes eagerly (groups included), and LEN payloads
            ...
            /// it carries padding outside the canonical-minimal policy or
            /// a root-layer container nests past `limit`.
            ///
            /// # Examples
            ///
    ...
    (
        ...
        impl<$($lt),*> $Machine<$($lt),*> {
            ...
            /// buffer moves in), the root layer materializes eagerly
            /// (groups included), and LEN payloads wait for
            #[doc = concat!(" [`", stringify!($Machine), "::descend`].")]
            ///
            /// # Errors
            ///
            ...
            /// coordinate class (`i32::MAX` bytes), [`OpenFault::Wire`]
            /// when the root layer violates the wire grammar, and
            /// [`OpenFault::Refused`] when it carries padding outside the
            /// canonical-minimal policy or a root-layer container nests
            /// past `limit`.
            ///
            /// # Examples
            ///
    ...
    (
        ...
        impl<$($lt),*> $Machine<$($lt),*> {
            ...
            $crate::editor::grouped::one_shot_machine!(@1s_transfer_readers $cap, $Machine, [$noun] [$a_noun] [$A_noun] [$doc_mod] [$doc_open] [$doc_open_empty]);
            $crate::editor::grouped::one_shot_machine!(@1s_zone_of $cap);
            ...
            /// A record's direct interior in wire order: group members,
            /// or a descended LEN's records. Empty for scalars and for
            /// containers whose interior never opened.
            ///
            /// # Panics
            ///
            #[doc = concat!(" Panics if `handle` was not minted by this ", $noun, " (the arena")]
            ...
            $crate::editor::grouped::one_shot_machine!(@1s_status $cap, $Machine, [$noun] [$a_noun] [$A_noun] [$doc_mod] [$doc_open] [$doc_open_empty]);
            ...
            /// containers have been opened: group interiors are there from
            /// the scan, an unopened or faulted LEN answers as itself, and
            /// a position on a group's framing tags names the group.
            /// Authored records have no source geometry and are never
            /// named; edit state is not consulted (a deleted record still
            /// owns its source bytes). The walk follows the materialized
            /// sibling chains, so its cost is the chain lengths along one
            /// root-to-leaf path; a resident hex view wants the session's
            /// run-table bisection instead (feature `session-*`).
            #[must_use]
            ...
            $crate::editor::grouped::one_shot_machine!(@source_spans $acc, [$noun] [$doc_mod] [$doc_open], $Machine);

            $crate::editor::grouped::one_shot_machine!(@1s_nesting $cap, $Machine, [$noun] [$a_noun] [$A_noun] [$doc_mod] [$doc_open] [$doc_open_empty]);

            /// Designates the record for cross-machine transfer: the
            /// exact source record bytes — a group's whole closure with
            /// its end tag — bound to their proved field, kind, framing
            /// geometry, and structural group depth. The designation
            /// names the original admitted occurrence — a pending value
            /// replacement does not ride, and rows without a live source
            /// occurrence (command-authored or deleted ones) refuse.
            ///
            /// # Errors
            ///
            /// [`Fault::NotSourceBacked`](crate::source::grouped::Fault::NotSourceBacked)
            ...
            ) -> Result<crate::source::grouped::RecordRef<'_>, crate::source::grouped::Fault>
            {
                ...
                if !row.has_source() || row.deleted() {
                    return Err(crate::source::grouped::Fault::NotSourceBacked);
                }
                let at = usize_of(row.start.as_inner());
                let end = usize_of(row.span_end());
                let (tag_w, delim_w) =
                    $crate::editor::grouped::one_shot_machine!(@met_geometry $acc, row);
                Ok(crate::source::grouped::RecordRef::mint(
                    ...
                    delim_w,
                    row.payload_len.as_inner(),
                    self.structural_nesting(handle.0),
                    $crate::editor::grouped::one_shot_machine!(@mint_proof $acc),
                ))
            }

            $crate::editor::grouped::one_shot_machine!(@1s_value_reads $cap, $Machine, [$noun] [$a_noun] [$A_noun] [$doc_mod] [$doc_open] [$doc_open_empty]);

            // ── descent ──

            $crate::editor::grouped::one_shot_machine!(@1s_descend $cap, $Machine, [$noun] [$a_noun] [$A_noun] [$doc_mod] [$doc_open] [$doc_open_empty]);
            ...
            $crate::editor::grouped::one_shot_machine!(@1s_set_scalar $cap, $Machine, [$noun] [$a_noun] [$A_noun] [$doc_mod] [$doc_open] [$doc_open_empty]);

            $crate::editor::grouped::one_shot_machine!(@set_varint $acc, [$noun] [$doc_mod] [$doc_open], $Machine);
            ...
            #[doc = concat!(" use ", $doc_mod, "::", stringify!($Machine), ";")]
            ///
            /// // group f2 { varint f3=1 } · varint f1=7
            /// let msg = [0x13, 0x18, 0x01, 0x14, 0x08, 0x07];
            #[doc = concat!(" let mut ", $noun, " = ", $doc_open, ".unwrap();")]
            #[doc = concat!(" let group = ", $noun, ".top().next().unwrap();")]
            #[doc = concat!(" ", $noun, ".delete(group).unwrap();")]
            #[doc = concat!(" assert_eq!(", $noun, ".save().unwrap(), [0x08, 0x07]);")]
            /// ```
            #[inline]
            #[track_caller]
            ...
            $crate::editor::grouped::one_shot_machine!(@1s_container_gate $cap, $Machine, [$noun] [$a_noun] [$A_noun] [$doc_mod] [$doc_open] [$doc_open_empty]);
            ...
            $crate::editor::grouped::one_shot_machine!(@1s_apply_insert $cap, $Machine, [$noun] [$a_noun] [$A_noun] [$doc_mod] [$doc_open] [$doc_open_empty]);

            $crate::editor::grouped::one_shot_machine!(@1s_link_row $cap, $Machine, [$noun] [$a_noun] [$A_noun] [$doc_mod] [$doc_open] [$doc_open_empty]);

            $crate::editor::grouped::one_shot_machine!(@1s_transfer_faces $cap, $Machine, [$noun] [$a_noun] [$A_noun] [$doc_mod] [$doc_open] [$doc_open_empty]);
            ...
            /// payload and for an imported group (a sealed exact-byte
            /// closure — a splice under it would vanish at save; save
            /// and reopen to edit inside it),
            /// [`EditFault::IndexSpaceExhausted`] when the row or
            ...
            #[doc = concat!(" use ", $doc_mod, "::{Descent, EditFault, InsertAt, ", stringify!($Machine), "};")]
            ///
            /// // LEN f2 wrapping { varint f1=1 }
            /// let msg = [0x12, 0x02, 0x08, 0x01];
            #[doc = concat!(" let mut ", $noun, " = ", $doc_open, ".unwrap();")]
            #[doc = concat!(" let wrapper = ", $noun, ".top().next().unwrap();")]
            /// let field = FieldNumber::new(2).unwrap();
            ///
            /// // Inserting into an undescended LEN is refused…
            /// assert_eq!(
            #[doc = concat!("     ", $noun, ".insert_varint(InsertAt::TailOf(Some(wrapper)), field, 5).err(),")]
            ///     Some(EditFault::TargetUnopened)
            /// );
            /// // …descend first, then the insert lands and the LEN prefix
            /// // recomputes at save.
            #[doc = concat!(" assert!(matches!(", $noun, ".descend(wrapper).unwrap(), Descent::Opened { .. }));")]
            #[doc = concat!(" ", $noun, ".insert_varint(InsertAt::TailOf(Some(wrapper)), field, 5).unwrap();")]
            #[doc = concat!(" assert_eq!(", $noun, ".save().unwrap(), [0x12, 0x04, 0x08, 0x01, 0x10, 0x05]);")]
            /// ```
            #[inline]
            #[track_caller]
            ...
            /// Inserts an empty group at the anchor; it grows purely by
            /// further insertion, and emits minimal open and end tags at
            /// save.
            ///
            /// # Errors
            ///
            #[doc = concat!(" As [`", stringify!($Machine), "::insert_varint`], less the store edge (a group")]
            /// stores no value).
            ///
            /// # Panics
            ///
            ...
            #[doc = concat!(" use ", $doc_mod, "::{InsertAt, ", stringify!($Machine), "};")]
            ///
            #[doc = concat!(" let mut ", $noun, " = ", $doc_open_empty, ".unwrap();")]
            /// let f1 = FieldNumber::new(1).unwrap();
            /// let f2 = FieldNumber::new(2).unwrap();
            #[doc = concat!(" let group = ", $noun, ".insert_group(InsertAt::TailOf(None), f1).unwrap();")]
            #[doc = concat!(" ", $noun, ".insert_varint(InsertAt::TailOf(Some(group)), f2, 3).unwrap();")]
            #[doc = concat!(" assert_eq!(", $noun, ".save().unwrap(), [0x0B, 0x10, 0x03, 0x0C]);")]
            /// ```
            #[inline]
            #[track_caller]
            pub fn insert_group(&mut self, at: InsertAt, field: FieldNumber) -> Result<Handle, EditFault> {
                let plan = self.resolve_anchor(at)?;
                let id = self.mint_insert()?;
                self.apply_insert(&plan, id, field, RecordKind::Group, 0);
                Ok(Handle(id))
            }

            // ── save ──

            $crate::editor::grouped::one_shot_machine!(@1s_subtree_dirt $cap, $Machine, [$noun] [$a_noun] [$A_noun] [$doc_mod] [$doc_open] [$doc_open_empty]);

            $crate::editor::grouped::one_shot_machine!(@1s_settle $cap, $Machine, [$noun] [$a_noun] [$A_noun] [$doc_mod] [$doc_open] [$doc_open_empty]);

            $crate::editor::grouped::one_shot_machine!(@1s_size_emit $cap $acc, $Machine, [$noun] [$a_noun] [$A_noun] [$doc_mod] [$doc_open] [$doc_open_empty]);
            ...
            #[doc = concat!(" ", $noun, ".set_varint(first, 7).unwrap();")]
            ///
            /// // Append semantics: the caller owns the buffer and its
            /// // prefix.
            ...
            /// containers (groups included) enclosing their interiors.
            /// One sizing pass runs first and its priced bodies feed the
            /// span walk directly, so the table prices exactly what
            #[doc = concat!(" [`", stringify!($Machine), "::save`] would produce, without emitting a byte.")]
            ///
            ...
            #[doc = concat!(" use ", $doc_mod, "::", stringify!($Machine), ";")]
            ///
            /// // group f2 { varint f3=1 } · varint f1=42; edit inside the
            /// // group.
            /// let msg = [0x13, 0x18, 0x01, 0x14, 0x08, 0x2A];
            #[doc = concat!(" let mut ", $noun, " = ", $doc_open, ".unwrap();")]
            #[doc = concat!(" let group = ", $noun, ".top().next().unwrap();")]
            #[doc = concat!(" let inner = ", $noun, ".children(group).next().unwrap();")]
            #[doc = concat!(" ", $noun, ".set_varint(inner, 300).unwrap(); // one byte grows to two")]
            ///
            #[doc = concat!(" let spans = ", $noun, ".save_spans().unwrap();")]
            /// let table: Vec<_> = spans.iter().collect();
            /// // Output: 13 18 AC 02 14 08 2A — the group encloses its kid.
            ...
            $crate::editor::grouped::one_shot_machine!(@1s_splice_spans $cap, $Machine, [$noun] [$a_noun] [$A_noun] [$doc_mod] [$doc_open] [$doc_open_empty]);

            $crate::editor::grouped::one_shot_machine!(@canonical $cap $acc, [$noun] [$doc_mod] [$doc_open], $Machine);
        }
    };
    ...
    (@canonical plain tolerant, [$noun:literal] [$doc_mod:literal] [$doc_open:literal], $Machine:ident) => {
            ...
            /// locates each value, prefix, payload, and group end tag.
            fn settle_canonical(&self, row: &Row) -> CanonicalArm {
                ...
                match row.base() {
                    Base::Intact => match row.kind {
                        ...
                        RecordKind::Group => CanonicalArm::Group {
                            head,
                            end_word: group_end_word(row.field),
                            first: row.kid,
                        },
                    },
                    // Groups are never `Replaced` (the setter's kind gate
                    // closes that path), so the group arm below serves
                    // authored groups; the dispatch stays total.
            ...
            /// clean machine still pays it in full. Group frames ride
            /// the spine without a length slot.
            ///
            /// # Errors
            ///
            ...
            fn canonical_size_pass(&self) -> Result<(u32, Vec<u32>), SaveFault> {
                ...
                loop {
                    let Some(id) = cur else {
                        ...
                        acc = frame.outer
                            + match frame.close {
                                CanonicalClose::Len { head, slot, at } => {
                                    let body = u32::try_from(acc)
                                        .ok()
                                        .and_then(PayloadLen::new)
                                        .ok_or(SaveFault::BodyOverCap { at })?;
                                ...
                                CanonicalClose::Group { head, end_word } => {
                                    u64::from(encoded_len32(head))
                                        + acc
                                        + u64::from(encoded_len32(end_word))
                                }
                    ...
                    match self.settle_canonical(row) {
                        ...
                        CanonicalArm::OpenLen { head, first, at } => {
                            let slot = bodies.len();
                            bodies.push(0);
                            spine.push(CanonicalFrame {
                                next: row.next,
                                outer: acc,
                                close: CanonicalClose::Len { head, slot, at },
                            });
                            acc = 0;
                            cur = first;
                            continue;
                        }
                        CanonicalArm::Group { head, end_word, first } => {
                            spine.push(CanonicalFrame {
                                next: row.next,
                                outer: acc,
                                close: CanonicalClose::Group { head, end_word },
                            });
                            acc = 0;
                            cur = first;
                            continue;
            ...
            /// writing into the shared emitter. Climbing out of
            /// containers follows parent links — the spine is the arena
            /// itself — and a group's minimal end tag emits at its
            /// climb-out. Returns the count of body slots consumed, for
            /// the faces' seam assertion.
            ...
            #[doc = concat!(" ", $noun, " already materialized: the root layer, every group")]
            /// interior (in-band syntax), plus each source LEN interior
            /// a successful descend committed. Every head tag, LEN
            /// prefix, varint value, and group framing word inside it
            ...
            /// order, duplicates, liveness, group nesting, and the
            /// fixed-width bits are untouched, as is every observable of
            #[doc = concat!(" this ", $noun, " — the face reads `&self`, sizes call-locally,")]
            /// and caches nothing.
            ///
            ...
            #[doc = concat!(" use ", $doc_mod, "::", stringify!($Machine), ";")]
            ///
            /// // group f1 (open tag padded to two bytes) wrapping
            /// // varint f2=1 (value padded to two bytes).
            /// let msg = [0x8B, 0x00, 0x10, 0x81, 0x00, 0x0C];
            #[doc = concat!(" let ", $noun, " = ", $doc_open, ".unwrap();")]
            ///
            /// // Fidelity keeps the padded kept words; the canonical
            /// // face re-emits the group framing and the in-band
            /// // interior minimally, without any descent.
            #[doc = concat!(" assert_eq!(", $noun, ".save().unwrap(), msg);")]
            #[doc = concat!(" assert_eq!(", $noun, ".save_canonical().unwrap(), [0x0B, 0x10, 0x01, 0x0C]);")]
            /// ```
    ...
    (@canonical transfer tolerant, [$noun:literal] [$doc_mod:literal] [$doc_open:literal], $Machine:ident) => {
            ...
            fn settle_canonical(&self, row: &Row) -> CanonicalArm {
                ...
                match row.base() {
                    ...
                    // source subspan. An imported record re-emits
                    // minimally under the canonical standard: its met
                    // framing is decoded from the slot, never preserved
                    // (the ordinary save's fidelity is the byte-exact
                    // face).
                    Base::Src => match row.src_value() {
                        // A dirty descended interior joined the
                        // closure and recurses (groups re-derive both
                        // words); a clean import re-emits through the
                        // slot normalizer.
                        SrcValue::Imported(value) => {
                            if row.opened() && row.dirty() {
                                if matches!(row.kind, RecordKind::Group) {
            ...
            /// The canonical price of one imported record: every framing
            /// word and varint value re-encodes at its value's own
            /// width, LEN interiors ride opaque, and a group's whole
            /// token stream — nested closures included — walks flat
            /// until the record's extent ends. The walk cannot refuse:
            /// imported designations are structurally complete.
            fn canonical_import_size(&self, value: PayloadAt) -> u64 {
                let bytes = self.import_slot(value);
                let mut total: u64 = 0;
                let mut pos = 0usize;
                while pos < bytes.len() {
                    let Ok((word, width)) = slice::tag_word(bytes, pos, bytes.len()) else {
                        unreachable!("imported records are structurally complete")
                    };
                    pos += usize::from(width);
                    total += u64::from(encoded_len32(word));
                    match classify(Low3::from_word(word)) {
                        TagClass::Record(kind) => match kind {
                            RecordKind::Varint => {
                                let Ok((v, w)) = slice::value64(bytes, pos, bytes.len()) else {
                                    unreachable!("imported records are structurally complete")
                                };
                                pos += usize::from(w);
                                total += u64::from(encoded_len64(v));
                            }
                            RecordKind::I32 => {
                                pos += 4;
                                total += 4;
                            ...
                            RecordKind::Len => {
                                let Ok((len, w)) = slice::len_word(bytes, pos, bytes.len()) else {
                                    unreachable!("imported records are structurally complete")
                                };
                                pos += usize::from(w) + usize_of(len.as_inner());
                                total += u64::from(encoded_len32(len.as_inner()))
                                    + u64::from(len.as_inner());
                            }
                            // The interior records follow inline.
                            RecordKind::Group => {}
                        },
                        TagClass::GroupEnd => {}
                        TagClass::Unassigned => {
                            unreachable!("imported records carry assigned codes only")
                        }
                    }
                }
                total
            ...
            /// [`Self::canonical_import_size`]'s emitting twin: the same
            /// flat token walk, re-encoding into the shared emitter.
            fn canonical_import_emit<'s, O: Out<'s>>(&'s self, value: PayloadAt, emit: &mut O) {
                let bytes = self.import_slot(value);
                let mut pos = 0usize;
                while pos < bytes.len() {
                    let Ok((word, width)) = slice::tag_word(bytes, pos, bytes.len()) else {
                    ...
                    match classify(Low3::from_word(word)) {
                        TagClass::Record(kind) => match kind {
                            ...
                            RecordKind::I32 => {
                                emit.bytes(&bytes[pos..pos + 4]);
                                pos += 4;
                            }
                            RecordKind::I64 => {
                                emit.bytes(&bytes[pos..pos + 8]);
                                pos += 8;
                            }
                            RecordKind::Len => {
                                let Ok((len, w)) = slice::len_word(bytes, pos, bytes.len()) else {
                                ...
                                let body = pos + usize::from(w);
                                let end = body + usize_of(len.as_inner());
                                emit.varint(u64::from(len.as_inner()));
                                emit.bytes(&bytes[body..end]);
                                pos = end;
                            }
                            // The interior records follow inline.
                            RecordKind::Group => {}
            ...
            fn canonical_size_pass(&self) -> Result<(u32, Vec<u32>), SaveFault> {
                ...
                loop {
                    ...
                    match self.settle_canonical(row) {
                        ...
                        CanonicalArm::Import { value } => acc += self.canonical_import_size(value),
                    }
                    cur = row.next;
                }
                let total = u32::try_from(acc)
            ...
            fn canonical_emit_pass<'s, O: Out<'s>>(&'s self, emit: &mut O, bodies: &[u32]) -> usize {
                ...
                loop {
                    ...
                    match self.settle_canonical(row) {
                        ...
                        CanonicalArm::Import { value } => self.canonical_import_emit(value, emit),
                    }
                    cur = row.next;
                }
                emit.flush();
    ...
    (@canonical $cap:ident tolerant, [$noun:literal] [$doc_mod:literal] [$doc_open:literal], $Machine:ident) => {
            ...
            fn settle_canonical(&self, row: &Row) -> CanonicalArm {
                ...
                match row.base() {
                    ...
                    // face).
                    Base::Src => match row.src_value() {
                        SrcValue::Imported(value) => CanonicalArm::Import { value },
                        SrcValue::PayloadRe(_) | SrcValue::PayloadNew => {
                            let (at, len) = self.designated_span(row);
                            CanonicalArm::OpaqueLen {
                                head,
            ...
            /// [`Self::canonical_import_size`]'s emitting twin: the same
            /// flat token walk, re-encoding into the shared emitter.
            fn canonical_import_emit<O: Out>(&self, value: PayloadAt, emit: &mut O) {
                let bytes = self.import_slot(value);
                let mut pos = 0usize;
                while pos < bytes.len() {
                    let Ok((word, width)) = slice::tag_word(bytes, pos, bytes.len()) else {
    ...
    (
        ...
        impl<$($lt),*> $Machine<$($lt),*> {
            $crate::editor::grouped::one_shot_machine!(@import_borrowed $cap $acc $Machine, payload: $p, [$noun]);
            $crate::editor::grouped::one_shot_machine!(@import_copied $cap $acc $Machine, name: copy_record_from_copy, doc: " twin: the record bytes are staged by copy at the command, for designations that cannot outlive the call", [$noun]);

            $crate::editor::grouped::one_shot_machine!(@1s_pb_mixed $cap, $Machine, [$noun] [$a_noun] [$A_noun] [$doc_mod] [$doc_open] [$doc_open_empty]);

            $crate::editor::grouped::one_shot_machine!(@1s_sp_mixed $cap, payload: $p, $Machine, [$noun] [$a_noun] [$A_noun] [$doc_mod] [$doc_open] [$doc_open_empty]);

            $crate::editor::grouped::one_shot_machine!(@1s_spc_mixed $cap, payload: $p, $Machine, [$noun] [$a_noun] [$A_noun] [$doc_mod] [$doc_open] [$doc_open_empty]);

            $crate::editor::grouped::one_shot_machine!(@1s_spp_mixed $cap, payload: $p, $Machine, [$noun] [$a_noun] [$A_noun] [$doc_mod] [$doc_open] [$doc_open_empty]);
    ...
    (
        ...
        impl<$($lt),*> $Machine<$($lt),*> {
            ...
            $crate::editor::grouped::one_shot_machine!(@1s_pb_borrowed $cap, $Machine, [$noun] [$a_noun] [$A_noun] [$doc_mod] [$doc_open] [$doc_open_empty]);


            $crate::editor::grouped::one_shot_machine!(@1s_sp_borrowed $cap, mixed: $Mixed, payload: $p, $Machine, [$noun] [$a_noun] [$A_noun] [$doc_mod] [$doc_open] [$doc_open_empty]);


            $crate::editor::grouped::one_shot_machine!(@1s_spp_borrowed $cap, payload: $p, $Machine, [$noun] [$a_noun] [$A_noun] [$doc_mod] [$doc_open] [$doc_open_empty]);
    ...
    (
        ...
        impl<$($lt),*> $Machine<$($lt),*> {
            $crate::editor::grouped::one_shot_machine!(@import_copied $cap $acc $Machine, name: copy_record_from, doc: ": the record bytes are staged by copy at the command, so no designation lifetime binds the caller", [$noun]);

            $crate::editor::grouped::one_shot_machine!(@1s_pb_copied $cap, $Machine, [$noun] [$a_noun] [$A_noun] [$doc_mod] [$doc_open] [$doc_open_empty]);

            $crate::editor::grouped::one_shot_machine!(@1s_sp_copied $cap, mixed: $Mixed, $Machine, [$noun] [$a_noun] [$A_noun] [$doc_mod] [$doc_open] [$doc_open_empty]);
    ...
    (
        @import_borrowed $cap:ident tolerant $Machine:ident, payload: $p:lifetime, [$noun:literal]
            ...
            /// designation's exact bytes — met tag spelling, framing
            /// widths, a group's whole closure and end tag — contribute
            /// whole at save, nothing re-encoded. The record bytes stay
            /// borrowed until the save copies them once into the output,
            /// so the designation's backing must outlive this machine's
            /// payload tenure. The imported record is output-authored:
            /// its status reads `Inserted`, it answers no source span,
            /// and it does not designate onward; a LEN interior is the
            /// source's opaque declaration, and a group closure is a
            /// sealed exact-byte declaration — no splice or walk lands
            /// inside it (save and reopen to edit there).
            ///
            /// # Errors
            ///
            /// [`EditFault::DepthExceeded`] when the enclosing
            /// destination containers plus the designation's group depth
            /// pass the declared bound,
            ...
            pub fn copy_record_from(
                &mut self,
                source: crate::source::grouped::RecordRef<$p>,
                at: InsertAt,
            ) -> Result<Handle, EditFault> {
                let plan = self.resolve_anchor(at)?;
                self.transfer_depth_gate(plan.parent, source.group_depth())?;
                let id = self.mint_insert()?;
                let value = self
                    .payloads
    ...
    (
        @import_borrowed $cap:ident canonical $Machine:ident, payload: $p:lifetime, [$noun:literal]
            ...
            /// Copies one designated external record to the anchor: the
            /// designation's exact bytes — a group's whole closure and
            /// end tag included — contribute whole at save, nothing
            /// re-encoded. This host admits canonically, so the argument
            /// is the proof-carrying form — a tolerant designation
            /// upgrades through `try_canonical`, which refuses padded
            /// framing anywhere in the closure before any mutation. The
            /// record bytes stay borrowed until the save copies them
            /// once into the output, so the designation's backing must
            /// outlive this machine's payload tenure. The imported
            /// record is output-authored: its status reads `Inserted`,
            /// it answers no source span, and it does not designate
            /// onward; a LEN interior is the source's opaque
            /// declaration, and a group closure is a sealed exact-byte
            /// declaration — no splice or walk lands inside it (save
            /// and reopen to edit there).
            ///
            /// # Errors
            ///
            /// [`EditFault::DepthExceeded`] when the enclosing
            ...
            pub fn copy_record_from(
                &mut self,
                source: crate::source::grouped::CanonicalRecordRef<$p>,
                at: InsertAt,
            ) -> Result<Handle, EditFault> {
                let plan = self.resolve_anchor(at)?;
                self.transfer_depth_gate(plan.parent, source.group_depth())?;
    ...
    (
        @import_copied $cap:ident tolerant $Machine:ident, name: $name:ident, doc: $doc:literal, [$noun:literal]
            ...
            #[doc = concat!($doc, ". The designation's exact bytes — met tag spelling,")]
            /// framing widths, a group's whole closure and end tag —
            ...
            /// designate onward; a group closure is a sealed exact-byte
            /// declaration — no splice or walk lands inside it (save
            /// and reopen to edit there).
            ///
            /// # Errors
            ...
            pub fn $name(
                &mut self,
                source: crate::source::grouped::RecordRef<'_>,
                at: InsertAt,
            ) -> Result<Handle, EditFault> {
                let plan = self.resolve_anchor(at)?;
                self.transfer_depth_gate(plan.parent, source.group_depth())?;
    ...
    (
        @import_copied $cap:ident canonical $Machine:ident, name: $name:ident, doc: $doc:literal, [$noun:literal]
            ...
            /// anywhere in the closure before any mutation. The
            ...
            /// no source span, and it does not designate onward; a group
            /// closure is a sealed exact-byte declaration — no splice or
            /// walk lands inside it (save and reopen to edit there).
            ///
            /// # Errors
            ///
            /// [`EditFault::DepthExceeded`] when the enclosing
            ...
            pub fn $name(
                &mut self,
                source: crate::source::grouped::CanonicalRecordRef<'_>,
                at: InsertAt,
            ) -> Result<Handle, EditFault> {
                let plan = self.resolve_anchor(at)?;
                self.transfer_depth_gate(plan.parent, source.group_depth())?;
    ...
    (
        ...
        impl<$($lt),*> $Frame<'_ $(, $lt)*> {
            ...
            $crate::editor::grouped::one_shot_machine!(@1s_frame_apply $cap, $Machine, [$noun] [$a_noun] [$A_noun] [$doc_mod] [$doc_open] [$doc_open_empty]);
        }
