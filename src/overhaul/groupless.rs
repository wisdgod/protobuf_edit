//! The groupless overhaul: group codes are a capability refusal.
//!
//! Groupless toolchains overhaul their own traffic; a group code
//! (3 or 4) is well-formed wire *outside this language* — refusing
//! it is this dialect's correctness feature, typed distinctly from
//! the format's unassigned codes. The open walk scans the top
//! layer alone — every LEN payload is an opaque declaration until
//! a descend commits to it — and zero source bytes are retained:
//! payloads are skipped through the supply's own seek, and every
...
//! branch-free formula.
//!
//! Coordinates: write · sequential-repeatable · offline · groupless · tolerant (type-level) · commit-only.
//!
//! # Examples
//!
//! ```
//! use protobuf_edit::DepthLimit;
//! use protobuf_edit::overhaul::groupless::Overhaul;
...
/// Group grammar variants do not exist in this dialect's
/// vocabulary — not unreachable, absent.
#[non_exhaustive]
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
pub enum FaultKind {
...
/// `None`. Widths are stored input facts: padding is accepted and
/// span arithmetic must reproduce it byte-exactly. Authored rows
/// carry no source geometry; their emission is minimal by
/// construction.
///
...
struct Row {
    ...
    /// encoded width; I32/I64: 4/8. Authored rows: zero.
    payload_len: u64,
    ...
    /// never read).
    tag_width: WordWidth,
    /// LEN: the length prefix's actual width. Scalars and
    /// authored rows: `None`.
...
macro_rules! overhaul_machine {
    ...
    /// Opens the editor: one walk scans the top layer — LEN
    /// payloads stay opaque declarations, skipped through the
    /// supply's own seek — and measures the source's total
    /// length. An unlawful root layer refuses whole; the source
    /// rides back beside the mark.
    ///
    /// `depth` bounds container nesting for the descend walks,
    /// with no default.
    ///
    /// # Errors
    ///
    /// `(source, OpenFault)` — the source beside the mark — when
    /// the root layer is unlawful, the supply refuses mid-walk,
    /// the record count would leave the row-index class, or the
    /// offset would leave the coordinate space.
    ...
    /// caller's target fault; `Ok(false)` means a verdict is
    /// already resident (or the extent is empty and opens
    /// walk-free); `Ok(true)` means a walk is owed.
    ...
    /// resident: a wire fault, the dialect capability, or the
    /// declared depth bound parks on the record and projects
    /// unchanged on every later call, costing no further walk.
    /// One walk per fresh verdict (an empty payload opens
    /// walk-free).
    ///
    ...
    /// Resolves several unopened LEN handles in one source-ordered
    /// walk — the batch face that makes k scattered descents cost
    /// one walk instead of k. Handles already carrying a resident
    /// verdict are skipped (idempotent); depth refusals park
    /// walk-free; each extent commits atomically, and a wire
    /// refusal inside one extent parks on that handle while the
    /// walk continues to the next.
    ///
    /// # Errors
    ///
    ...
    pub fn materialize(
        ...
        // Walk-free settlements first: resident verdicts, empty
        // extents, depth refusals.
    ...
    /// values are row-resident), [`FetchFault::Oversize`] for an
    /// extent past the address space, [`FetchFault::Torn`] when
    ...
    fn size_pass<E>(&self) -> Result<(u64, Vec<u64>), SaveFault<E>> {
        struct Frame {
            ...
            slot: usize,
            at: u64,
            /// The source tag's met width, moved from the row.
            tag_w: WordWidth,
            /// The source prefix's met width, moved from the row.
            prefix_src_w: WordWidth,
            src_len: u64,
        }
        ...
        loop {
            let Some(id) = cur else {
                ...
                let prefix = if body == frame.src_len {
                    u64::from(frame.prefix_src_w.w())
                } else {
                    u64::from(encoded_len64(body))
                };
                acc = frame.outer + u64::from(frame.tag_w.w()) + prefix + body;
                cur = frame.next;
                continue;
            };
            let row = self.row(id);
    ...
    fn compile<E>(&self) -> Result<Script<'a>, SaveFault<E>> {
        ...
        loop {
            let Some(id) = cur else {
                ...
                // The pop epilogue: an opened LEN settles its
                // prefix slot against the booked interior.
...
/// The minimal head word of an authored record.
...
/// returning the chain's first row.
///
