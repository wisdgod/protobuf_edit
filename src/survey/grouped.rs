//! The grouped survey: the full six-code wire language, groups
//! walked structurally.
//!
//! Groups are containers framed by tags — no length prefix
//! declares their extent, so the walk measures it: a group's
//! interior span is known at its end tag, never before. One index
//! walk builds a preorder row table over all six codes; zero
//! source bytes are retained (opaque LEN payloads are skipped
//! through the supply's own seek; group interiors are parsed —
//! they cannot be skipped, their extent is unmeasured until their
//! end tag). Every scalar word is banked in its row as the walk
//! decodes it to step.
//!
//! Partition theorem: a record's bytes are `tag ⊎ delim ⊎
//! payload`, order kind-dependent — LEN's delimiter (the length
//! prefix) precedes its payload, a group's (the end tag) follows
//! it — so the record span is one formula for all kinds while the
//! payload's position dispatches on kind.
//!
//! Coordinates: read · sequential-repeatable · offline · grouped · Standard (value-level).
//!
//! # Examples
//!
//! ```
...
//! use protobuf_edit::survey::grouped::Survey;
//! use protobuf_edit::wire::grouped::RecordKind;
//!
//! // group f1 { varint f2=7 } — legacy framing, walked whole.
//! let msg = [0x0B, 0x10, 0x07, 0x0C];
//! let tree =
//!     Survey::open(SliceSource::new(&msg), DepthLimit::REFERENCE, &mut NoAdvice).unwrap();
//! assert!(tree.is_complete());
//!
//! let group = tree.top().next().unwrap();
//! assert_eq!(tree.kind(group), RecordKind::Group);
//! let inner = tree.children(group).next().unwrap();
//! assert_eq!(tree.varint_word(inner), Some(7));
//! ```
...
/// it.
///
/// Partition theorem (every span method cites it): a record's
/// bytes are `tag ⊎ delim ⊎ payload`, pairwise disjoint, union
/// the whole record, order kind-dependent — so the record span is
/// one formula for all kinds (the group end tag commutes into the
/// sum), while the payload's *position* dispatches on kind.
/// Widths are stored input facts: padding is accepted and span
/// arithmetic must reproduce it byte-exactly.
///
/// The word column banks every scalar's decoded value; container
/// rows leave it zero.
#[derive(Clone, Copy)]
struct Row {
    ...
    /// width; I32/I64: 4/8; Group: measured interior span.
    payload_len: u64,
    /// The scalar's decoded word (zero for container rows).
    word: u64,
    ...
    /// prefix. Group: the end tag, recorded at close — `None` for
    /// clipped groups whose end tag never appeared. Scalars:
    /// `None` (self-delimiting payloads).
...
impl Row {
    ...
    const fn payload_span(&self) -> SourceSpan {
        let start = match self.kind {
            RecordKind::Len => self.start + self.tag_w() + self.delim_w(),
            _ => self.start + self.tag_w(),
        };
...
/// record's kind do not exist in the type, and each variant's
/// segments partition the record's span exactly. A clipped group
/// (fault boundary before its end tag) is its own variant.
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
...
impl<S: StableReplaySource> Survey<S> {
    ...
    /// mark.
    ///
    /// `advice` is consulted at every LEN record head (groups are
    /// structural containers, walked without consultation);
    ...
    /// and a canonical one judges every varint word (group end
    /// tags included) against its minimal encoding.
    ///
    /// # Errors
    ///
    /// As [`Survey::open`].
    ...
    /// are disjoint (seals and group framing forbid partial
    /// overlap): binary search for the last start at or before
    /// `pos`, then walk the parent chain to the first containing
    /// span.
    #[inline]
    #[must_use]
    #[allow(
    ...
    /// The whole-record span (head tag through the record's last
    /// byte — a closed group's end tag included). A clipped LEN
    /// keeps its declared, sealed span; a clipped group ends at
    /// the cut.
    ///
    /// # Panics
    ///
    ...
    pub fn source_spans(&self, id: NodeId) -> RecordSpans {
        ...
        match r.kind {
            ...
            RecordKind::Group => {
                let interior = SourceSpan::new(tag.end(), tag.end() + r.payload_len);
                r.delim_width.map_or(RecordSpans::ClippedGroup { tag, interior }, |end| {
                    RecordSpans::Group {
                        tag,
                        interior,
                        end_tag: SourceSpan::new(
                            interior.end(),
                            interior.end() + u64::from(end.as_inner()),
                        ),
                    }
                })
            }
    ...
    /// Reads the record's payload bytes (a group's interior) into
    /// `out` (appended; the buffer truncates back to its entry
    /// length on any refusal — never poisoned, a retry is
    /// lawful). One fetch walk, verifying only that the source
    /// still reaches the extent's end: bytes that moved beneath
    /// unchanged coordinates are appended as they now read (the
    ...
    /// views, in order — the unbounded-extent face (a grouped
    /// interior is measured, not declared, and may exceed the
    /// address space). One fetch walk, verifying only that the
    ...
    /// walk for all of them. Nested and overlapping extents are
    /// lawful: a byte covered by several requests is handed to
    /// each. The walk verifies only that the source still reaches
    /// each extent's end: bytes that moved beneath unchanged
...
/// Open-container frame kind. Which close law applies at the
/// extent end: a LEN closes clean, a group there is unterminated.
#[derive(Clone, Copy)]
enum FrameKind {
    Group,
    Len,
...
/// One open container. The frame simultaneously carries the
/// parent link (`row`), the open field, the close law, and the
...
struct Frame {
    ...
    /// lends to [`Ancestry`]).
    field: FieldNumber,
    /// The enclosing extent's end, restored on close (a LEN seals
    /// a new zone; a group saves the value it inherits
    /// unchanged).
    prev_zone: u64,
...
struct Machine<'v, W: ReplayWalk, A: Advisor> {
    ...
    /// open and close for an O(1) dispose. `Commit` frames and
    /// groups are not absorbing; the root is implicitly
    /// committed.
...
impl<W: ReplayWalk, A: Advisor> Machine<'_, W, A> {
    ...
    /// Pushes an open container and saves the live registers in
    /// it.
    fn open(
        &mut self,
        row: NodeId,
    ...
    fn clip_group_row(&mut self, row: NodeId, cut: u64) {
        let idx = row.index();
        let descendants = (self.rows.len() - 1 - idx) as u32;
        // SAFETY: frame rows are in-table (as `seal_descendants`).
        let r = unsafe { self.rows.get_unchecked_mut(idx) };
        let body_start = r.start + u64::from(r.tag_width.as_inner());
        r.payload_len = cut - body_start;
    ...
    /// `descendants` and group end facts are written only at
    /// close, so no written state needs repair.
    ...
    /// Groups declare no extent and are never the target — a
    /// group-only stack falls back (its innermost group is
    /// unclosed at the cut). `overrun_row` names a LEN row that is
    /// not (or no longer) on the stack — a skipped opaque LEN, a
    /// demoted absorber — the innermost fallback candidate.
    #[cold]
    #[allow(
        clippy::as_conversions,
    ...
    fn eof_clip(&mut self, overrun_row: Option<NodeId>, fallback: Failure) -> Failure {
        let outer_len = self
            .stack
            .iter()
            .find(|frame| matches!(frame.kind, FrameKind::Len))
            .map(|frame| frame.row);
        let Some(target) = outer_len.or(overrun_row) else {
            return fallback;
        };
        ...
        // SAFETY: the row is a LEN (LEN frames and the named
        // fallbacks alone are offered), so its payload length was
        // minted from a `PayloadLen` and narrows back into the
        // class.
        let declared = unsafe { PayloadLen::new_unchecked(row.payload_len as u32) };
        self.rows.truncate(target.index());
        // Group frames enclosing the dropped LEN survive on the
        // stack; the final clip closes them at the cut.
        while let Some(frame) = self.stack.last() {
            if frame.row.index() >= target.index() {
                self.stack.pop();
            } else {
                break;
            }
    ...
    fn close_frame(&mut self, frame: Frame) -> Result<(), Stop<W::Error>> {
        match frame.kind {
            FrameKind::Len => {
    ...
    /// test away; the canonical one judges each varint word
    /// between its read and its classification (so a padded
    /// group, end, or zero-field tag is a width refusal first).
    fn record<const MINIMAL: bool>(&mut self) -> Result<Flow, Stop<W::Error>> {
        ...
        let (word, tag_width) = match self.pump.step_tag(std) {
            ...
            StepRead::End => {
                // Clean only at the root: an open LEN's declared
                // extent is the lie the resident parse would have
                // refused at its open; a group-only stack is the
                // innermost group unclosed at the cut.
                ...
                let fallback = Failure {
                    fault: Fault {
                        at: self.pump.off,
                        kind: FaultKind::GroupUnclosed { open: frame.field },
                    },
                    cut: self.pump.off,
                };
    ...
    /// A group opens: structural descent — no advisor question
    /// exists (the interior must be parsed to find the end tag),
    /// only the depth bound judges.
    fn group_open(
        &mut self,
        at: u64,
        field: FieldNumber,
        ...
        let row = self.push_row(at, field, RecordKind::Group, tag_width, 0, None, 0)?;
        // A group inherits its opener's zone: it saves and
        // restores the same value.
        self.open(row, field, FrameKind::Group, self.pump.zone, false);
        Ok(())
    }

    /// The end tag must match the innermost open frame, and that
    /// frame must be a group (a group never spans a LEN
    /// boundary). `at` is the end tag's first byte; the pump
    /// already stands past the tag.
    fn group_close(
        &mut self,
        at: u64,
        found: FieldNumber,
    ...
    /// with the same close-time writes as a normal pop, so the
    /// row table leaves the machine already satisfying every row
    /// invariant. Groups clip to the boundary (their end tag
    /// never appeared); LEN rows keep their declared spans (the
    /// seal is an input fact independent of walk progress).
    fn clip(mut self, failure: Failure) -> Parts {
        ...
        Parts { rows: self.rows.into_boxed_slice(), indexed_end: cut, fault: Some(failure.fault) }
    }
