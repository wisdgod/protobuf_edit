//! The shared commission layer: the coordinate classes, the edit
//! algebra with its revision log, the value stores, and the layer
//! plumbing both dialect commission editors build on.
//!
//! A commission editor is the editing review's stable-replay
//! twin: the same command set, revision log, canonical-minimal
//! admission, and two-pass save, over a source that is walked
//! from byte zero as many times as asked instead of borrowed as a
//! slice. A padded tag, length prefix, or varint value — in the
//! grouped dialect, a padded group end tag — is lawful tolerant
//! wire refused by this machine's standard: at the root the open
//! refuses whole (the source rides back beside the mark), inside
//! a payload the refusal parks as a resident verdict. Admission
//! proved met ≡ minimal, so the rows store no framing widths —
//! every window derives from the record's own facts — every
//! scanned scalar's decoded word is banked at the scan, and
//! untouched records ride saves byte-exactly, which under this
//! door is already minimal emission. Reverting every command
//! restores the source reading exactly, with zero walks: the
//! banked words re-speak the scanned reading.
//!
...
//! abort: a commission editor carries revisable interactive state
...
//! speculates.
//!
//! Output acceptance: admission proved every scanned framing word
//! minimal and authored words emit minimal, so
//! `save`/`save_into`/`save_sink` guarantee `CanonicalMinimal`
//! outright — no separate canonical-output family exists, exactly
//! as on the buffered review twin — with the one caller-declared
//! exception: an authored payload's interior passes through
//! unchanged.
//!
//! Coordinates: write · sequential-repeatable · offline · canonical (type-level) · revisable.
//!
//! # Choosing a face
//!
//! - Opening: `Commission::open` takes the source and scans its
//!   top layer in one walk — LEN payloads stay opaque
//!   declarations, skipped through the supply's own seek, and a
//!   refusal (padding included) returns the source beside the
//!   mark. No depth argument: descent is caller-stepped and every
//!   scan is iterative. The canonical commit-only replay editor
//!   is `refit`; the buffered revisable twin over a borrowed
//!   slice is `review`.
//! - Commands: `set_varint`/`set_i32`/`set_i64`/`set_payload`
//!   replace values; `insert_varint`/`insert_i32`/`insert_i64`/
//!   `insert_payload` (the grouped commission adds `insert_group`)
//!   author records; `delete` shrouds and `undelete` restores
//!   exactly; `clear_edit` clears a replacement back to the
//!   scanned state. All walk-free.
...
//!   and the resident scan judges the same canonical standard);
...
//!   identity supply. Each byte-producing save is one walk, and
//!   every output closes under `CanonicalMinimal`.
//! - Payload backing, by type: `Commission` copies payloads at
//!   the command — temporaries welcome, no payload lifetime on
//!   the type. Its sibling `BorrowCommission<'p, S>` retains
//!   borrowed slices instead — no staging copy, and every payload
//!   owner outlives the machine — and `MixCommission<'p, S>`
//!   selects the backing per install; the recipes below work the
//!   borrowed and mixed profiles.
//! - Hex-view supply: `span`/`source_spans` give record geometry
//!   (whole-source `u64` coordinates, every width the minimal
//!   theorem), and `narrowest` answers "which record covers this
//!   byte" — both walk-free.
//!
//! Both dialect commission editors ship the same faces; the crate
...
//! tolerant replay editor (`maintain`) accepts:
//!
//! ```
//! # #[cfg(feature = "commission-groupless")] {
//! use protobuf_edit::commission::groupless::{Commission, FaultKind, OpenFault};
...
//! use protobuf_edit::commission::InsertAt;
//! use protobuf_edit::commission::groupless::Commission;
//! use protobuf_edit::replay_source::SliceSource;
//!
//! let msg = [0x08, 0x2A];
...
//! ```
//! # #[cfg(feature = "commission-groupless")] {
//! use protobuf_edit::commission::groupless::BorrowCommission;
//! use protobuf_edit::replay_source::SliceSource;
//!
//! let template = vec![0x08, 0x2A];
//! // LEN f2 "a".
//! let source = [0x12, 0x01, 0x61];
//! let mut editor = BorrowCommission::open(SliceSource::new(&source)).unwrap();
//! let record = editor.top().next().unwrap();
...
(the copy-only `Commission` is the escape hatch for temporaries):
...
//!
//! The mixed-backing profile: `MixCommission` selects the backing
...
//! ```
//! # #[cfg(feature = "commission-groupless")] {
//! use protobuf_edit::commission::groupless::MixCommission;
//! use protobuf_edit::replay_source::SliceSource;
//!
//! let template = vec![0x08, 0x2A];
//! let source = [0x12, 0x01, 0x61];
//! let mut editor = MixCommission::open(SliceSource::new(&source)).unwrap();
//! let record = editor.top().next().unwrap();
//! editor.set_payload(record, &template).unwrap();
//! {
//!     let transient = vec![0x08, 0x07];
...
/// A commission editor's name for one record row.
///
...
/// A subtree aggregate the commission editor maintains. Both marks
