//! The groupless router (groupless wire traffic).
//!
...
//! container stack is uniformly committed-LEN frames: the endpoint
//! cascade cannot fault, and the sink has no group events to
//! invite.
//!
//! Coordinates: read · stream · static · groupless · Standard (value-level).
//!
//! # Examples
//!
//! A LEN both targeted and routed through — the tap-and-commit
//! arm: its body streams as segments while the interior's own
//! matches interleave, each construct's bytes ahead of that
//! construct's events.
//!
//! ```
...
//!         value: u64,
//!     ) -> ControlFlow<()> {
//!         self.values.push((path.index(), value));
//!         ControlFlow::Continue(())
//!     }
//! }
//!
//! // Path 0 taps field 3 itself; path 1 continues through it.
...
//! router.feed(&msg, &mut tape).unwrap();
//! router.finish().unwrap();
//! assert_eq!(tape.body, [0x08, 0x01]);
//! assert_eq!(tape.values, [(1, 1), (1, 42)]);
//! ```
...
/// (with an empty program, the wire-level validator itself).
///
...
/// and tile its body exactly in source order.
pub trait Sink {
...
/// The groupless router's refusal classes, sectioned by
/// [`FaultClass`] (grammar sites, then policy, then capability);
/// [`class`](Self::class) answers the section.
///
/// The set is the groupless scanner's — the machines drive one
/// pump and one LEN admission — spelled as this module's own type
/// (scenario modules share no public types).
///
...
pub enum FaultKind {
    ...
    /// Committing this container would exceed the caller's declared
    /// [`DepthLimit`] bound.
    DepthExceeded {
    ...
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match *self {
            ...
            Self::DepthExceeded { field } => write!(
                f,
                "committing the container of field {} would exceed the bound",
                field.as_inner()
            ),
...
/// live one rides the cursor), the field for exit events, and —
/// for a tapped container — the open-tap mark its exit consumes.
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
struct LenFrame {
    prev_zone: u64,
    field: FieldNumber,
...
/// The one-pass groupless routing machine.
///
...
impl<'r> Router<'r> {
    ...
    /// let program = Program::over(&paths).unwrap();
    /// let mut router = Router::new(&program, Standard::Tolerant, DepthLimit::REFERENCE);
    /// let flow = router.feed(&[0x08, 0x2A, 0x10, 0x07], &mut First).unwrap();
    ...
    /// truncation fault; a still-open committed LEN is
    ...
    /// Resolves every sealed endpoint at the cursor: pops the
    /// frame, restores the shadowed zone, closes the matcher
    /// layer, and — for a tapped frame — closes its tap with the
    /// exit events. Uniform LEN frames: this pop path cannot
    /// fault.
    fn cascade<S: Sink>(&mut self, sink: &mut S) -> ControlFlow<Result<Flow, Fault>> {
        while self.pump.off == self.pump.zone {
            ...
            // `off < u64::MAX` through every consuming path — the
            // cursor can never equal the root sentinel.
            let frame = unsafe { self.stack.pop().unwrap_unchecked() };
            self.pump.zone = frame.prev_zone;
            self.matcher.exit();
            if let Some(open) = frame.tap
    ...
    fn len_word<S: Sink, const MINIMAL: bool>(
        ...
        if routed {
            ...
            self.stack.push(LenFrame { prev_zone: self.pump.zone, field, tap });
            self.pump.zone = end;
    ...
    fn take_tap(&mut self, _open: OpenTap) -> Tap {
        ...
        // one closed with its own frame or counted mode before this
        // close could run. No path abandons a mark over a live
        // machine — a sink callback that unwinds mid-feed leaves
        // the terminal latch armed, so no later feed can reach a
        // close site over a half-stepped stack.
        unsafe { self.taps.pop().unwrap_unchecked() }
    }

    /// Closes the innermost tap: exit events for its paths
    /// (ascending — the arena kept visit order), then the frame
    /// and its ids retire, consuming the open mark.
