//! The grouped stream-ingest draft: chunked input parsed as it
...
//! publishes provisionally at its open tag with its own final
//! layer descriptor (following records name it as parent and
//! chain in that layer), and is patched at the verified close with
//! its record end and actual end-tag width, so group state survives
//! chunk edges as row-arena and layer coordinates, never a second
//! stack. No depth bound exists here or on the finished draft —
//! the buffered twin's own law: nesting is bounded by the
//! coordinate class and the fallible growth edges. An orphan or
...
//! `draft-groupless`).
//!
//! Coordinates: write · stream · offline · grouped · tolerant (type-level) · owned · revisable.
//!
//! # Examples
//!
//! ```
...
//! assert_eq!(draft.save().unwrap(), [0x08, 0x96, 0x81, 0x00, 0x13, 0x18, 0x07, 0x14]);
//!
...
crate::revise::grouped::revising_machine! {
    ...
    a_noun: "a draft",
    A_noun: "A draft",
    door: "{ let mut ingest = protobuf_edit::stream_draft::grouped::Ingest::new(); ingest.feed(&msg).unwrap(); ingest.finish() }",
    doc_mod: "protobuf_edit::stream_draft::grouped",
}
...
crate::revise::grouped::revising_machine! {
    ...
    #[doc = " `transfer-stream-draft-grouped`) seal through"]
...
crate::revise::grouped::revising_machine! {
    ...
    a_noun: "a draft",
    A_noun: "A draft",
    door: "{ let mut ingest = protobuf_edit::stream_draft::grouped::Ingest::new(); ingest.feed(&msg).unwrap(); ingest.finish_borrow() }",
    doc_mod: "protobuf_edit::stream_draft::grouped",
}
...
pub enum ResourceSite {
    ...
    Rows,
    /// The layer table, at a group's provisional publish.
    Layers,
...
///
/// The wire alphabet is the buffered twin's own ([`Fault`]), so a
/// fault here names the same construct, with the same buffered
/// coordinates, that opening the concatenated bytes would name.
/// The coordinate-class and resource refusals are this cell's own:
/// the stream is admitted against the finished editor's source cap
/// before bytes are read, and every growth edge is fallible.
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
...
/// rides inside [`IngestFaultKind::Wire`], so both conventions
        ...
        match self.kind {
            ...
            IngestFaultKind::Resource(site) => {
                let site = match site {
                    ...
                    ResourceSite::Layers => "layer table",
...
struct IngestCore {
    source: Vec<u8>,
    rows: Vec<Row>,
    /// Interior-layer descriptors, one minted per group open.
    layers: Vec<Layer>,
...
impl IngestCore {
    ...
    /// a fallible edge inside the drive loop — and answers its
    /// coordinate.
    fn publish(&mut self, row: Row) -> Result<RowId, IngestFault> {
        ...
        match self.last {
            ...
            None => match self.open {
                Some(group) => {
                    self.layers[group_layer(&self.rows, group).index()].first = Some(id);
                }
    ...
    fn head(&mut self, rest: &mut &[u8]) -> Result<(), IngestFault> {
        ...
        match kind {
            ...
            RecordKind::Group => {
                if self.layers.try_reserve(1).is_err() {
                    return Err(IngestFault {
                        at: self.offset(),
                        kind: IngestFaultKind::Resource(ResourceSite::Layers),
                    });
                }
                let Some(layer) = u32::try_from(self.layers.len()).ok().and_then(LayerId::new)
                else {
                    // Every group spends at least two source bytes
                    // and feed admission bounds the stream at
                    // `i32::MAX` bytes, so the layer table cannot
                    // leave its domain.
                    debug_assert!(false, "root ingest exhausted the layer domain");
                    // SAFETY: the layer-count bound argued above.
                    unsafe { core::hint::unreachable_unchecked() }
                };
                // The provisional publish: a group row remembers its
                // own tag offset in `end` until the verified close,
                // and owns its final layer descriptor from birth.
                ...
                // The reservation above covers this push.
                self.layers.push(Layer::EMPTY);
                // SAFETY: `id` was just published.
                unsafe { self.rows.get_unchecked_mut(id.index()) }.set_slot(Slot::Opened(layer));
                self.open = Some(id);
                self.last = None;
            }
        }
    ...
    /// row's record end and actual end-tag width, seal its layer's
    /// tail anchor, and pop the open-group chain by one parent link.
    fn close_group(
        &mut self,
        start: u32,
        width: WordWidth,
        ...
        row.end = start + width.w();
        row.delim_width = Some(width);
        self.open = row.parent;
        self.layers[group_layer(&self.rows, group).index()].last = self.last;
        ...
        match self.phase {
            Phase::Head => {
                if self.carry.width == 0 {
                    ...
                    // refuse the seal. The provisional row's `end`
                    // still holds its own tag offset.
                    let group = self.open?;
...
impl Ingest {
    ...
    /// Wire faults ([`IngestFaultKind::Wire`])
    /// and a refused row or layer publish
