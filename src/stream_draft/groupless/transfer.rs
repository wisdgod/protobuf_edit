//! The groupless stream-ingest draft's transfer siblings.
//!
...
//! intact, the state both edit algebras share), and the accumulated
//! source moves without a copy. [`Ingest::finish_transfer_borrow`]
//! seals the same parts into the borrowed-payload sibling.
//!
...
crate::revise::groupless::revising_machine! {
    ...
    a_noun: "a draft",
    A_noun: "A draft",
    door: "{ let mut ingest = protobuf_edit::stream_draft::groupless::Ingest::new(); ingest.feed(&msg).unwrap(); ingest.finish_transfer() }",
    doc_mod: "protobuf_edit::stream_draft::groupless::transfer",
}
...
crate::revise::groupless::revising_machine! {
    ...
    a_noun: "a draft",
    A_noun: "A draft",
    door: "{ let mut ingest = protobuf_edit::stream_draft::groupless::Ingest::new(); ingest.feed(&msg).unwrap(); ingest.finish_transfer_borrow() }",
    doc_mod: "protobuf_edit::stream_draft::groupless::transfer",
}
...
/// The sealed root layer under this module's own coordinate mint.
fn transfer_root(root: &super::Layer) -> Layer {
    Layer {
        first: root.first,
        last: root.last,
        dirty_kids: root.dirty_kids,
        history_kids: root.history_kids,
        source: root.source.map(transfer_run_id),
    }
}
...
impl Ingest {
    ...
    /// and the accumulated source moves without a copy.
    ///
    /// # Errors
    ///
    /// As [`Ingest::finish`], plus
    /// [`IngestFaultKind::Resource`] at [`ResourceSite::Rows`] when
    /// the allocator refuses the re-tagged row table.
    ///
    /// # Panics
    ///
    ...
    /// use protobuf_edit::stream_draft::groupless::transfer::InsertAt;
    ///
    ...
    /// assert_eq!(draft.save().unwrap(), [0x12, 0x02, 0x68, 0x69, 0x08, 0x05]);
    /// draft.revert();
    /// assert_eq!(draft.save().unwrap(), [0x08, 0x05, 0x12, 0x02, 0x68, 0x69]);
    /// ```
