//! The groupless editing markup: handle-based edits with precise
...
//! — at the root that refusal stops the open (the caller's buffer
//! was never touched), inside a payload it is a resident verdict
//! and the payload stays readable as bytes.
//!
...
//! [`Markup::descend`], whose verdict is resident.
//!
...
//!
//! Coordinates: write · buffered · offline · groupless · tolerant (type-level) · borrowed · revisable.
//!
//! # Examples
//!
//! ```
...
//! assert_eq!(markup.payload_bytes(tops[1]).unwrap(), [0x68, 0x69]);
//!
//! // The padded varint rides verbatim; the same-length payload
//! // keeps its prefix.
//! markup.set_payload(tops[1], &[0x79, 0x6F]).unwrap();
...
crate::revise::groupless::revising_machine! {
    ...
    doc_mod: "protobuf_edit::markup::groupless",
}
...
crate::revise::groupless::revising_machine! {
    ...
    #[doc = " `TransferBorrowMarkup` (feature `transfer-markup-groupless`) add"]
...
crate::revise::groupless::revising_machine! {
    ...
    #[doc = ""]
    #[doc = " ```"]
    #[doc = " use protobuf_edit::markup::groupless::MixMarkup;"]
    #[doc = ""]
