//! The grouped commission editor: groups are transparent
//! containers, edited with precise undo over a stable-replay
//! source under canonical admission.
//!
...
//! with no declared bound (the frame stack grows fallibly, like
//! every other growth edge here; descent into LENs stays
//! caller-stepped). Group grammar violations
...
//! varint value, or group end tag is lawful tolerant wire refused
//! by this machine's standard ([`FaultKind::NonMinimal`], policy
//! class; a padded end tag refuses at its own offset through the
//! tag site) — refused whole at the root, parked resident inside
//! a payload. Admission proved met ≡ minimal, so the rows store
//! no framing widths — every window derives from the record's own
//! facts — and every scanned scalar's decoded word is banked at
//! the scan. LEN payloads stay opaque until
...
//! moved; group framing tags ride verbatim around interior edits;
//! command-authored records emit minimally. Every output
...
//! fidelity reading exactly, with zero walks: the banked words
//! re-speak the scanned reading.
//!
...
//!
//! Coordinates: write · sequential-repeatable · offline · grouped · canonical (type-level) · revisable.
//!
//! # Examples
//!
//! ```
...
//! // edits, and revision restores the source exactly.
...
/// into LENs is caller-stepped, and group nesting rides a
/// fallible frame stack. Wire-declared quantities are quoted as
/// their wire types; a bad record never reaches the row table, so
/// its field number travels with the fault — inside the [`Stage`]
/// coordinate for varint reads and inside the [`NonMinimal`]
/// carrier for padding refusals (the tag site carries none: no
...
pub enum OpenFault<E> {
    /// The root layer violates the wire grammar (group framing
    /// included), the canonical admission standard, or the
...
impl Row {
    ...
    fn delim_w(&self) -> u64 {
        match self.kind {
            ...
            RecordKind::Group => u64::from(encoded_len32(group_end_word(self.field))),
    ...
    /// own width, a LEN's declared payload length. A group's
    /// extent rides the 8-byte role column instead
    /// ([`Row::span_end`]).
...
/// Groups arrive with a vacant word role — their close patches
/// the banked end into the role column.
#[allow(
    ...
    loop {
        ...
        let (word, _) = match pump.step_tag(Standard::CanonicalMinimal) {
            ...
            StepRead::NonMinimal { width } => {
                ...
                // width locates its first byte. A padded group end
                // tag refuses here too, at its own offset.
                return Err(Halt::Wire(Fault {
        ...
        match classify(low3) {
            ...
            TagClass::GroupEnd => {
                ...
                // the end tag) — the end tag's width is the
                // minimal theorem, the canonical step just proved
                // the spelling — then seals the interior chain
                // into the group's layer.
            ...
            fn size_pass<E>(&self) -> Result<(u64, Vec<u64>), SaveFault<E>> {
                ...
                loop {
                    let Some(id) = cur else {
                        ...
                        match frame {
                            SizeFrame::Len { next, outer, slot, at, tag_w } => {
                                let body = acc;
                                ...
                                acc = outer + tag_w + u64::from(encoded_len64(body)) + body;
                                cur = next;
                            }
