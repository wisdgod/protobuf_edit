//! The groupless commission editor: handle-based edits with
//! precise undo over a stable-replay source under canonical
//! admission, saved in two passes.
//!
//! This dialect speaks the four-code wire language: a group code
//! (3 or 4) is well-formed wire outside it, refused as a
//! capability judgment ([`FaultKind::GroupCode`]) distinct from
//! grammar faults — at the root that refusal stops the open (the
//! source rides back beside the mark), inside a payload it is a
//! resident verdict and the payload stays readable as bytes.
//!
//! Admission is canonical-minimal: a padded tag, length prefix,
//! or varint value is lawful tolerant wire refused by this
//! machine's standard ([`FaultKind::NonMinimal`], policy class) —
//! the same door split as the wire refusals: refused whole at the
//! root, parked resident inside a payload. Admission proved met ≡
//! minimal, so the rows store no framing widths — every window
//! derives from the record's own facts — and every scanned
//! scalar's decoded word is banked at the scan. The root layer is
//! flat and eager; LEN payloads stay opaque until
//! [`Commission::descend`], whose verdict is resident.
//!
//! The fidelity contract, face by face: records no live edit
//! touches ride into the output bit-exact (already minimal — the
//! door proved it); a replaced record keeps its source tag bytes
//! verbatim; a LEN prefix rides verbatim while its body length is
//! unchanged and is re-authored minimally only when the length
//! moved; command-authored records emit minimally. Every output
//! therefore closes under `CanonicalMinimal`, with the one
//! caller-declared exception: an authored payload's interior
//! passes through unchanged. Reverting a command restores the
//! fidelity reading exactly — `revert_all` makes the save the
//! source again — and costs no walk: the banked words re-speak
//! the scanned reading.
//!
...
//!
//! Coordinates: write · sequential-repeatable · offline · groupless · canonical (type-level) · revisable.
//!
//! # Examples
//!
//! ```
...
//!
//! // A same-length payload replacement keeps its prefix verbatim.
//! editor.set_payload(tops[1], &[0x79, 0x6F]).unwrap();
...
use crate::commission::FLAG_HIST;
...
use crate::replay_source::{
    AuthoredAt, FaultAt, Handed, NonMinimal, ReplayFault, ReplayPhase, ReplayWalk, SliceSource,
    SlotAt, SourceAt, SourceSpan, StableReplaySource, SupplyFault,
};
...
use crate::commission::{EditStatus, InsertAt};
...
/// and a [`ReadFault::SourceEnd`] names the zone's end; a
/// [`FaultKind::NonMinimal`] names the padded construct's first
/// byte; truncation kinds name the zone's end; structural kinds
...
/// refusal is the policy section: the declared standard is the
/// type-level pole itself. It declares no depth bound — descent
/// is caller-stepped and every scan is iterative. Wire-declared
...
/// elsewhere. Group grammar variants do not exist in this
...
impl FaultKind {
    ...
    /// for. Policy membership names its configuration datum: for
    /// the minimality refusal, the type-level canonical standard
    /// itself.
    #[inline]
    pub const fn class(self) -> FaultClass {
        match self {
            ...
            Self::NonMinimal(_) => FaultClass::Policy,
...
/// answer for the walked source bytes, not for any pending edit.
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
...
impl Row {
    ...
    /// Derived windows — the canonical theorem: admission proved
    /// every scanned spelling minimal, so widths are theorems of
    /// the record's own facts, never stored. Every use sits
    /// behind a state or kind dispatch that proves the row is
    /// scanned.
    fn tag_w(&self) -> u64 {
        u64::from(encoded_len32(head_word(self.field, self.kind)))
    }

    fn delim_w(&self) -> u64 {
        match self.kind {
            RecordKind::Len => u64::from(encoded_len64(self.len_or_met.payload_len(self.kind))),
            RecordKind::Varint | RecordKind::I32 | RecordKind::I64 => 0,
        }
    }

    /// The scanned value side's byte extent, derived per kind: a
    /// varint's banked word at its minimal spelling (the word is
    /// immutable — replacements go to the store — so the
    /// derivation is stable for the row's life), a fixed scalar's
    /// own width, a LEN's declared payload length.
    fn value_extent(&self) -> u64 {
        match self.kind {
            RecordKind::Varint => u64::from(encoded_len64(self.word_or_end.word(self.kind))),
            RecordKind::I32 => 4,
            RecordKind::I64 => 8,
    ...
    /// disjointness derives it — no end column exists.
...
fn push_row<E>(
    ...
    rows.push(Row {
        ...
        word_or_end: match kind {
            ...
            RecordKind::Len => ScalarWordOrGroupEnd::vacant(),
        },
        field,
        len_or_met,
        parent,
...
/// (the root: the sentinel). On any halt the caller discards the
/// provisional rows; nothing here touches published state.
#[allow(
    ...
    loop {
        ...
        let (word, _) = match pump.step_tag(Standard::CanonicalMinimal) {
            ...
            StepRead::NonMinimal { width } => {
                ...
                return Err(Halt::Wire(Fault {
                    at: zone.fault_at(pump.off - u64::from(width.w())),
        ...
        match classify(low3) {
            TagClass::Record(RecordKind::Varint) => {
                ...
                let (value, _) = match pump.step_value(Standard::CanonicalMinimal) {
                    StepRead::Done { value, width } => (value, width),
                    StepRead::SealCut => {
                        return Err(Halt::Wire(Fault {
                            at: zone.fault_at(pump.off),
            ...
            TagClass::GroupCode => {
                return Err(Halt::Wire(Fault {
                    at: zone.fault_at(at),
                    kind: FaultKind::GroupCode { field, code: low3 },
                }));
            }
            TagClass::Unassigned => {
                return Err(Halt::Wire(Fault {
...
/// while its body length is unchanged (minimal either way — the
/// canonical theorem). Windows are whole-source coordinates
/// derived from the record's own facts.
enum Arm {
...
struct SizeFrame {
    ...
    at: u64,
    /// The tag window (minimal — the admission theorem).
    tag_w: u64,
}
    ...
    ($self:ident, $script:ident, $lens:ident, $book:ident) => {{
        ...
        loop {
            let Some(id) = cur else {
                ...
                // An opened LEN's close: its prefix slot settles
...
macro_rules! commission_machine {
    (
        $(#[$mdoc:meta])*
        ...
        impl<$($plt,)? S: StableReplaySource> $Machine<$($plt,)? S> {
            ...
            pub fn open(mut source: S) -> Result<Self, (S, OpenFault<S::Error>)> {
                ...
                let outcome = match source.begin() {
                    Ok(walk) => {
                        let mut pump = Pump::new(walk);
                        scan_layer(&mut pump, &mut rows, ScanZone::Source, None, None)
            ...
            /// theorems of the record's own facts — the canonical
            /// door proved every scanned spelling minimal.
            /// Walk-free and source-infallible.
            ///
            /// # Errors
            ///
            ...
            pub fn narrowest(&self, pos: u64) -> Option<Handle> {
                ...
                while let Some(run_id) = run {
                    ...
                    if pos >= row.span_end(unsafe { run_at(row) }) {
                        // Records pave their layer without gaps, so
                        // only the root run's tail can leave a
                        // candidate short: the position lies past
                ...
                match outcome {
                    Ok(chain) => {
                        match self.seal_scan(id, chain.first, chain.prev, ScanZone::Authored {
                            ...
                            Ok(()) => Ok(None),
                            Err(fault) => {
                                // Discard the provisional rows: the
                                // slot publishes whole or not at
            ...
            pub fn materialize(
                ...
                for &(start, end, id) in &pending {
                    ...
                    let outcome =
                        scan_layer(&mut pump, rows, ScanZone::Source, Some(id), Some(end));
                    pump.zone = u64::MAX;
                    match outcome {
                        Ok(chain) => {
                            seal_scan_in(
            ...
            commission_machine!(@set_payload $pay $(<$plt>)?, Machine: $Machine);
            ...
            commission_machine!(@insert_payload $pay $(<$plt>)?, Machine: $Machine);
            ...
            /// words re-speak the scanned reading — with one
            /// walk-visible consequence: re-descending a re-sealed
            /// source container later costs one fresh walk (fresh
            /// rows, a fresh layer, a fresh run).
            #[inline]
            ...
            /// (Already minimal — the door proved it.)
            #[inline]
            ...
            /// Serializes into a fresh `Vec<u8>`: the sizing pass
                ...
                match fold(&mut self.source, &script, self.total.as_inner(), &mut |view| {
                    ...
                    Ok(()) => {
                        assert!(
                            ...
                            "commission save: the fold emits exactly the compiled length"
                        );
                        Ok(())
                    }
                    Err(fault) => {
            ...
            /// the cross-save identity recipe in [`crate::commission`]
            ...
            pub fn save_spans(&self) -> Result<SaveSpans, SaveFault<S::Error>> {
                ...
                assert!(covered == total, "commission spans: the span walk covers the priced save");
                Ok(SaveSpans { entries })
            }

        }

        commission_machine!(@save_view $Machine $(<$plt>)?, store: $Store);
    };
        ...
        impl<'a $(, $plt)?> SaveView<'a, $Store> {
            ...
            fn settle(&self, row: &Row) -> Arm {
                match row.edit {
                    ...
                    Edit::Inserted(value) => Arm::NewValue {
            ...
            fn scalar_word(&self, kind: RecordKind, value: ValueAt) -> Word {
                match kind {
                    ...
                    // rows.
                    RecordKind::Len => unreachable!("scalar value states sit on scalar rows"),
                }
            }
                ...
                loop {
                    let Some(id) = cur else {
                        ...
                        if body > u64::from(PayloadLen::MAX.as_inner()) {
                            return Err(SaveFault::BodyOverCap { at: frame.at });
                        }
                        bodies[frame.slot] = body;
                        // The prefix is the settled body's minimal
                        // width either way under the canonical
                        // door: an unchanged body's source spelling
                        // is that same width (the booking walk
                        // rides it through the copy step).
                        acc = frame.outer + frame.tag_w + u64::from(encoded_len64(body)) + body;
                        cur = frame.next;
                        continue;
                    };
                    let row = self.row(id);
                    match self.settle(row) {
                        ...
                        Arm::ReBody { slot, .. } => {
                            ...
                            acc += row.tag_w() + u64::from(encoded_len64(len)) + len;
                        }
                        Arm::NewBody { head, slot } => {
                            let len = self.store.zone_bytes(slot).len() as u64;
                ...
                loop {
                    let Some(id) = cur else {
                        ...
                        let Some(at) = frames.pop() else {
                            unreachable!("commission spans: climb without an open frame")
                        };
                    ...
                    match self.settle(row) {
                        ...
                        Arm::ReBody { slot, .. } => {
                            ...
                            // The prefix is minimal on both booking
                            // paths (the canonical identity).
                            let len = row.tag_w() + u64::from(encoded_len64(plen)) + plen;
                            entries.push((Handle(id), SourceSpan::new(out, out + len)));
                            out += len;
                        }
                        Arm::NewBody { head, slot } => {
                        ...
                        Arm::Spine { first, .. } => {
                            ...
                            out += row.tag_w() + u64::from(encoded_len64(body));
                            open = Some(id);
                            cur = first;
                            continue;
                        }
...
commission_machine! {
    #[doc = " A commissioning editor over a stable-replay source, with"]
    #[doc = " copied payloads."]
    #[doc = ""]
    ...
    #[doc = " is exact: [`Commission::revert`] walks the log backwards and"]
    #[doc = " restores the save-observable state of the previous step,"]
    #[doc = " byte fidelity included, with zero walks; orphaned"]
    ...
    #[doc = ""]
    #[doc = " Every save closes under `CanonicalMinimal` outright —"]
    #[doc = " admission proved the source minimal and authored words emit"]
    #[doc = " minimal — so no separate canonical-output family exists,"]
    #[doc = " exactly as on the buffered review twin."]
    machine Commission,
    store: Store,
    pay: copy,
    door: "Commission::open(SliceSource::new(&msg))",
}

commission_machine! {
    #[doc = " A commissioning editor over a stable-replay source, with"]
    #[doc = " borrowed payloads: [`Commission`]'s sibling for callers whose"]
    ...
    #[doc = ""]
    #[doc = " Everything else is [`Commission`]'s contract: canonical"]
    #[doc = " admission, byte fidelity for untouched records (already"]
    #[doc = " minimal — the door proved it), exact undo, and monotonic"]
    #[doc = " storage — and it stays plain data over its source handle"]
    #[doc = " and payload borrows."]
    machine BorrowCommission<'p>,
    store: BorrowStore<'p>,
    pay: borrow,
    door: "BorrowCommission::open(SliceSource::new(&msg))",
}
...
commission_machine! {
    ...
    #[doc = ""]
    #[doc = " [`Commission`]'s sibling for callers who mix long-lived"]
...
const _: () = {
    ...
    let copy = core::mem::size_of::<Commission<SliceSource<'_>>>();
    let borrow = core::mem::size_of::<BorrowCommission<'_, SliceSource<'_>>>();
    let mixed = core::mem::size_of::<MixCommission<'_, SliceSource<'_>>>();
...
commission_frames!(Commission, PayloadFrame, SizedPayloadFrame);
commission_frames!(MixCommission<'p>, MixPayloadFrame, MixSizedPayloadFrame);
