//! The groupless refit: group codes are a capability refusal,
//! padded varint constructs a policy refusal.
//!
...
//! the format's unassigned codes. Admission is canonical-minimal:
//! a padded tag, length prefix, or varint value is lawful tolerant
//! wire refused by this machine's standard
//! ([`FaultKind::NonMinimal`], policy class) — at the root that
//! refusal stops the open (the source rides back beside the mark),
//! inside a payload it is a resident verdict and the payload stays
//! readable as bytes. The open walk scans the top layer alone —
//! every LEN payload is an opaque declaration until a descend
//! commits to it — and zero source bytes are retained: payloads
//! are skipped through the supply's own seek, and every scalar
//! word is banked in its row as the walk decodes it to step.
//!
...
//! verbatim is already canonical emission. Without groups the
//! partition theorem strengthens: the delimiter (only LEN's
//! length prefix remains) always *precedes* the payload, so both
//! the record span and the payload span are one branch-free
//! formula.
//!
//! Coordinates: write · sequential-repeatable · offline · groupless · canonical (type-level) · commit-only.
//!
//! # Examples
//!
//! ```
...
//! editor.set_payload(tops[1], b"world").unwrap();
//!
//! let saved = editor.save().unwrap();
//! assert_eq!(saved, [0x08, 0x07, 0x12, 0x05, b'w', b'o', b'r', b'l', b'd']);
//! ```
//!
//! A padded source refuses at the door — the same bytes the
//! tolerant replay editor (`overhaul`) accepts:
//!
//! ```
//! use protobuf_edit::DepthLimit;
...
/// none. No width column exists: canonical admission proved met ≡
/// minimal, so the tag window derives from the field and kind and
/// the prefix window from the declared length — theorems, never
/// stored facts (the tolerant replay editor's two width columns
/// are erased here; padding absorbs the bytes, and the erasure's
/// value is type-level — no width fact exists to keep coherent).
/// Authored rows carry no source geometry; their emission is
...
struct Row {
    ...
    /// zero, never read).
    start: u64,
    /// Scanned payload extent. LEN: declared; Varint: the value's
    /// scanned (minimal) width; I32/I64: 4/8. Authored rows: zero.
    payload_len: u64,
...
impl Row {
    ...
    /// declared length's minimal spelling (the admission theorem
    /// again); scalars carry none. Scanned rows only.
    ...
    /// End of the whole-record span (scanned rows only).
    const fn span_end(&self) -> u64 {
        self.start + self.tag_w() + self.delim_w() + self.payload_len
    }

    /// The payload extent's start (scanned rows only).
...
fn stage_value(script: &mut Script<'_>, kind: RecordKind, word: u64) {
    match kind {
        ...
        RecordKind::Len => unreachable!("the value arms carry scalar kinds only"),
    }
}
...
macro_rules! refit_machine {
    (
        ...
        impl<$($plt,)? S: StableReplaySource> $Machine<$($plt,)? S> {
            ...
            /// the canonical-minimal standard — LEN payloads stay
            /// opaque declarations, skipped through the supply's
            /// own seek — and measures the source's total length.
            /// An unlawful or padded root layer refuses whole; the
            /// source rides back beside the mark.
            ///
            /// `depth` bounds container nesting for the descend
            /// walks, with no default.
            ///
            /// # Errors
            ///
            /// `(source, OpenFault)` — the source beside the mark —
            /// when the root layer is unlawful or padded, the
            /// supply refuses mid-walk, the record count would
            /// leave the row-index class, or the offset would leave
            ...
            pub fn open(mut source: S, depth: DepthLimit) -> Result<Self, (S, OpenFault<S::Error>)> {
                ...
                let outcome = match source.begin() {
                    Ok(walk) => {
                        ...
                        scan_layer(&mut pump, &mut rows, None, None).map(|first| (first, pump.off))
                    }
            ...
            /// is the caller's target fault; `Ok(false)` means a
            /// verdict is already resident (or the extent is empty
            /// and opens walk-free); `Ok(true)` means a walk is
            /// owed.
            ...
            /// parses on the first call under the canonical
            /// standard — an explicit commitment that these bytes
            ...
            pub fn descend(
                ...
                let outcome = scan_extent(&mut self.source, &mut self.rows, id, start, end);
                match outcome {
                    Ok(first) => {
            ...
            /// Resolves several unopened LEN handles in one
            /// source-ordered walk — the batch face that makes k
            /// scattered descents cost one walk instead of k.
            /// Handles already carrying a resident verdict are
            /// skipped (idempotent); depth refusals park
            /// walk-free; each extent commits atomically, and a
            /// refusal inside one extent — wire or padding — parks
            /// on that handle while the walk continues to the
            /// next.
            ///
            /// # Errors
            ///
            ...
            pub fn materialize(
                ...
                for &handle in handles {
                    ...
                    pending.push((row.payload_at(), row.payload_at() + row.payload_len, id));
                }
                if pending.is_empty() {
                    return Ok(());
                }
                ...
                for &(start, end, id) in &pending {
                    ...
                    pump.zone = end;
                    let mark = rows.len();
                    let outcome = scan_layer(&mut pump, rows, Some(id), Some(end));
                    pump.zone = u64::MAX;
                    match outcome {
                        Ok(first) => {
        ...
        impl<'a $(, $plt)?> SaveView<'a, $Store> {
            ...
            fn size_pass<E>(&self) -> Result<(u64, Vec<u64>), SaveFault<E>> {
                struct Frame {
                    ...
                    slot: usize,
                    at: u64,
                    /// The tag window (minimal — the admission
                    /// theorem).
                    tag_w: u64,
                }
            ...
            fn compile<E>(&self) -> Result<Script<'a>, SaveFault<E>> {
                ...
                loop {
                    let Some(id) = cur else {
                        ...
                        // The pop epilogue: an opened LEN settles
...
/// by the open walk, so a short walk is a tear.
fn scan_extent<S: StableReplaySource>(
    ...
    pump.zone = end;
    scan_layer(&mut pump, rows, Some(parent), Some(end))
}

/// Scans one layer at the pump's position into the row arena
/// under the canonical-minimal standard, returning the chain's
/// first row.
///
