...
// ─── the sink save and the output span table, over groups ───

#[test]
fn the_sink_save_and_span_table_cover_groups() {
    // group f2 { varint f3 } · varint f1
    let data = h("13 18 01 14 08 2A");
    let mut p = open(&data);
    let t = tops(&p);
    let inner = p.children(t[0]).next().unwrap();
    p.set_varint(inner, 300).unwrap();
    let group = p.insert_group(InsertAt::TailOf(None), fnum(5)).unwrap();
    let fresh = p.insert_varint(InsertAt::TailOf(Some(group)), fnum(1), 7).unwrap();

    let expected = saved(&p);
    assert_eq!(expected, h("13 18 AC 02 14 08 2A 2B 08 07 2C"));
    ...
    // which closes before its container).
    let far = table.iter().map(|(_, span)| span.end()).max().unwrap();
    assert_eq!(far, p.save_len().unwrap());
    assert_eq!(&expected[table[3].1.as_range()], h("2B 08 07 2C").as_slice());
}

#[test]
fn a_clean_grouped_patch_spans_the_source_with_its_end_tags() {
    // A padded group frame rides verbatim into the table.
    let data = h("93 00 18 01 94 00 08 2A");
    let p = open(&data);
    let t = tops(&p);
    let inner = p.children(t[0]).next().unwrap();
    let spans = p.save_spans().unwrap();
    let table: Vec<_> = spans.iter().collect();
    assert_eq!(table, [(t[0], Span::new(0, 6)), (inner, Span::new(2, 4)), (t[1], Span::new(6, 8))]);
}

// ─── the scatter payload and the staged frame ───

#[test]
fn scatter_and_framed_payloads_equal_their_whole_slice_twins_over_groups() {
    // group f2 { LEN f3 "ab" } — the replaced LEN sits inside
    // group framing; a fresh LEN inserts into the group.
    let data = h("13 1A 02 61 62 14");
    let mut whole = open(&data);
    let group = tops(&whole)[0];
    let inner = whole.children(group).next().unwrap();
    ...
    let group = tops(&scattered)[0];
    let inner = scattered.children(group).next().unwrap();
    let set_parts: [&[u8]; 2] = [b"hel", b"lo"];
    scattered.set_payload_parts(inner, &set_parts).unwrap();
    let insert_parts: [&[u8]; 2] = [b"x", b"y"];
    scattered.insert_payload_parts(InsertAt::TailOf(Some(group)), fnum(4), &insert_parts).unwrap();
    assert_eq!(saved(&scattered), expected);
    assert_eq!(scattered.payload_bytes(inner), None, "no contiguous view before the gather");

    // The staged-frame twin: abandoned first, then finished.
    let mut framed = open(&data);
    let group = tops(&framed)[0];
    let inner = framed.children(group).next().unwrap();
    {
        let mut frame = framed.begin_set_payload(inner).unwrap();
        frame.write(b"junk").unwrap();
    }
    assert_eq!(saved(&framed), data, "an abandoned frame changes nothing");
    let mut frame = framed.begin_set_payload(inner).unwrap();
    ...
    assert_eq!(framed.payload_bytes(minted), Some(&b"xy"[..]));
}
...
fn abandoned_and_refused_frames_reclaim_the_staging_column() {
    ...
    {
        let mut frame =
            p.begin_insert_payload_sized(InsertAt::TailOf(Some(group)), fnum(4), 8).unwrap();
    ...
    let mut frame = p.begin_set_payload_sized(inner, 3).unwrap();
    frame.write(b"ab").unwrap();
    assert!(matches!(
        frame.finish().err(),
    ...
    p.set_payload(inner, b"hi").unwrap();
...
fn the_price_tracks_the_emission_at_every_stage() {
    // f1 varint · f5 group {f1 varint} · f3 varint · f4 LEN "abc"
    // — the pricing arithmetic and the fused save are independent
    // machines; equality at every stage is the differential, and a
    // group root's extent covers both framing tags.
    let data = h("08 01 2B 08 07 2C 18 2A 22 03 61 62 63");
    let mut p = open(&data);
    let t = tops(&p);
    #[track_caller]
    ...
    // Two edits under one group root: one splice root, priced once.
    let inner = p.children(t[1]).next().unwrap();
    ...
    // An authored group root: no source extent, plus its size —
    // and deleting it afterwards cancels both.
    let fresh = p.insert_group(InsertAt::TailOf(None), fnum(9)).unwrap();
    p.insert_varint(InsertAt::TailOf(Some(fresh)), fnum(1), 300).unwrap();
    check(&p);
    p.delete(fresh).unwrap();
    check(&p);

    // Dirty-then-deleted: the interior edit priced this group once;
    // deletion now prices it as pure removal, end tag included.
    p.delete(t[1]).unwrap();
    check(&p);
}
...
fn opens_scalars_lens_and_groups_in_one_layer() {
    ...
    assert_eq!(p.kind(t[1]), RecordKind::Len);
    assert_eq!(p.kind(t[2]), RecordKind::Group);
    // Groups materialize with the scan; LENs stay lazy.
    assert_eq!(p.children(t[2]).count(), 1);
    assert_eq!(p.children(t[1]).count(), 0);
...
fn tolerant_admission_stores_padded_widths_as_input_facts() {
    ...
    // to 2 · group with end tag padded to 2
    let data = h("8800 968100 12 8200 6869 13 1801 9400");
    let p = open(&data);
    let t = tops(&p);
    assert_eq!(t.len(), 3);
    // The padded varint still reads its value.
    ...
    let RecordSpans::Group { end_tag, .. } = p.source_spans(t[2]).unwrap() else { unreachable!() };
    assert_eq!((end_tag.start(), end_tag.end()), (13, 15));
}
...
#[test]
fn group_pairing_is_judged_at_scan() {
    assert!(matches!(
        Patch::open(&h("13 1801"), DepthLimit::REFERENCE).err(),
        Some(OpenFault::Wire(Fault { at: 0, kind: FaultKind::GroupUnclosed { open } }))
            if open.as_inner() == 2
    ));
    assert!(matches!(
        Patch::open(&h("14"), DepthLimit::REFERENCE).err(),
        Some(OpenFault::Wire(Fault { at: 0, kind: FaultKind::GroupEndOrphan { .. } }))
    ));
    assert!(matches!(
        Patch::open(&h("13 24"), DepthLimit::REFERENCE).err(),
        Some(OpenFault::Wire(Fault { at: 1, kind: FaultKind::GroupEndMismatch { .. } }))
    ));
}

#[test]
fn the_depth_bound_refuses_at_open_and_is_resident_at_descend() {
    // Two nested groups against a bound of one.
    let nested = h("13 13 14 14");
    assert!(matches!(
        Patch::open(&nested, DepthLimit::MIN).err(),
        Some(OpenFault::Refused(Refusal::DepthExceeded { at: 1, .. }))
    ));
    // A LEN inside a descended LEN against the same bound: the
...
fn an_unedited_patch_saves_the_input_bit_exactly() {
    // Padding in every framing role, groups included.
    let data = h("8800 968100 12 8200 6869 13 1801 9400 0D 01000000");
    let p = open(&data);
    assert_eq!(saved(&p), data);
...
fn descents_are_reads_and_spend_no_fidelity() {
    // padded varint · LEN{varint} · group{varint} · I32. Descend
    // the LEN (the group is open from the scan) and edit nothing:
    // the save is still the input, bit for bit — the walkless
    // clean arm rests on the dirty witness, and reads never raise
    // it.
    let data = h("8800 968100 12 8200 0801 13 1801 9400 0D 01000000");
    let mut p = open(&data);
    let t = tops(&p);
    assert!(matches!(p.descend(t[1]).unwrap(), Descent::Opened { .. }));
    assert_eq!(saved(&p), data);
    // An edit elsewhere leaves the opened, untouched containers on
    // their verbatim arm: framing and interiors ride bit-exactly.
    p.set_i32(t[3], 0xAA).unwrap();
    assert_eq!(saved(&p), h("8800 968100 12 8200 0801 13 1801 9400 0D AA000000"));
}
...
#[test]
fn a_group_end_tag_rides_verbatim_under_interior_edits() {
    let data = h("13 1801 9400");
    let mut p = open(&data);
    let group = p.top().next().unwrap();
    let inner = p.children(group).next().unwrap();
    p.set_varint(inner, 9).unwrap();
    assert_eq!(saved(&p), h("13 1809 9400"));
}
...
fn a_deleted_record_vanishes_whole_with_its_subtree() {
    // group f2 { varint f3 · LEN f3 } · varint f1
    let data = h("13 1801 1A020801 14 0807");
    let mut p = open(&data);
    let t = tops(&p);
...
fn descend_is_an_explicit_commitment_with_resident_verdicts() {
    ...
    // an unclosed group (a wire fault under commitment).
    let data = h("12020801 1A010B");
    let mut p = open(&data);
    let t = tops(&p);
    ...
    assert!(matches!(
        ...
        Descent::Faulted(Fault { at: 6, kind: FaultKind::GroupUnclosed { open } }) if open.as_inner() == 1
    ));
    assert!(matches!(
        p.descend(t[1]).unwrap(),
        Descent::Faulted(Fault { at: 6, kind: FaultKind::GroupUnclosed { open } }) if open.as_inner() == 1
    ...
    assert_eq!(p.payload_bytes(t[1]).unwrap(), h("0B"));
...
#[test]
fn groups_are_open_from_the_scan() {
    let data = h("13 1801 14");
    let mut p = open(&data);
    let group = p.top().next().unwrap();
    let Descent::Opened { first: Some(inner) } = p.descend(group).unwrap() else { unreachable!() };
    assert_eq!(p.varint_word(inner).unwrap(), 1);
}
...
#[test]
fn insertion_reaches_groups_descended_lens_and_authored_groups() {
    let data = h("13 14 12020801");
    let mut p = open(&data);
    let t = tops(&p);
    // A scanned group is open from the scan.
    p.insert_varint(InsertAt::TailOf(Some(t[0])), fnum(1), 1).unwrap();
    // A LEN needs the descent commitment first.
    assert_eq!(
        p.insert_varint(InsertAt::TailOf(Some(t[1])), fnum(1), 2).err(),
        Some(EditFault::TargetUnopened)
    );
    assert!(matches!(p.descend(t[1]).unwrap(), Descent::Opened { .. }));
    p.insert_varint(InsertAt::TailOf(Some(t[1])), fnum(1), 2).unwrap();
    // An authored group grows purely by insertion.
    let group = p.insert_group(InsertAt::TailOf(None), fnum(2)).unwrap();
    p.insert_varint(InsertAt::TailOf(Some(group)), fnum(3), 3).unwrap();
    assert_eq!(saved(&p), h("13 0801 14 1204 0801 0802 13 1803 14"));
}

#[test]
fn insertion_gates_refuse_scalars_deleted_and_authored_containers() {
    let data = h("089601 12026869 13 14");
    let mut p = open(&data);
    let t = tops(&p);
    assert_eq!(
...
fn narrowest_resolves_source_positions_to_the_materialized_depth() {
    // group f1 (0..4: open tag 0, interior varint f1=1 at 1..3,
    // end tag 3) · LEN f2 len=2 (4..8) wrapping varint f1=1.
    let data = h("0B 0801 0C 12 02 0801");
    let mut p = open(&data);
    let t = tops(&p);
    // Group framing tags name the group; the interior is there
    // from the scan and needs no descend.
    assert_eq!(p.narrowest(0), Some(t[0]));
    assert_eq!(p.narrowest(3), Some(t[0]));
    let inner_group = p.children(t[0]).next().unwrap();
    assert_eq!(p.narrowest(1), Some(inner_group));
    assert_eq!(p.narrowest(2), Some(inner_group));
    // The whole LEN span answers as the LEN while unopened; a
    // descend deepens payload positions only.
    ...
    assert_eq!(p.narrowest(5), Some(t[1]));
    // Past the document: no owner.
    assert_eq!(p.narrowest(8), None);
}
