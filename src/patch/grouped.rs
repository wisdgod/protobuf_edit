//! The grouped one-shot patch: borrowed input, width-carrying
...
//! and the patch lives on. Descent is an explicit commitment:
...
//!
//! Coordinates: write · buffered · offline · grouped · tolerant (type-level) · borrowed · commit-only.
//!
//! # Examples
//!
//! ```
...
//! let inner = patch.children(tops[1]).next().unwrap();
...
crate::editor::grouped::one_shot_machine! {
    ...
    /// `transfer-patch-grouped`) adds whole-record relocation,
    ...
    a_noun: "a patch",
    A_noun: "A patch",
    doc_mod: "protobuf_edit::patch::grouped",
