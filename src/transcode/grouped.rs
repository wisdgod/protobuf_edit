//! The grouped transcoder: groups walk by syntax, the free
//! layer is the root plus pure group chains.
//!
//! Suppression (a dropped group) is a stack-floor marker: inside
//! it no rule is asked and nothing is emitted, but wire law runs
//! unchanged — nested groups still frame and pair, LEN payloads
//! skip by count, depth still binds. The suppressed group's end
//! tag emits nothing and notifies nothing (the rule never saw an
//! enter). Dropping is a free-layer verb, so a suppressed walk
//! always sits at the root zone: no entered LEN exists anywhere
//! while the floor is set.
//!
//! Coordinates: write · stream · online · grouped · Standard (value-level) · commit-only.
//!
//! # Examples
//!
//! ```
...
//! use protobuf_edit::transcode::grouped::{FreeGroup, Rule, Transcoder};
//! use protobuf_edit::{DepthLimit, FieldNumber};
//!
//! // Drop the field-2 group tree whole; keep everything else.
//! struct DropGroup2;
//! impl Rule for DropGroup2 {
//!     fn on_group(&mut self, _at: u64, field: FieldNumber) -> FreeGroup<'_> {
//!         if field.as_inner() == 2 { FreeGroup::Drop } else { FreeGroup::Keep }
//!     }
//! }
//!
//! // varint f1=150 · group f2 { varint f3=1 } · varint f1=42
//! let msg = [0x08, 0x96, 0x01, 0x13, 0x18, 0x01, 0x14, 0x08, 0x2A];
//! let mut out = Vec::new();
//! let mut t = Transcoder::new(Standard::Tolerant, DepthLimit::REFERENCE);
//! t.feed(&msg, &mut DropGroup2, &mut |b: &[u8]| out.extend_from_slice(b)).unwrap();
//! t.finish(&mut DropGroup2, &mut |b: &[u8]| out.extend_from_slice(b)).unwrap();
//! assert_eq!(out, [0x08, 0x96, 0x01, 0x08, 0x2A]);
//! ```
...
/// A free-layer group verdict.
#[must_use = "the verdict determines this record's output"]
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
pub enum FreeGroup<'a> {
    /// Frame tags ride verbatim; the body is walked and asked.
    Keep,
    /// The whole tree vanishes (suppressed to the matching end;
    /// rules inside are never asked).
    Drop,
    ...
    /// chunk source ([`Rule::on_source`]), then ask again
    /// ([`FreeScalar::InsertSource`]'s account contract).
    InsertSource(PayloadLen),
}

/// The rule program: one ask per record at its decision point.
///
/// Free and locked faces are separated so the variable-length
/// verbs are unspellable where the length algebra is sealed. All
/// defaults keep/pass — `impl Rule for ()` is the bit-identical
/// transcoder.
///
/// Answers must not depend on input chunking (fragment boundaries
/// carry no meaning); `at` arguments are absolute input offsets of
/// the record head.
pub trait Rule {
    ...
    /// A free-layer LEN head completed (payload disposition).
    fn on_len(&mut self, at: u64, field: FieldNumber, len: PayloadLen) -> FreeLen<'_> {
        let _ = (at, field, len);
        FreeLen::Pass
    }

    /// A free-layer group opened.
    fn on_group(&mut self, at: u64, field: FieldNumber) -> FreeGroup<'_> {
        let _ = (at, field);
        FreeGroup::Keep
    }

    /// A locked-layer varint completed. Rewrites are equal-length:
    /// padded to the source width under `Tolerant`, exact-width
    ...
    /// A redirected payload fragment arrived (Transform/Divert).
    /// The returned bytes emit in place immediately; fragment
    /// boundaries follow input chunking and carry no meaning.
    ...
    /// it returns empty; returned bytes emit in place.
    ...
    /// A kept group opened (fires on both layers — the locked
    /// enter has no ask, so the notification is the single face
    /// path tracking hangs on).
    fn on_group_enter(&mut self, field: FieldNumber, at: u64) {
        let _ = (field, at);
    }

    /// A kept group closed (`at` = the end tag's head).
    fn on_group_exit(&mut self, field: FieldNumber, at: u64) {
        let _ = (field, at);
    }

    /// The stream tail (root layer, free by definition): called
    /// repeatedly until it returns empty; returned bytes emit
...
pub enum WireBreach {
    ...
    Truncated,
    /// Group framing broke (orphaned, mismatched, crossing,
    /// unclosed).
    Grouping,
...
impl WireBreach {
    ...
    /// the one capability member is the coordinate ceiling
    /// ([`OffsetExhausted`](Self::OffsetExhausted)) — the dialect's
    ...
    pub const fn class(self) -> FaultClass {
        match self {
            ...
            Self::OffsetExhausted => FaultClass::Capability,
        }
    }
}
...
impl core::fmt::Display for WireBreach {
    ...
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match *self {
            ...
            Self::Grouping => f.write_str("group framing broke"),
...
/// The rule-breach classes: the residue the type split cannot
/// eliminate (lengths are runtime data).
#[non_exhaustive]
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
pub enum RuleFaultKind {
    /// A locked varint rewrite wider than the source (any
    /// standard).
    RewriteOverflow {
        /// The record's field.
        field: FieldNumber,
    ...
    /// (under `CanonicalMinimal`, where padding would break
    /// re-ingestion).
    ...
    /// A replacement's length differs from the announced payload
    /// length.
    ReplaceLenMismatch {
        /// The record's field.
        field: FieldNumber,
...
/// endpoint (the live one rides the pump); both kinds keep their
...
/// The one-pass streaming transcoder.
///
/// Terminal states are final: after a fault, `feed`/`finish`
/// panics (a caller bug); a clean end goes through
/// `finish(self)`. Only after `finish` returns `Ok` does the
/// emitted byte sequence carry any promise.
#[must_use]
pub struct Transcoder {
    ...
    depth: DepthLimit,
    stage: StagedHead,
    /// Suppression floor: the dropped group's frame index.
    floor: Option<usize>,
}

impl Transcoder {
    /// All configuration is explicit: the input acceptance
    /// standard (which also scopes the equal-length rewrite
    /// judgment, so output re-ingests under it) and the nesting
    /// bound.
    #[inline]
    ...
    fn drive<R: Rule, O: FnMut(&[u8]), const MINIMAL: bool>(
        ...
        loop {
            ...
            // Suppression sits at the root zone by construction,
            // so no endpoint ever arrives suppressed: the exit
            // notification needs no gate.
            while self.pump.off == self.pump.zone {
                ...
                match self.stack.pop() {
                    Some(Frame::Len { prev_zone, field }) => {
                        self.pump.zone = prev_zone;
                        rule.on_len_exit(field, self.pump.off);
                    }
                    Some(Frame::Group { field: _ }) => {
                        return Err(self.wire(self.pump.off, WireBreach::Grouping));
                    }
    ...
    /// # Errors
    ///
    /// EOF inside a construct or a counted payload, or a container
    /// still open, is the matching breach summarized into
    ...
    /// use protobuf_edit::transcode::Standard;
    /// use protobuf_edit::transcode::grouped::{Fault, Transcoder, WireBreach};
    ///
    /// // A varint tag arrived; its value never did.
    /// let mut t = Transcoder::new(Standard::Tolerant, DepthLimit::REFERENCE);
    /// t.feed(&[0x08], &mut (), &mut |_: &[u8]| {}).unwrap();
    /// let fault = t.finish(&mut (), &mut |_: &[u8]| {}).unwrap_err();
    ...
    pub fn finish<R: Rule, O: FnMut(&[u8])>(
        ...
        // — and a still-open LEN summarize alike: the stream ended
        // inside a record, [`WireBreach::Truncated`] at the final
        // offset. A still-open group is the framing breach.
        match self.mode {
        ...
        match self.stack.last() {
            ...
            Some(&Frame::Group { .. }) => {
                return Err(self.wire(at, WireBreach::Grouping));
            }
            None => {}
        }
        loop {
    ...
    fn head<R: Rule, O: FnMut(&[u8]), const MINIMAL: bool>(
        ...
        // The pump leaves the carry holding the completed tag until
        // the record stages it, so the construct start is still live
        // for the group punctuation and the structural refusals.
        let start = self.pump.construct_start();
        let low3 = Low3::from_word(word);
        let Some(field) = FieldNumber::from_word(word) else {
            return self.halt_wire(start, WireBreach::Tag);
        };
        match classify(low3) {
            ...
            TagClass::Record(RecordKind::Group) => self.group_open(field, start, rule, out),
            TagClass::GroupEnd => self.group_end(field, start, rule, out),
            TagClass::Unassigned => self.halt_wire(start, WireBreach::Tag),
        }
    }

    /// Stages the completed head for the record's verdict and
    /// frees the carry for the value construct. Suppressed records
    /// skip the copy: nothing inside a dropped tree is asked or
    /// emitted, so the head has no consumer.
    ...
    fn group_open<R: Rule, O: FnMut(&[u8])>(
        &mut self,
        field: FieldNumber,
        at: u64,
        rule: &mut R,
        ...
        if self.stack.len() >= usize::from(self.depth.as_inner()) {
            return self.halt_wire(at, WireBreach::Depth);
        }
        if self.suppressed() {
            self.stack.push(Frame::Group { field });
            self.pump.carry.clear();
            return ControlFlow::Continue(());
        }
        // The group ask is a free-layer verb; a locked walk opens
        // kept groups without asking (the enter notification is
        // the locked layer's whole group face).
        if !self.pump.locked() {
            loop {
                match rule.on_group(at, field) {
                    FreeGroup::Keep => break,
                    FreeGroup::Drop => {
                        self.floor = Some(self.stack.len());
                        self.stack.push(Frame::Group { field });
                        self.pump.carry.clear();
                        return ControlFlow::Continue(());
                    }
                    FreeGroup::Insert(bytes) => out(bytes),
                    FreeGroup::InsertSource(need) => {
                        if let Err(fault) = self.pump_source(rule, out, need, field, at) {
        ...
        self.pump.carry.clear();
        self.stack.push(Frame::Group { field });
        rule.on_group_enter(field, self.pump.off);
        ControlFlow::Continue(())
    }

    fn group_end<R: Rule, O: FnMut(&[u8])>(
        &mut self,
        end: FieldNumber,
        at: u64,
        rule: &mut R,
        ...
        match self.stack.last() {
            None => self.halt_wire(at, WireBreach::Grouping),
            Some(&Frame::Len { .. }) => self.halt_wire(at, WireBreach::Grouping),
            Some(&Frame::Group { field }) if field != end => {
                self.halt_wire(at, WireBreach::Grouping)
            }
            Some(&Frame::Group { field }) => {
                self.stack.pop();
                if self.floor == Some(self.stack.len()) {
                    // The dropped tree's own end tag: suppression
                    // ends silently (the rule never saw an enter).
                    self.floor = None;
                } else if !self.suppressed() {
                    out(self.pump.carry.bytes());
                    rule.on_group_exit(field, at);
                }
                self.pump.carry.clear();
                ControlFlow::Continue(())
            }
    ...
    fn varint_value<R: Rule, O: FnMut(&[u8]), const MINIMAL: bool>(
        ...
        if self.suppressed() {
            // Wire law ran in the step; the record has no other
            // consumer inside a dropped tree.
        } else if self.pump.locked() {
    ...
    fn len_word<R: Rule, O: FnMut(&[u8]), const MINIMAL: bool>(
        ...
        // unsealed cursor (the root, or inside an open group), an end needing the sentinel coordinate (or beyond)
    ...
    fn settle_len<O: FnMut(&[u8])>(
        ...
        match verb {
            ...
            LenVerb::Commit => {
                ...
                out(self.stage.bytes());
                out(self.pump.carry.bytes());
                self.stack.push(Frame::Len { prev_zone: self.pump.zone, field });
                self.pump.zone = end;
                self.mode = Mode::Head;
            }
