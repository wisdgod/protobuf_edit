...
thread_local! {
    ...
    static DECOMPRESSED_ERRORS: RefCell<FxHashMap<MessageId, String>> = RefCell::new(FxHashMap::default());
}
...
pub(crate) fn cached_decompressed_error(id: MessageId) -> Option<String> {
    DECOMPRESSED_ERRORS.with(|cell| cell.borrow().get(&id).cloned())
}

pub(crate) fn store_decompressed_error(id: MessageId, error: String) {
    DECOMPRESSED_BYTES.with(|cell| {
        cell.borrow_mut().remove(&id);
    });
    DECOMPRESSED_ERRORS.with(|cell| {
